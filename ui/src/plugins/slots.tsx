 ...
 *
 * Plugin bundles are built with `external: ["@jasminiaai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__jasminiaPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__jasminiaPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__jasminiaPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__jasminiaPluginBridge__?.sdkUi ?? {};
        function missing(name) {
          return function MissingJasmin.iaSdkUiComponent() {
            throw new Error('Jasmin.ia plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@jasminiaai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@jasminiaai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@jasminiaai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@jasminiaai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@jasminiaai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__jasminiaPluginBridge__) {
 ...
 * self-register via `window.jasminiaPlugins.registerReactComponent()`.
 ...
 * of `@jasminiaai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
