...
import { PROJECT_COLORS, isUuidLike, type BudgetPolicySummary } from "@jasminiaai/shared";
...
function ProjectIssuesList({ projectId, companyId }: { projectId: string; companyId: string }) {
  ...
  return (
    <IssuesList
      ...
      projectId={projectId}
      viewStateKey="jasminia:project-issues-view"
...
function ProjectPluginOperationsList({
  ...
  return (
    <IssuesList
      ...
      liveIssueIds={liveIssueIds}
      projectId={projectId}
      viewStateKey={`jasminia:project-plugin-operations-view:${pluginKey}`}
      onUpdateIssue={(id, data) => updateIssue.mutate({ id, data })}
    />
  );
}
...
export function ProjectDetail() {
  ...
  if (routeProjectRef && activeTab === null) {
    ...
    if (project?.id) {
      try { cachedTab = localStorage.getItem(`jasminia:project-tab:${project.id}`); } catch {}
    }
  ...
  const handleTabChange = (tab: ProjectTab) => {
    ...
    if (project?.id) {
      try { localStorage.setItem(`jasminia:project-tab:${project.id}`, tab); } catch {}
    }
