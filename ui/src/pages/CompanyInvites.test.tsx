...
vi.mock("@/context/CompanyContext", () => ({
  useCompany: () => ({
    ...
    selectedCompany: { id: "company-1", name: "Jasmin.ia", issuePrefix: "PAP" },
  }),
}));
...
describe("CompanyInvites", () => {
  ...
  const inviteHistory = Array.from({ length: 25 }, (_, index) => {
    ...
    return {
      ...
      invitedByUser: {
        ...
        email: `board${inviteNumber}@jasminia.local`,
        image: null,
      },
  ...
  beforeEach(() => {
    ...
    createCompanyInviteMock.mockResolvedValue({
      inviteUrl: "https://jasminia.local/invite/new-token",
  ...
  it("renders a human-only invite flow and keeps invite history in a table", async () => {
    ...
    expect(clipboardWriteTextMock).toHaveBeenCalledWith("https://jasminia.local/invite/new-token");
    expect(container.textContent).toContain("Latest invite link");
    expect(container.textContent).toContain("This URL includes the current Jasmin.ia domain returned by the server.");
    expect(container.textContent).toContain("https://jasminia.local/invite/new-token");
    ...
    const inviteFieldButton = Array.from(container.querySelectorAll("button")).find(
      (button) => button.textContent?.includes("https://jasminia.local/invite/new-token"),
    );
