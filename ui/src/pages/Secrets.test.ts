...
import type { SecretProviderDescriptor } from "@jasminiaai/shared";
import {
...
describe("Secrets page provider helpers", () => {
  it("previews the derived AWS managed path from provider health details", () => {
    ...
    ).toBe("jasminia/prod-us-1/company-123/anthropic-api-key");
  });
  ...
  it("uses provider health copy when an unconfigured provider reports missing bootstrap inputs", () => {
      ...
      providers: [
        {
          ...
          status: "warn",
          message:
            "AWS Secrets Manager provider is not ready: missing JASMINIA_SECRETS_AWS_DEPLOYMENT_ID.",
        },
      ],
    };
    ...
    expect(
      ...
      "AWS Secrets Manager is not configured in this deployment. AWS Secrets Manager provider is not ready: missing JASMINIA_SECRETS_AWS_DEPLOYMENT_ID.",
    );
  });
