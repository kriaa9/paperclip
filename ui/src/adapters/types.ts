...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@jasminia/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@jasminia/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@jasminia/adapter-utils").TranscriptEntry[];
