...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Jasmin.ia API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "jasminiaApiUrl",
                  String(config.jasminiaApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "jasminiaApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://jasminia.example"
            />
          </Field>
          ...
          <Field label="Claimed API key path">
            <DraftInput
              ...
              immediate
              className={inputClass}
              placeholder="~/.openclaw/workspace/jasminia-claimed-api-key.json"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "jasminia"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="jasminia"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Jasmin.ia persists a device key during onboarding so pairing approvals
