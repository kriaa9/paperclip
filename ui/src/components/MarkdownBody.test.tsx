...
describe("MarkdownBody", () => {
  ...
  it("renders user, agent, project, and skill mentions as chips", () => {
    const html = renderToStaticMarkup(
      <QueryClientProvider client={new QueryClient()}>
        <ThemeProvider>
          <MarkdownBody>
            {`[@Taylor](${buildUserMentionHref("user-123")}) [@CodexCoder](${buildAgentMentionHref("agent-123", "code")}) [@Jasmin.ia App](${buildProjectMentionHref("project-456", "#336699")}) [/release-changelog](${buildSkillMentionHref("skill-789", "release-changelog")})`}
          </MarkdownBody>
        </ThemeProvider>
      </QueryClientProvider>,
    );
    ...
    expect(html).toContain("--jasminia-mention-icon-mask");
    expect(html).toContain('href="/projects/project-456"');
    expect(html).toContain('data-mention-kind="project"');
    expect(html).toContain("--jasminia-mention-project-color:#336699");
  ...
  it("linkifies bare issue identifiers in markdown text", () => {
    ...
    expect(html).toContain("jasminia-markdown-issue-ref");
    expect(html).not.toContain("jasminia-mention-chip--issue");
  });
  ...
  it("preserves absolute issue URLs as external links", () => {
    ...
    expect(html).not.toContain("jasminia-markdown-issue-ref");
  });
  ...
  it("leaves wiki links as text unless explicitly enabled", () => {
    const html = renderMarkdown("See [[wiki/entities/jasminia]].");

    expect(html).toContain("[[wiki/entities/jasminia]]");
    expect(html).not.toContain('href="/wiki/page/wiki/entities/jasminia.md"');
  });
  ...
  it("renders wiki links with a custom resolver when enabled", () => {
    ...
    expect(html).toContain('href="/wiki/page/wiki/entities/jasminia.md"');
    expect(html).toContain('data-jasminia-wiki-link="true"');
    expect(html).toContain('data-jasminia-wiki-target="wiki/entities/jasminia"');
    expect(html).toContain(">Jasmin.ia</a>");
    expect(html).toContain('href="/wiki/page/wiki/entities/dotta-b.md"');
    expect(html).toContain(">wiki/entities/dotta-b</a>");
    expect(html).not.toContain("[[wiki/entities/jasminia");
  });

  it("keeps wiki links as text when the custom resolver rejects them", () => {
    const html = renderMarkdown(
      "See [[wiki/entities/jasminia]].",
      [],
      {
        enableWikiLinks: true,
    ...
    expect(html).toContain("[[wiki/entities/jasminia]]");
    expect(html).not.toContain('data-jasminia-wiki-link="true"');
    expect(html).not.toContain('href="/wiki/page/wiki/entities/jasminia"');
  });

  it("does not render wiki links inside code spans or code blocks", () => {
    const html = renderMarkdown(
      "Inline `[[wiki/entities/jasminia]]`.\n\n```md\n[[wiki/entities/dotta-b]]\n```",
      [],
      {
        enableWikiLinks: true,
        wikiLinkRoot: "/wiki/page",
  ...
  it("applies wrap-friendly styles to long inline content", () => {
    ...
    expect(html).toContain('class="jasminia-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden');
  ...
  it("renders markdown tables in a horizontally scrollable region", () => {
    ...
    expect(html).toContain('class="jasminia-markdown-table-scroll"');
  ...
  it("renders a copy button alongside fenced code blocks", () => {
    ...
    expect(html).toContain("jasminia-markdown-codeblock");
    expect(html).toContain("jasminia-markdown-codeblock-copy");
  ...
  it("does not render a copy button on inline code", () => {
    ...
    expect(html).not.toContain("jasminia-markdown-codeblock-copy");
  });
