...
import type { Issue, Project } from "@jasminiaai/shared";
...
describe("IssuesList", () => {
  ...
  it("renders server search results instead of filtering the full issue list locally", async () => {
    ...
    const { root } = renderWithQueryClient(
      <IssuesList
        ...
        agents={[]}
        projects={[]}
        viewStateKey="jasminia:test-issues"
  ...
  it("uses workspace group defaults when creating an issue from a grouped section", async () => {
    localStorage.setItem(
      "jasminia:test-issues:company-1",
  ...
  it("uses context-scoped persisted column visibility", async () => {
    localStorage.setItem("jasminia:test-issues:company-1:issue-columns", JSON.stringify(["id", "assignee"]));
  ...
  it("filters the list to a single workspace when a workspace name is clicked", async () => {
    localStorage.setItem("jasminia:test-issues:company-1:issue-columns", JSON.stringify(["id", "workspace"]));
    mockInstanceSettingsApi.getExperimental.mockResolvedValue({ enableIsolatedWorkspaces: true });
    mockExecutionWorkspacesApi.listSummaries.mockResolvedValue([
      {
        id: "workspace-alpha",
