...
type Jasmin.iaSprite = (typeof JASMINIA_SPRITES)[number];
...
function spriteSize(sprite: Jasmin.iaSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = JASMINIA_SPRITES[(baseCol + baseRow) % JASMINIA_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = JASMINIA_SPRITES[Math.floor(Math.random() * JASMINIA_SPRITES.length)]!;
