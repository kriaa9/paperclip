...
describe("IssueChatThread system notice routing", () => {
  it("renders authorType=system comments as a SystemNotice rather than a user bubble", () => {
    ...
    expect(container.textContent).toContain("Jasmin.ia needs a disposition");
  ...
  it("labels system notice source as Jasmin.ia when no run agent can be resolved", () => {
    const comment: IssueChatComment = {
    ...
    const status = container.querySelector('[role="status"]');
    expect(status).not.toBeNull();
    expect(status?.textContent).toContain("Jasmin.ia");
    expect(status?.textContent).not.toContain("You");
  });

  it("falls back to Jasmin.ia in the system notice header when run agent is unknown to agentMap", () => {
    const comment: IssueChatComment = {
    ...
    expect(sourceLink?.textContent).toBe("Jasmin.ia");
  });
  ...
  it("folds stale successful-run disposition warnings into the activity log disclosure style", () => {
    ...
    expect(row?.textContent).not.toContain("Jasmin.ia needs a disposition before this issue can continue.");
