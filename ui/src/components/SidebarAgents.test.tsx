...
describe("SidebarAgents", () => {
  ...
  it("keeps top mode in stored org-aware order", async () => {
    localStorage.setItem("jasminia.agentOrder:company-1:user-1", JSON.stringify(["agent-b", "agent-a", "agent-c"]));
  ...
  it("sorts alphabetically and persists the selected mode per company and user", async () => {
    ...
    expect(localStorage.getItem("jasminia.agentSortMode:company-1:user-1")).toBe("alphabetical");
  });
