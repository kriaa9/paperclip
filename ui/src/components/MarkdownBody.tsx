...
function MarkdownIssueLink({
  ...
  return (
    <Link
      ...
      className="jasminia-markdown-issue-ref"
      title={title}
...
function createWikiLinkNode(href: string, wikiLink: ParsedWikiLink): MarkdownAstNode {
  return {
    ...
    data: {
      hProperties: {
        "data-jasminia-wiki-link": "true",
        "data-jasminia-wiki-target": wikiLink.target,
      },
    },
...
function CodeBlock({
  ...
  return (
    <div className="jasminia-markdown-codeblock">
      <pre
        {...preProps}
        ref={preRef}
      ...
      <button
        ...
        title={label}
        className="jasminia-markdown-codeblock-copy"
      ...
      >
        ...
        <span className="jasminia-markdown-codeblock-copy-label">{label}</span>
      </button>
    </div>
  );
}
...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`jasminia-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="jasminia-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("jasminia-mermaid-status", error && "jasminia-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="jasminia-mermaid-source">
...
export function MarkdownBody({
  ...
  const components: Components = {
    ...
    table: ({ node: _node, style: tableStyle, children: tableChildren, ...tableProps }) => (
      <div className="jasminia-markdown-table-scroll" role="region" aria-label="Scrollable table" tabIndex={0}>
    ...
    a: ({ node: _node, href, style: linkStyle, children: linkChildren, ...anchorProps }) => {
      ...
      const isWikiLink = dataProps["data-jasminia-wiki-link"] === "true";
      ...
      if (parsed) {
        ...
        return (
          <a
            href={targetHref}
            className={cn(
              "jasminia-mention-chip",
              `jasminia-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "jasminia-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "jasminia-markdown prose prose-sm min-w-0 max-w-full break-words overflow-hidden",
