...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Jasmin.ia", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Jasmin.ia",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Jasmin.ia");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Jasmin.ia");
    expect(buildDocumentTitle([], null)).toBe("Jasmin.ia");
  });
});
