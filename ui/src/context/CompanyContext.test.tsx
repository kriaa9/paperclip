...
import type { Company } from "@jasminiaai/shared";
...
describe("CompanyProvider", () => {
  ...
  it("does not expose a stale stored company id before companies load", async () => {
    localStorage.setItem("jasminia.selectedCompanyId", "stale-company");
  ...
  it("replaces a stale stored company id with the first loaded company", async () => {
    ...
    expect(localStorage.getItem("jasminia.selectedCompanyId")).toBe("company-1");
  });
});
