...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal Jasmin.ia issue links", () => {
    expect(parseIssuePathIdFromPath("#
    expect(parseIssueReferenceFromHref("#
  });
  ...
  it("preserves absolute Jasmin.ia issue URLs so origin, port, and hash are not lost", () => {
