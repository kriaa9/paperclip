...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("jasminia-worktree-enabled") !== "true") return null;

  const name = readMetaContent("jasminia-worktree-name");
  const color = normalizeHexColor(readMetaContent("jasminia-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("jasminia-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
