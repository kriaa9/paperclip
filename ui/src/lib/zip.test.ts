...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("jasminia-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("jasminia-demo/COMPANY.md");
  ...
  it("reads a Jasmin.ia zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
        ".jasminia.yaml": "schema: jasminia/v1\n",
      },
      "jasminia-demo",
    );
  ...
  it("reads standard DEFLATE zip archives created outside Jasmin.ia", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("jasminia-demo");
