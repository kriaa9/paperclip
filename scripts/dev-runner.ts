...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[jasminia] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`jasminiaai worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const devServerStatusFilePath = path.join(repoRoot, ".jasminia", "dev-server-status.json");
const devServerStatusToken = mode === "dev" ? randomUUID() : null;
const devServerStatusTokenHeader = "x-jasminia-dev-server-status-token";
...
const ignoredRelativePaths = new Set([
  ".jasminia/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[jasminia] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[jasminia] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[jasminia] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  JASMINIA_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.JASMINIA_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.JASMINIA_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.JASMINIA_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.JASMINIA_DEV_SERVER_STATUS_TOKEN;
  env.JASMINIA_MIGRATION_PROMPT ??= "never";
  env.JASMINIA_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[jasminia] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.JASMINIA_BIND = effectiveBind;
  if (bindHost) {
    env.JASMINIA_BIND_HOST = bindHost;
  } else {
    delete env.JASMINIA_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.JASMINIA_DEPLOYMENT_MODE;
    delete env.JASMINIA_DEPLOYMENT_EXPOSURE;
    delete env.JASMINIA_AUTH_BASE_URL_MODE;
    console.log("[jasminia] dev mode: local_trusted (bind=loopback)");
  } else {
    env.JASMINIA_DEPLOYMENT_MODE = "authenticated";
    env.JASMINIA_DEPLOYMENT_EXPOSURE = "private";
    env.JASMINIA_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[jasminia] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.JASMINIA_BIND;
  delete env.JASMINIA_BIND_HOST;
  delete env.JASMINIA_DEPLOYMENT_MODE;
  delete env.JASMINIA_DEPLOYMENT_EXPOSURE;
  delete env.JASMINIA_AUTH_BASE_URL_MODE;
  console.log("[jasminia] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[jasminia] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@jasminiaai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[jasminia] Command failed with code ${status.code}: pnpm --filter @jasminiaai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[jasminia] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.JASMINIA_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[jasminia] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[jasminia] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@jasminiaai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[jasminia] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@jasminiaai/server", serverScript, ...forwardedArgs],
