...
# This keeps @jasminiaai/server publish artifacts self-contained for static UI serving.

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
UI_DIST="$REPO_ROOT/ui/dist"
SERVER_UI_DIST="$REPO_ROOT/server/ui-dist"

echo "  -> Building @jasminiaai/ui..."
pnpm --dir "$REPO_ROOT" --filter @jasminiaai/ui build
