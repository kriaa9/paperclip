#!/usr/bin/env bash
#
# Kill all local Jasmin.ia dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all jasminia dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/jasminia(-[^/]+)?/' | grep node | grep -v grep || true)
...
candidate_pidfiles=()
candidate_pidfiles+=(
  "$HOME"/.jasminia/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.jasminia/instances/*/db/postmaster.pid
  "$REPO_ROOT"/.jasminia/runtime-services/instances/*/db/postmaster.pid
)

for sibling_root in "$REPO_PARENT"/jasminia*; do
  [[ -d "$sibling_root" ]] || continue
  candidate_pidfiles+=(
    "$sibling_root"/.jasminia/instances/*/db/postmaster.pid
    "$sibling_root"/.jasminia/runtime-services/instances/*/db/postmaster.pid
  )
done
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 && ${#browser_pids[@]} -eq 0 ]]; then
  echo "No Jasmin.ia dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Jasmin.ia dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Jasmin.ia node processes..."
