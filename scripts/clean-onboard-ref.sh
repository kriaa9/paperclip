...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Jasmin.ia data dir to use
  JASMINIA_HOST=127.0.0.1    Host passed to the onboarded server
  JASMINIA_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/jasminia-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/jasminia-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export JASMINIA_HOST="${JASMINIA_HOST:-127.0.0.1}"
export JASMINIA_PORT="${JASMINIA_PORT:-3100}"
export JASMINIA_OPEN_ON_LISTEN="${JASMINIA_OPEN_ON_LISTEN:-false}"
...
echo "JASMINIA_HOST: $JASMINIA_HOST"
echo "JASMINIA_PORT: $JASMINIA_PORT"
