...
JASMINIA_HOST_PORT="${JASMINIA_HOST_PORT:-3100}"
JASMINIA_HOST_FROM_CONTAINER="${JASMINIA_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.jasminia-openclaw.override.yml"
...
detect_jasminia_base_url() {
  ...
  for candidate in "$JASMINIA_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${JASMINIA_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${JASMINIA_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
jasminia_base_url="$(detect_jasminia_base_url || true)"
...
Jasmin.ia URL for OpenClaw container:
EOF
  if [[ -n "$jasminia_base_url" ]]; then
    cat <<EOF
  $jasminia_base_url
