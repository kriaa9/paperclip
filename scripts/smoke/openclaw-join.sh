...
COMPANY_ID="${COMPANY_ID:-${JASMINIA_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-jasminia-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-jasminia-openclaw-smoke}"
...
Provide one of:
  JASMINIA_AUTH_HEADER=\"Bearer <board-token>\"
  JASMINIA_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Jasmin.ia OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.jasminia.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
