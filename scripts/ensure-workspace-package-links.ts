...
async function ensureWorkspaceLinksCurrent(workspaceDir: string) {
  ...
  console.log(`[jasminia] detected stale workspace package links for ${workspaceDir}; relinking dependencies...`);
  for (const mismatch of mismatches) {
    console.log(
      `[jasminia]   ${mismatch.packageName}: ${mismatch.actualPath ?? "missing"} -> ${mismatch.expectedPath}`,
    );
  }
