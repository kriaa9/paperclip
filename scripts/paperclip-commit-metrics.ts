...
const DEFAULT_QUERY = "\"Co-Authored-By: Jasmin.ia <noreply@jasminia.com>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/jasminia-commit-metrics-cache.json");
...
const JASMINIA_EMAIL = "noreply@jasminia.com";
const JASMINIA_NAME = "jasminia";
...
function printHelp() {
  console.log(`Usage: tsx scripts/jasminia-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === JASMINIA_EMAIL) || (displayName && displayName.toLowerCase() === JASMINIA_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Jasmin.ia commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "jasminia-commit-metrics",
