...
base_cwd="${JASMINIA_WORKSPACE_BASE_CWD:?JASMINIA_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${JASMINIA_WORKSPACE_CWD:?JASMINIA_WORKSPACE_CWD is required}"
jasminia_home="${JASMINIA_HOME:-$HOME/.jasminia}"
jasminia_instance_id="${JASMINIA_INSTANCE_ID:-default}"
jasminia_dir="$worktree_cwd/.jasminia"
worktree_config_path="$jasminia_dir/config.json"
worktree_env_path="$jasminia_dir/.env"
worktree_name="${JASMINIA_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${JASMINIA_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.jasminia/config.json" || -L "$base_cwd/.jasminia/config.json" ) ]]; then
  source_config_path="$base_cwd/.jasminia/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$jasminia_home/instances/$jasminia_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$jasminia_dir"
...
write_fallback_worktree_config() {
  ...
  JASMINIA_DIR="$jasminia_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  JASMINIA_WORKTREES_DIR="${JASMINIA_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const jasminiaDir = process.env.JASMINIA_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.JASMINIA_WORKTREES_DIR) ?? "~/.jasminia-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(jasminiaDir, "config.json");
  const envPath = path.resolve(jasminiaDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "jasminia",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.JASMINIA_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.JASMINIA_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.JASMINIA_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "JASMINIA_HOME=" + JSON.stringify(worktreeHome),
    "JASMINIA_INSTANCE_ID=" + JSON.stringify(instanceId),
    "JASMINIA_CONFIG=" + JSON.stringify(configPath),
    "JASMINIA_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "JASMINIA_IN_WORKTREE=true",
    "JASMINIA_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.JASMINIA_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("JASMINIA_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  ...
  if [[ "$needs_install" -eq 1 ]]; then
    backup_suffix=".jasminia-backup-${BASHPID:-$$}"
    moved_symlink_paths=()
