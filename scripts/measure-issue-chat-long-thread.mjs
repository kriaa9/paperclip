...
const baseUrl = (process.env.JASMINIA_PERF_BASE_URL || "http://localhost:3100").replace(/\/$/, "");
const companyPrefix = process.env.JASMINIA_PERF_COMPANY_PREFIX;
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".jasminia/auth.json");
  try {
...
const boardToken = process.env.JASMINIA_PERF_BEARER_TOKEN || loadBoardToken();
