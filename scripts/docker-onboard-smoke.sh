...
IMAGE_NAME="${IMAGE_NAME:-jasminia-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
JASMINIAAI_VERSION="${JASMINIAAI_VERSION:-latest}"
...
JASMINIA_DEPLOYMENT_MODE="${JASMINIA_DEPLOYMENT_MODE:-authenticated}"
JASMINIA_DEPLOYMENT_EXPOSURE="${JASMINIA_DEPLOYMENT_EXPOSURE:-private}"
JASMINIA_PUBLIC_URL="${JASMINIA_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@jasminia.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-jasminia-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_JASMINIAAI_VERSION=%q\n' "$JASMINIAAI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e JASMINIA_DEPLOYMENT_MODE="$JASMINIA_DEPLOYMENT_MODE" \
      -e JASMINIA_DEPLOYMENT_EXPOSURE="$JASMINIA_DEPLOYMENT_EXPOSURE" \
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$JASMINIA_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$JASMINIA_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$JASMINIA_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$JASMINIA_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$JASMINIA_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$JASMINIA_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg JASMINIAAI_VERSION="$JASMINIAAI_VERSION" \
...
echo "    Public URL: $JASMINIA_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $JASMINIA_DEPLOYMENT_MODE/$JASMINIA_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/jasminia-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$JASMINIA_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $JASMINIA_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$JASMINIA_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $JASMINIA_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
