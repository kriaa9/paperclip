...
describe("ssh env-lab fixture", () => {
  ...
  it("starts an isolated sshd fixture and executes commands through it", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-ssh-fixture-"));
    ...
    await expect(
      buildSshSpawnTarget({
        spec: {
          ...
          port: 22,
          username: "ssh-user",
          remoteCwd: "/srv/jasminia/workspace",
          remoteWorkspacePath: "/srv/jasminia/workspace",
          privateKey: null,
          knownHosts: null,
  ...
  it("syncs a local directory into the remote fixture workspace", async () => {
    ...
    await writeFile(path.join(localDir, "message.txt"), "hello from jasminia\n", "utf8");
    ...
    expect(result.stdout).toContain("hello from jasminia");
  ...
  it("round-trips a git workspace through the SSH fixture", async () => {
    ...
    await git(localRepo, ["config", "user.name", "Jasmin.ia Test"]);
    await git(localRepo, ["config", "user.email", "test@jasminia.dev"]);
    ...
    await runSshCommand(
      config,
      `cd ${JSON.stringify(started.workspaceDir)} && git config user.name "Jasmin.ia SSH" && git config user.email "ssh@jasminia.dev" && git add tracked.txt untracked.txt && git commit -m "remote update" >/dev/null && printf "remote dirty\\n" > tracked.txt && printf "remote extra\\n" > remote-only.txt`,
      { timeoutMs: 30_000, maxBuffer: 256 * 1024 },
    );
  ...
  it("round-trips remote git commits through the managed runtime restore path", async () => {
    ...
    await runSshCommand(
      config,
      `cd ${JSON.stringify(prepared.workspaceRemoteDir)} && git config user.name "Jasmin.ia SSH" && git config user.email "ssh@jasminia.dev" && printf "committed\\n" > tracked.txt && git add tracked.txt && git commit -m "remote update" >/dev/null && printf "dirty remote\\n" > tracked.txt`,
      { timeoutMs: 30_000, maxBuffer: 256 * 1024 },
    );
  ...
  it("merges concurrent remote commits through the managed runtime restore path", async () => {
    ...
    await runSshCommand(
      config,
      `cd ${JSON.stringify(preparedA.workspaceRemoteDir)} && git config user.name "Jasmin.ia SSH" && git config user.email "ssh@jasminia.dev" && printf "from run a\\n" > run-a.txt && git add run-a.txt && git commit -m "remote update a" >/dev/null`,
      { timeoutMs: 30_000, maxBuffer: 256 * 1024 },
    );
    await runSshCommand(
      config,
      `cd ${JSON.stringify(preparedB.workspaceRemoteDir)} && git config user.name "Jasmin.ia SSH" && git config user.email "ssh@jasminia.dev" && printf "from run b\\n" > run-b.txt && git add run-b.txt && git commit -m "remote update b" >/dev/null`,
      { timeoutMs: 30_000, maxBuffer: 256 * 1024 },
    );
    ...
    await expect(readFile(path.join(localRepo, "run-a.txt"), "utf8")).resolves.toBe("from run a\n");
    await expect(readFile(path.join(localRepo, "run-b.txt"), "utf8")).resolves.toBe("from run b\n");
    expect(await git(localRepo, ["log", "-1", "--pretty=%s"])).toContain("Jasmin.ia SSH sync merge");
