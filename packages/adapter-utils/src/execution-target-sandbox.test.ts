...
describe("sandbox adapter execution targets", () => {
  ...
  it("starts a localhost Jasmin.ia bridge for sandbox targets in bridge mode", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-execution-target-bridge-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".jasminia-runtime", "codex");
    ...
    const apiServer = createServer((req, res) => {
      requests.push({
        ...
        runId: typeof req.headers["x-jasminia-run-id"] === "string" ? req.headers["x-jasminia-run-id"] : null,
      });
    ...
    try {
      ...
      expect(bridge?.env.JASMINIA_API_URL).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(bridge?.env.JASMINIA_API_KEY).not.toBe("real-run-jwt");
      expect(bridge?.env.JASMINIA_API_BRIDGE_MODE).toBe("queue_v1");

      const response = await fetch(`${bridge!.env.JASMINIA_API_URL}/api/agents/me`, {
        headers: {
          authorization: `Bearer ${bridge!.env.JASMINIA_API_KEY}`,
  ...
  it("uses the effective adapter timeout when starting the sandbox callback bridge", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-execution-target-bridge-timeout-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".jasminia-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("fails oversized host responses with a 502 before returning them to the sandbox client", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-execution-target-bridge-limit-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".jasminia-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
