...
describe("sandbox managed runtime", () => {
  ...
  it("preserves excluded local workspace artifacts during restore mirroring", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-sandbox-restore-"));
    cleanupDirs.push(rootDir);
    const sourceDir = path.join(rootDir, "source");
    const targetDir = path.join(rootDir, "target");
    await mkdir(path.join(sourceDir, "src"), { recursive: true });
    await mkdir(path.join(targetDir, ".claude"), { recursive: true });
    await mkdir(path.join(targetDir, ".jasminia-runtime"), { recursive: true });
    ...
    await writeFile(path.join(targetDir, ".jasminia-runtime", "state.json"), "{}\n", "utf8");

    await mirrorDirectory(sourceDir, targetDir, {
      preserveAbsent: [".jasminia-runtime", ".claude", ".claude.json"],
    });
    ...
    await expect(readFile(path.join(targetDir, ".jasminia-runtime", "state.json"), "utf8")).resolves.toBe("{}\n");
  ...
  it("syncs workspace and assets through a provider-neutral sandbox client", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-sandbox-managed-"));
    cleanupDirs.push(rootDir);
    ...
    await writeFile(path.join(localWorkspaceDir, ".jasminia-runtime", "state.json"), "{}\n", "utf8");
    ...
    await expect(readFile(path.join(localWorkspaceDir, ".jasminia-runtime", "state.json"), "utf8")).resolves.toBe("{}\n");
  });
});
