...
describe("sandbox callback bridge", () => {
    ...
    return {
      execute: async (input: {
        ...
        if (
          ...
          env.JASMINIA_TEST_STDIN = input.stdin;
          args[1] = `printf '%s' \"$JASMINIA_TEST_STDIN\" | (${args[1]})`;
        }
        try {
  ...
  it("round-trips localhost bridge requests over the sandbox queue without forwarding the bridge token", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-runtime-"));
    ...
    const queueDir = path.posix.join(prepared.runtimeRootDir, "jasminia-bridge");
    ...
    const okResponse = await fetch(`${bridge.baseUrl}/api/agents/me?view=compact`, {
      headers: {
        ...
        "x-jasminia-run-id": "run-bridge-1",
    ...
    expect(seenRequests[0]?.headers["x-jasminia-run-id"]).toBeUndefined();

  });

  it("denies non-allowlisted requests by default", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-default-policy-"));
    cleanupDirs.push(rootDir);
  ...
  it("drains already-queued requests on stop", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-drain-"));
    cleanupDirs.push(rootDir);
  ...
  it("writes fast 503 responses for queued requests that miss the drain deadline", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-drain-timeout-"));
    cleanupDirs.push(rootDir);
  ...
  it("handles SSH queue polling failures without emitting an unhandled rejection", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-ssh-failure-"));
    cleanupDirs.push(rootDir);
    ...
    try {
      const worker = await startSandboxCallbackBridgeWorker({
        client: {
          ...
          listJsonFiles: async () => {
            throw new Error(
              "list /remote/.jasminia-runtime/gemini/jasminia-bridge/queue/requests failed with exit code 255: kex_exchange_identification: read: Connection reset by peer",
            );
          },
          readTextFile: async () => {
  ...
  it("serializes remote response writes so stop does not recreate a late orphaned response", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-response-lock-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(
      readdir(directories.responsesDir).then((entries) =>
        entries.filter((entry) => entry.endsWith(".tmp") || entry.includes(".jasminia-write.lock")),
      ),
    ).resolves.toEqual([]);
  });

  it("rejects non-JSON request bodies and full queues at the bridge server", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-server-guards-"));
    cleanupDirs.push(rootDir);
  ...
  it("returns a 502 when the host response times out", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-timeout-"));
    cleanupDirs.push(rootDir);
  ...
  it("returns a 502 for malformed host response files", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-malformed-response-"));
    cleanupDirs.push(rootDir);
  ...
  it("reuses an already-uploaded bridge entrypoint when the remote file hash matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-sync-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(readFile(path.posix.join(remoteAssetDir, "jasminia-bridge-server.mjs"), "utf8")).resolves.toBe(expandedSource);
    await expect(
      readdir(remoteAssetDir).then((entries) =>
        entries.filter(
          (entry) =>
            entry.endsWith(".jasminia-upload.b64") ||
            entry.endsWith(".partial") ||
            entry === ".jasminia-bridge-upload.lock",
        ),
      ),
    ).resolves.toEqual([]);
  });
  ...
  it("rejects a corrupted bridge entrypoint upload without committing a torn remote file", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-sync-corrupt-"));
    cleanupDirs.push(rootDir);
    ...
    await expect(readFile(path.posix.join(remoteAssetDir, "jasminia-bridge-server.mjs"), "utf8")).rejects.toThrow();
    await expect(
      readdir(remoteAssetDir).then((entries) =>
        entries.filter(
          (entry) =>
  ...
  it("marks command-managed bridge operations with the bridge execution channel", async () => {
    ...
    await client.makeDir("/workspace/.jasminia-runtime/codex/jasminia-bridge/queue");

    expect(runner.execute).toHaveBeenCalledWith(expect.objectContaining({
      env: {
        JASMINIA_SANDBOX_EXEC_CHANNEL: "bridge",
      },
    }));
  });
});
