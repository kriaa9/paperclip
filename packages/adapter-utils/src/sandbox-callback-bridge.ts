...
const SANDBOX_CALLBACK_BRIDGE_ENTRYPOINT = "jasminia-bridge-server.mjs";
const SANDBOX_EXEC_CHANNEL_ENV = "JASMINIA_SANDBOX_EXEC_CHANNEL";
...
// reverse bridge. Keep this in sync with the Jasmin.ia skill in
// `skills/jasminia/SKILL.md` and `references/api-reference.md`.
...
export function buildSandboxCallbackBridgeEnv(input: {
  ...
  return {
    ...
    JASMINIA_BRIDGE_QUEUE_DIR: input.queueDir,
    JASMINIA_BRIDGE_TOKEN: input.bridgeToken,
    JASMINIA_BRIDGE_HOST: input.host?.trim() || "127.0.0.1",
    JASMINIA_BRIDGE_PORT: String(input.port && input.port > 0 ? Math.trunc(input.port) : 0),
    JASMINIA_BRIDGE_POLL_INTERVAL_MS: String(
      normalizeTimeoutMs(input.pollIntervalMs, DEFAULT_BRIDGE_POLL_INTERVAL_MS),
    ),
    JASMINIA_BRIDGE_RESPONSE_TIMEOUT_MS: String(
      normalizeTimeoutMs(input.responseTimeoutMs, DEFAULT_BRIDGE_RESPONSE_TIMEOUT_MS),
    ),
    JASMINIA_BRIDGE_MAX_QUEUE_DEPTH: String(
      normalizeTimeoutMs(input.maxQueueDepth, DEFAULT_BRIDGE_MAX_QUEUE_DEPTH),
    ),
    JASMINIA_BRIDGE_MAX_BODY_BYTES: String(
...
export async function createSandboxCallbackBridgeAsset(): Promise<SandboxCallbackBridgeAsset> {
  const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-bridge-asset-"));
...
export function createFileSystemSandboxCallbackBridgeQueueClient(): SandboxCallbackBridgeQueueClient {
  return {
    ...
    writeResponseFile: async (responsePath, body, options = {}) => {
      ...
      const lockDir = `${responsePath}.jasminia-write.lock`;
...
export function createCommandManagedSandboxCallbackBridgeQueueClient(input: {
  ...
  return {
    ...
    writeTextFile: async (remotePath, body) => {
      ...
      const tempPath = `${remotePath}.jasminia-upload.b64`;
      await runChecked(
...
export async function startSandboxCallbackBridgeWorker(input: {
  ...
  const processRequestFile = async (fileName: string) => {
    ...
    try {
      ...
      console.warn(
        `[jasminia] sandbox callback bridge handler failed for ${request.id}: ${error instanceof Error ? error.message : String(error)}`,
      );
      await writeBridgeResponse(input.client, requestPath, responsePath, {
        id: request.id,
        status: 502,
  ...
  const failPendingRequests = async (message: string) => {
    ...
    for (const fileName of fileNames) {
      ...
      try {
        ...
        console.warn(
          `[jasminia] sandbox callback bridge failed to abort pending request ${requestId}: ${error instanceof Error ? error.message : String(error)}`,
        );
      } finally {
        await input.client.remove(requestPath).catch(() => undefined);
      }
  ...
  const loop = (async () => {
    try {
      ...
      console.warn(`[jasminia] ${message}`);
      try {
        await failPendingRequests(message);
      } catch (failPendingError) {
        console.warn(
          `[jasminia] sandbox callback bridge failed to abort queued requests after worker failure: ${failPendingError instanceof Error ? failPendingError.message : String(failPendingError)}`,
        );
      }
    } finally {
      settled = true;
...
export async function syncSandboxCallbackBridgeEntrypoint(input: {
  ...
  const remoteUploadPath = `${remoteEntrypoint}.jasminia-upload.b64`;
  const remoteLockDir = path.posix.join(input.assetRemoteDir, ".jasminia-bridge-upload.lock");
...
const queueDir = process.env.JASMINIA_BRIDGE_QUEUE_DIR;
const bridgeToken = process.env.JASMINIA_BRIDGE_TOKEN;
const host = process.env.JASMINIA_BRIDGE_HOST || "127.0.0.1";
const port = Number(process.env.JASMINIA_BRIDGE_PORT || "0");
const pollIntervalMs = Number(process.env.JASMINIA_BRIDGE_POLL_INTERVAL_MS || "100");
const responseTimeoutMs = Number(process.env.JASMINIA_BRIDGE_RESPONSE_TIMEOUT_MS || "30000");
const maxQueueDepth = Number(process.env.JASMINIA_BRIDGE_MAX_QUEUE_DEPTH || "${DEFAULT_BRIDGE_MAX_QUEUE_DEPTH}");
const maxBodyBytes = Number(process.env.JASMINIA_BRIDGE_MAX_BODY_BYTES || "${DEFAULT_BRIDGE_MAX_BODY_BYTES}");
const allowedHeaders = new Set(${JSON.stringify([...DEFAULT_SANDBOX_CALLBACK_BRIDGE_HEADER_ALLOWLIST])});

if (!queueDir || !bridgeToken) {
  throw new Error("JASMINIA_BRIDGE_QUEUE_DIR and JASMINIA_BRIDGE_TOKEN are required.");
}
