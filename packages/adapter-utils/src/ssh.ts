...
async function createSshAuthArgs(
  ...
  if (config.strictHostKeyChecking) {
    if (config.knownHosts) {
      const knownHosts = await withTempFile("jasminia-ssh-known-hosts-", config.knownHosts, 0o600);
  ...
  if (config.privateKey) {
    const privateKey = await withTempFile("jasminia-ssh-key-", config.privateKey, 0o600);
...
async function importGitWorkspaceToSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-ssh-bundle-"));
  ...
  const tempRef = `refs/jasminia/ssh-sync/import/${randomUUID()}`;
  ...
  try {
    ...
    const remoteSetupScript = [
      "set -e",
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".jasminia-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".jasminia-runtime", "import-XXXXXX.bundle"))})`,
      ...
      `git -C ${shellQuote(input.remoteDir)} clean -fdx -e .jasminia-runtime >/dev/null`,
...
async function exportGitWorkspaceFromSsh(input: {
  ...
  const bundleDir = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-ssh-bundle-"));
  const bundlePath = path.join(bundleDir, "workspace.bundle");
  const importedRef = input.importedRef ?? `refs/jasminia/ssh-sync/imported/${randomUUID()}`;

  try {
    const exportScript = [
      "set -e",
      `git -C ${shellQuote(input.remoteDir)} update-ref refs/jasminia/ssh-sync/export HEAD`,
      `mkdir -p ${shellQuote(path.posix.join(input.remoteDir, ".jasminia-runtime"))}`,
      `tmp_bundle=$(mktemp ${shellQuote(path.posix.join(input.remoteDir, ".jasminia-runtime", "export-XXXXXX.bundle"))})`,
      'cleanup() { rm -f "$tmp_bundle"; git -C ' + shellQuote(input.remoteDir) + ' update-ref -d refs/jasminia/ssh-sync/export >/dev/null 2>&1 || true; }',
      'trap cleanup EXIT',
      `git -C ${shellQuote(input.remoteDir)} bundle create "$tmp_bundle" refs/jasminia/ssh-sync/export >/dev/null`,
      'cat "$tmp_bundle"',
    ].join("\n");
    ...
    await runLocalGit(input.localDir, ["fetch", "--force", bundlePath, `refs/jasminia/ssh-sync/export:${importedRef}`], {
      timeout: 60_000,
      maxBuffer: 1024 * 1024,
    });
...
async function integrateImportedGitHead(input: {
  ...
  const mergeCommit = await runLocalGit(
    ...
    [
      ...
      input.importedHead,
      "-m",
      `Jasmin.ia SSH sync merge ${input.importedHead.slice(0, 12)}`,
    ],
    {
      timeout: 60_000,
      maxBuffer: 64 * 1024,
...
export async function getSshEnvLabSupport(): Promise<SshEnvLabSupport> {
  if (process.platform === "darwin" && process.env.JASMINIA_ENABLE_DARWIN_SSH_ENV_LAB !== "1") {
    return {
      supported: false,
      reason: "SSH env-lab fixture is disabled on macOS; set JASMINIA_ENABLE_DARWIN_SSH_ENV_LAB=1 to opt in.",
    };
  }
...
export async function syncDirectoryFromSsh(input: {
  ...
  const auth = await createSshAuthArgs(input.spec);
  const stagingDir = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-ssh-sync-back-"));
...
export async function prepareWorkspaceForSshExecution(input: {
  ...
  await clearRemoteDirectory({
    spec: input.spec,
    remoteDir,
    preserveEntries: [".jasminia-runtime"],
  });
  await syncDirectoryToSsh({
    spec: input.spec,
    localDir: input.localDir,
    remoteDir,
    exclude: [".jasminia-runtime"],
  });
...
export async function restoreWorkspaceFromSshExecution(input: {
  ...
  if (input.baselineSnapshot) {
    ...
    const importedRef = input.restoreGitHistory
      ? `refs/jasminia/ssh-sync/imported/${randomUUID()}`
      : null;
    try {
