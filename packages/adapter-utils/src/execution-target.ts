...
function resolveDefaultJasminiaApiUrl(): string {
  ...
  // 3100 matches the default Jasmin.ia dev server port when the runtime does not provide one.
...
function isBridgeDebugEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.JASMINIA_BRIDGE_DEBUG?.trim().toLowerCase();
  ...
  if (detectCommand) {
    ...
    if (!recheck.timedOut && recheck.exitCode === 0) {
      if (input.onLog) {
        ...
        await input.onLog(
          "stderr",
          `[jasminia] Install command ${reason} (${installCommand}) but ${detectCommand} is on PATH; continuing.\n`,
        );
      }
      return;
    }
 ...
 * the directory state inside the environment, not on the Jasmin.ia host.
 *
...
export function runtimeAssetDir(
  ...
  return prepared.assetDirs[key] ?? path.posix.join(fallbackRemoteCwd, ".jasminia-runtime", key);
}
...
export async function startAdapterExecutionTargetJasminiaBridge(input: {
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side Jasmin.ia API token.");
  }

  const runtimeRootDir =
    input.runtimeRootDir?.trim().length
      ? input.runtimeRootDir.trim()
      : path.posix.join(target.remoteCwd, ".jasminia-runtime", input.adapterKey);
  const bridgeRuntimeDir = path.posix.join(runtimeRootDir, "jasminia-bridge");
  ...
  await onLog(
    "stdout",
    `[jasminia] Starting sandbox callback bridge for ${input.adapterKey} in ${bridgeRuntimeDir}.\n`,
  );
  ...
  try {
    ...
    // JASMINIA_BRIDGE_DEBUG opts into verbose stdout logs of every bridge
    ...
    worker = await startSandboxCallbackBridgeWorker({
      ...
      handleRequest: async (request) => {
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[jasminia] Bridge proxy ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        ...
        headers.set("x-jasminia-run-id", input.runId);
        ...
        if (bridgeDebugEnabled) {
          await onLog(
            "stdout",
            `[jasminia] Bridge proxy response ${response.status} for ${method} ${request.path}${request.query ? `?${request.query}` : ""}\n`,
          );
        }
        return {
  ...
  return {
    env: {
      JASMINIA_API_URL: server.baseUrl,
      JASMINIA_API_KEY: bridgeToken,
