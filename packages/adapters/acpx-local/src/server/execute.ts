...
const JASMINIA_MANAGED_CODEX_SKILLS_MANIFEST = ".jasminia-managed-skills.json";
...
async function prepareManagedCodexHome(input: {
  ...
  await onLog(
    "stdout",
    `[jasminia] Using Jasmin.ia-managed ACPX Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
...
async function buildSkillSetKey(input: {
  ...
  hash.update(`jasminia-acpx-${input.label}-skills:v1\n`);
...
async function prepareClaudeSkillRuntime(input: {
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[jasminia] Materialized ACPX Claude skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[jasminia] Failed to materialize ACPX Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
  ...
  const promptInstructions = selectedSkills.length > 0
    ? [
        "Jasmin.ia has materialized selected runtime skills for this ACPX Claude session.",
  ...
  return {
    ...
    commandNotes: selectedSkills.length > 0
      ? [`Materialized ${selectedSkills.length} Jasmin.ia skill(s) for ACPX Claude at ${skillsHome}.`]
      : [],
  };
}

async function readManagedCodexSkillsManifest(skillsHome: string): Promise<Set<string>> {
  const manifestPath = path.join(skillsHome, JASMINIA_MANAGED_CODEX_SKILLS_MANIFEST);
  try {
...
async function writeManagedCodexSkillsManifest(skillsHome: string, skillNames: Iterable<string>): Promise<void> {
  ...
  await fs.writeFile(
    path.join(skillsHome, JASMINIA_MANAGED_CODEX_SKILLS_MANIFEST),
...
async function reconcileManagedCodexSkills(input: {
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[jasminia] Revoked ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const entry of input.allSkills) {
    ...
    if (await removeSkillTarget(target)) {
      await input.onLog("stdout", `[jasminia] Revoked legacy ACPX Codex skill "${entry.runtimeName}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[jasminia] Revoked unavailable ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
}
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[jasminia] Materialized ACPX Codex skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[jasminia] Failed to inject ACPX Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
async function buildRuntime(input: {
  ...
  const workspaceContext = parseObject(context.jasminiaWorkspace);
  ...
  if (!hasExplicitApiKey && authToken) env.JASMINIA_API_KEY = authToken;
  ...
  const sessionKey = `jasminia:${agent.companyId}:${agent.id}:${taskKey}:${fingerprint}`;
...
async function applySessionConfigOptions(input: {
  ...
  if (!input.runtime.setConfigOption) {
    ...
    await input.onLog("stderr", `[jasminia] ${message}\n`);
  ...
  for (const option of options) {
    ...
    await input.onLog(
      "stdout",
      `[jasminia] Applied ACPX ${input.prepared.acpxAgent} config ${option.key}=${option.value}\n`,
    );
  }
}
...
async function buildPrompt(ctx: AdapterExecutionContext, resumedSession: boolean): Promise<{
  ...
  const taskContextNote = asString(context.jasminiaTaskMarkdown, "").trim();
...
export function createAcpxLocalExecutor(deps: ExecuteDeps = {}) {
  ...
  return async function executeAcpxLocal(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    if (!canResume && asString(previousParams.runtimeSessionName, "")) {
      await ctx.onLog(
        "stdout",
        `[jasminia] ACPX session "${asString(previousParams.runtimeSessionName, "")}" does not match the current agent/cwd/mode/runtime identity; starting fresh in "${prepared.cwd}".\n`,
      );
    }
    ...
    try {
      if (!handle) {
        try {
          ...
          await ctx.onLog(
            "stdout",
            `[jasminia] ACPX resume session "${resumeSessionId}" is unavailable; retrying with a fresh session.\n`,
          );
          handle = await runtime.ensureSession({
            sessionKey: prepared.sessionKey,
            agent: prepared.acpxAgent,
    ...
    try {
      ...
      await runtime.close({
        handle: sessionHandle,
        reason: "jasminia config cleanup",
    ...
    if (ctx.onMeta) {
      await ctx.onMeta({
        ...
        cwd: prepared.cwd,
        commandNotes: [
          `ACPX runtime embedded in Jasmin.ia with ${prepared.mode} session mode.`,
    ...
    try {
      ...
      if (terminal.status === "failed" || terminal.status === "cancelled" || timedOut) {
        ...
        if (warmHandleMatches(existing, runtime, sessionHandle) && existing) {
          await closeWarmHandle({
            ...
            entry: existing,
            reason: timedOut ? "jasminia timeout cleanup" : `jasminia turn ${terminal.status}`,
        ...
        if (existing && !warmHandleMatches(existing, runtime, sessionHandle)) {
          await runtime.close({
            handle: sessionHandle,
            reason: "jasminia duplicate warm handle cleanup",
            discardPersistentState: false,
          }).catch(() => {});
        } else {
        ...
        if (warmHandleMatches(existing, runtime, sessionHandle) && existing) {
          await closeWarmHandle({
            ...
            key: prepared.sessionKey,
            entry: existing,
            reason: "jasminia completed turn cleanup",
          });
        } else {
          await runtime.close({
            handle: sessionHandle,
      ...
      await runtime.close({
        handle: sessionHandle,
        reason: timedOut ? "jasminia timeout cleanup" : "jasminia error cleanup",
