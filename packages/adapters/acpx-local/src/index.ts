...
Use when:
- The agent should run through Agent Client Protocol via ACPX on the Jasmin.ia host or a managed execution environment.
- You want one built-in adapter that can target Claude, Codex, or a custom ACP server command.
- You need Jasmin.ia-managed session identity and live streamed ACP events in later ACPX runtime phases.
...
- mode (string, optional): persistent or oneshot. Defaults to persistent. Jasmin.ia keeps session state persistent and may close the live process between runs.
...
- stateDir (string, optional): ACPX state directory. Defaults to a Jasmin.ia-managed company/agent scoped location.
- instructionsFilePath (string, optional): absolute path to a markdown instructions file used by Jasmin.ia prompt construction.
...
- warmHandleIdleMs (number, optional): live ACPX process idle window after a successful persistent run. Defaults to 0, meaning Jasmin.ia shuts the process down after each run while retaining ACPX session state.
...
- ACPX currently requires Node >=22.12.0. Jasmin.ia keeps the repo-wide Node >=20 engine and surfaces the stricter runtime prerequisite through acpx_local diagnostics.
`;
