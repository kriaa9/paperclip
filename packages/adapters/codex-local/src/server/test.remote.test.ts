...
const {
  ...
  return {
    ...
    prepareAdapterExecutionTargetRuntime: vi.fn(async () => ({
      target: null,
      workspaceRemoteDir: "/remote/workspace/.jasminia-runtime/runs/test/workspace",
      runtimeRootDir: "/remote/workspace/.jasminia-runtime/runs/test/workspace/.jasminia-runtime/codex",
      assetDirs: {
        home: "/remote/workspace/.jasminia-runtime/runs/test/workspace/.jasminia-runtime/codex/home",
      },
      restoreWorkspace,
    })),
    prepareManagedCodexHome: vi.fn(async () => "/tmp/jasminia-managed-codex-home"),
    restoreWorkspace,
  };
});
...
describe("codex remote environment diagnostics", () => {
  ...
  it("stages managed CODEX_HOME in an isolated runtime dir and keeps the probe cwd on the original remote workspace", async () => {
    ...
    expect(runtimeInput?.workspaceLocalDir).toContain(`${os.tmpdir()}/jasminia-codex-envtest-`);
    ...
    expect(probeCall?.[4]).toMatchObject({
      ...
      env: expect.objectContaining({
        CODEX_HOME: "/remote/workspace/.jasminia-runtime/runs/test/workspace/.jasminia-runtime/codex/home",
      }),
    });
  ...
  it("avoids /tmp CODEX_HOME for remote API-key hello probes", async () => {
    ...
    const probeCall = runAdapterExecutionTargetProcess.mock.calls[0] as unknown as
      ...
      | undefined;
    expect(probeCall?.[4].env.CODEX_HOME).toContain("/remote/workspace/.jasminia-runtime/codex/probe-home-codex-envtest-");
