...
async function prepareCodexHelloProbe(input: {
  ...
  if (input.targetIsRemote && !input.probeApiKey) {
    ...
    preparedRuntimeWorkspaceLocalDir = await fs.mkdtemp(
      path.join(os.tmpdir(), `jasminia-codex-envtest-${input.runId}-`),
    );
    preparedRuntime = await prepareAdapterExecutionTargetRuntime({
      ...
      // `.jasminia-runtime/runs/<runId>/workspace` to whatever it gets, so
  ...
  if (input.probeApiKey) {
    const probeHome = input.targetIsRemote
      ? path.posix.join(input.cwd, ".jasminia-runtime", "codex", `probe-home-${input.runId}`)
      : path.join(os.tmpdir(), `jasminia-codex-probe-${input.runId}`);
    return {
      command: "sh",
      args: [
        "-c",
        'set -e; mkdir -p "$CODEX_HOME"; umask 077; printf "%s" "$_JASMINIA_CODEX_AUTH_JSON" > "$CODEX_HOME/auth.json"; unset _JASMINIA_CODEX_AUTH_JSON; trap \'rm -rf "$CODEX_HOME"\' EXIT INT TERM; "$0" "$@"',
        input.command,
        ...input.args,
      ],
      env: {
        ...
        _JASMINIA_CODEX_AUTH_JSON: JSON.stringify({ OPENAI_API_KEY: input.probeApiKey }),
      },
      cleanup,
    };
  }
        ...
        if (probe.timedOut) {
          ...
          checks.push({
            ...
            hint: probeApiKey
              ...
              : "Codex CLI does not read OPENAI_API_KEY from the environment; set OPENAI_API_KEY in this adapter's config (so Jasmin.ia writes it to `$CODEX_HOME/auth.json`) or run `codex login` on the host first.",
          });
        } else {
          checks.push({
