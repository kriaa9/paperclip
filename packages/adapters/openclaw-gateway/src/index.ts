...
Use when:
- You want Jasmin.ia to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the Jasmin.ia server.
...
- jasminiaApiUrl (string, optional): absolute Jasmin.ia base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/jasminia-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default jasminia)

Standard outbound payload additions:
- jasminia (object): standardized Jasmin.ia context added to every gateway agent request
- jasminia.workspace (object, optional): resolved execution workspace for this run
- jasminia.workspaces (array, optional): additional workspace hints Jasmin.ia exposed to the run
- jasminia.workspaceRuntime (object, optional): reserved workspace runtime metadata when explicitly supplied outside normal heartbeat execution
