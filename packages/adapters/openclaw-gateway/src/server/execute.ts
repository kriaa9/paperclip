...
const DEFAULT_CLIENT_VERSION = "jasminia";
...
export function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "jasminia";
  if (input.strategy === "run") {
    return prefixSessionKeyForAgent(`jasminia:run:${input.runId}`, input.agentId);
  }
  if (input.strategy === "issue" && input.issueId) {
    return prefixSessionKeyForAgent(`jasminia:issue:${input.issueId}`, input.agentId);
  }
...
const DEFAULT_CLAIMED_API_KEY_PATH = "~/.openclaw/workspace/jasminia-claimed-api-key.json";
...
function buildJasminiaEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  ...
  if (wakePayload.approvalId) jasminiaEnv.JASMINIA_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) jasminiaEnv.JASMINIA_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    jasminiaEnv.JASMINIA_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }
...
function buildWakeText(
  ...
  const claimedApiKeyPath = "~/.openclaw/workspace/jasminia-claimed-api-key.json";
  const orderedKeys = [
    "JASMINIA_RUN_ID",
    "JASMINIA_AGENT_ID",
    "JASMINIA_COMPANY_ID",
    "JASMINIA_API_URL",
    "JASMINIA_TASK_ID",
    "JASMINIA_WAKE_REASON",
    "JASMINIA_WAKE_COMMENT_ID",
    "JASMINIA_APPROVAL_ID",
    "JASMINIA_APPROVAL_STATUS",
    "JASMINIA_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = jasminiaEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = jasminiaEnv.JASMINIA_API_URL ?? "<set JASMINIA_API_URL>";

  const lines = [
    "Jasmin.ia wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `JASMINIA_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load JASMINIA_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $JASMINIA_API_KEY on every API call.",
    "- Use X-Jasmin.ia-Run-Id: $JASMINIA_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: JASMINIA_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$JASMINIA_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\",\"in_review\"]}",
    ...
    "   - GET /api/companies/$JASMINIA_COMPANY_ID/issues?assigneeAgentId=$JASMINIA_AGENT_ID&status=todo,in_progress,in_review,blocked",
...
function buildStandardJasminiaPayload(
  ...
  const runtimeServiceIntents = Array.isArray(ctx.context.jasminiaRuntimeServiceIntents)
    ? ctx.context.jasminiaRuntimeServiceIntents.filter(
  ...
  const standardJasminia: Record<string, unknown> = {
    ...
    apiUrl: jasminiaEnv.JASMINIA_API_URL ?? null,
  };
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "jasminia-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  agentParams.jasminia = jasminiaPayload;
