...
Use when:
- You want Jasmin.ia to run Pi (the AI coding agent) locally as the agent runtime
...
- Jasmin.ia requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/jasminias/ and resumed with --session.
