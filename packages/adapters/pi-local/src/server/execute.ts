...
const JASMINIA_SESSIONS_DIR = path.join(os.homedir(), ".pi", "jasminias");
...
async function ensurePiSkillsInjected(
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[jasminia] Removed maintainer-only Pi skill "${skillName}" from ${PI_AGENT_SKILLS_DIR}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[jasminia] ${result === "repaired" ? "Repaired" : "Injected"} Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[jasminia] Failed to inject Pi skill "${entry.runtimeName}" into ${PI_AGENT_SKILLS_DIR}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildPiSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-pi-skills-"));
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(JASMINIA_SESSIONS_DIR, { recursive: true });
  return JASMINIA_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(JASMINIA_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const piSkillEntries = await readJasmin.iaRuntimeSkillEntries(config, __moduleDir);
  const desiredPiSkillNames = resolveJasmin.iaDesiredSkillNames(config, piSkillEntries);
  if (!executionTargetIsRemote) {
  ...
  // invoke skill binaries (e.g. `jasminia-get-issue`) by name. Without this,
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[jasminia] Syncing workspace and Pi runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (runtimeSessionId && !canResumeSession) {
    const staleSessionCwdNote =
      savedSessionCwd !== null && !sessionHeaderCwdMatches
        ? ` Pi stored cwd "${savedSessionCwd}" in the session header, so Jasmin.ia will start a fresh session for "${effectiveExecutionCwd}".`
        : "";
    await onLog(
      "stdout",
      executionTargetIsRemote
        ? `[jasminia] Pi session "${runtimeSessionId}" does not match the current remote execution state and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote} Starting a fresh remote session.\n`
        : `[jasminia] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".${staleSessionCwdNote}\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[jasminia] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const renderedBootstrapPrompt =
    !canResumeSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderJasmin.iaWakePrompt(context.jasminiaWake, { resumedSession: canResumeSession });
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[jasminia] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
      );
