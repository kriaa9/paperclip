...
describe("pi remote execution", () => {
  ...
  it("prepares the workspace, syncs Pi skills, and restores workspace changes for remote SSH execution", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-pi-remote-"));
    ...
    expect(String(result.sessionId)).toContain(`${managedRemoteWorkspace}/.jasminia-runtime/pi/sessions/`);
    expect(prepareWorkspaceForSshExecution).toHaveBeenCalledTimes(1);
    expect(syncDirectoryToSsh).toHaveBeenCalledTimes(1);
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.jasminia-runtime/pi/skills`,
      followSymlinks: true,
    }));
    expect(runSshCommand).toHaveBeenCalledWith(
      expect.anything(),
      expect.stringContaining(".jasminia-runtime/pi/sessions"),
      expect.anything(),
    );
    ...
    expect(call?.[2]).toContain(`${managedRemoteWorkspace}/.jasminia-runtime/pi/skills`);
  ...
  it("resumes saved Pi sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-pi-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    ...
    await execute({
      ...
      runtime: {
        sessionId: `${managedRemoteWorkspace}/.jasminia-runtime/pi/sessions/session-123.jsonl`,
        sessionParams: {
          sessionId: `${managedRemoteWorkspace}/.jasminia-runtime/pi/sessions/session-123.jsonl`,
          cwd: managedRemoteWorkspace,
          remoteExecution: {
    ...
    expect(call?.[2]).toContain("--session");
    expect(call?.[2]).toContain(`${managedRemoteWorkspace}/.jasminia-runtime/pi/sessions/session-123.jsonl`);
  });

  it("starts a fresh remote Pi session when the saved session header cwd points at a different workspace", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-pi-remote-stale-session-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    await execute({
      ...
      runtime: {
        sessionId: "/remote/workspace/.jasminia-runtime/pi/sessions/session-123.jsonl",
        sessionParams: {
          sessionId: "/remote/workspace/.jasminia-runtime/pi/sessions/session-123.jsonl",
    ...
    const managedRemoteWorkspaceFresh = "/remote/workspace/.jasminia-runtime/runs/run-ssh-stale-session/workspace";
    const call = runChildProcess.mock.calls[0] as unknown as [string, string, string[]] | undefined;
    const sessionIndex = call?.[2].indexOf("--session") ?? -1;
    expect(sessionIndex).toBeGreaterThanOrEqual(0);
    const usedSession = sessionIndex >= 0 ? call?.[2][sessionIndex + 1] : null;
    expect(usedSession).toContain(`${managedRemoteWorkspaceFresh}/.jasminia-runtime/pi/sessions/`);
    expect(usedSession).not.toBe("/remote/workspace/.jasminia-runtime/pi/sessions/session-123.jsonl");
  });

  it("starts a fresh remote Pi session when the saved session header is empty or unreadable", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-pi-remote-empty-header-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    await execute({
      ...
      context: {
        jasminiaWorkspace: { cwd: workspaceDir, source: "project_primary" },
      },
      executionTransport: {
        remoteExecution: {
          host: "127.0.0.1",
  ...
  it("starts a fresh remote Pi session when the remote head command fails", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "jasminia-pi-remote-head-failure-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
