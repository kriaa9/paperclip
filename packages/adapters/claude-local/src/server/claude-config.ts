...
export async function prepareClaudeConfigSeed(
  ...
  if (copiedFiles.length > 0) {
    await onLog(
      "stdout",
      `[jasminia] Prepared Claude config seed "${targetDir}" from "${sourceDir}" (${copiedFiles.map((file) => file.name).join(", ")}).\n`,
    );
  } else {
    await onLog(
      "stdout",
      `[jasminia] No local Claude config seed files were found in "${sourceDir}". Remote Claude auth may still require login.\n`,
    );
  }
