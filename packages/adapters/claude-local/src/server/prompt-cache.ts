...
async function buildClaudePromptBundleKey(input: {
  ...
  hash.update("jasminia-claude-prompt-bundle:v1\n");
...
export async function prepareClaudePromptBundle(input: {
  ...
  for (const entry of skills) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[jasminia] Failed to materialize Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
