...
async function buildClaudeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  const entries: AdapterSkillEntry[] = availableEntries.map((entry) => ({
    ...
    origin: entry.required ? "jasminia_required" : "company_managed",
    originLabel: entry.required ? "Required by Jasmin.ia" : "Managed by Jasmin.ia",
    readOnly: false,
    sourcePath: entry.source,
    targetPath: null,
    detail: desiredSet.has(entry.key)
      ? "Will be materialized into the stable Jasmin.ia-managed Claude prompt bundle on the next run."
      : null,
