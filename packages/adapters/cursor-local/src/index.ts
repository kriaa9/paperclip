...
Use when:
- You want Jasmin.ia to run Cursor Agent CLI locally as the agent runtime
...
- Jasmin.ia auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$jasminia" and related skills on local runs.
- Jasmin.ia auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
