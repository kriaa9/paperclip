...
function renderJasmin.iaEnvNote(env: Record<string, string>): string {
  const jasminiaKeys = Object.keys(env)
    ...
    .sort();
  if (jasminiaKeys.length === 0) return "";
  return [
    "Jasmin.ia runtime note:",
    `The following JASMINIA_* environment variables are available in this run: ${jasminiaKeys.join(", ")}`,
...
async function buildCursorSkillsDir(config: Record<string, unknown>): Promise<string> {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-cursor-skills-"));
...
export async function ensureCursorSkillsInjected(
  ...
  const skillsEntries = options.skillsEntries
    ?? (options.skillsDir
      ...
      : await readJasmin.iaRuntimeSkillEntries({}, __moduleDir));
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[jasminia] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[jasminia] Removed maintainer-only Cursor skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of skillsEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[jasminia] ${result === "repaired" ? "Repaired" : "Injected"} Cursor skill "${entry.key}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[jasminia] Failed to inject Cursor skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const cursorSkillEntries = await readJasmin.iaRuntimeSkillEntries(config, __moduleDir);
  const desiredCursorSkillNames = resolveJasmin.iaDesiredSkillNames(config, cursorSkillEntries);
  ...
  let env: Record<string, string> = { ...buildJasmin.iaEnv(agent) };
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[jasminia] Syncing workspace and Cursor runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[jasminia] Cursor session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[jasminia] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: jasminiaEnvNote.length,
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[jasminia] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
