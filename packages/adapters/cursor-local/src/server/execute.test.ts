...
describe("cursor execute", () => {
  it("installs the default agent command on a fresh sandbox lease before execution", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-cursor-fresh-lease-"));
    ...
    try {
      ...
      expect(prompt).toContain("Follow the jasminia heartbeat.");
    } finally {
  ...
  it("reruns sandbox command resolution after managed runtime setup and keeps the original sandbox home", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-cursor-fresh-lease-managed-"));
