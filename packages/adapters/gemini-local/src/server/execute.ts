...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "JASMINIA_API_URL") || !hasNonEmptyEnvValue(env, "JASMINIA_API_KEY")) return "";
  return [
    "Jasmin.ia API access note:",
    "Use run_shell_command with curl to make Jasmin.ia API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $JASMINIA_API_KEY\\" \\"$JASMINIA_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $JASMINIA_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Jasmin.ia-Run-Id: $JASMINIA_RUN_ID\\" -d '{...}' \\"$JASMINIA_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject Jasmin.ia skills directly into `~/.gemini/skills/` via symlinks.
...
async function ensureGeminiSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[jasminia] Failed to prepare Gemini skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[jasminia] Removed maintainer-only Gemini skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[jasminia] ${result === "repaired" ? "Repaired" : "Linked"} Gemini skill: ${entry.key}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[jasminia] Failed to link Gemini skill "${entry.key}": ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[jasminia] Syncing workspace and Gemini runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[jasminia] Gemini session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[jasminia] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  const promptMetrics = {
    ...
    runtimeNoteChars: jasminiaEnvNote.length + apiAccessNote.length,
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[jasminia] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
