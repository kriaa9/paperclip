...
describe("cursor_cloud execute", () => {
  ...
  it("creates a fresh Cursor agent and injects Jasmin.ia env without CURSOR_API_KEY", async () => {
    ...
    expect(createMock.mock.calls[0]?.[0]).toMatchObject({
      ...
      name: "Jasmin.ia Cursor Cloud Agent",
      model: { id: "gpt-5.4" },
      cloud: {
        env: { type: "cloud" },
    ...
    expect(createMock.mock.calls[0]?.[0]?.cloud?.envVars).toMatchObject({
      EXTRA_FLAG: "1",
      JASMINIA_RUN_ID: "run-heartbeat-1",
      JASMINIA_TASK_ID: "issue-1",
      JASMINIA_WAKE_REASON: "issue_commented",
      JASMINIA_API_KEY: "jasminia-run-jwt",
    });
  ...
  it("maps non-finished Cursor results to failing Jasmin.ia runs", async () => {
