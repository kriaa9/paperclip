...
Use when:
- You want Jasmin.ia to run OpenCode locally as the agent runtime
...
- dangerouslySkipPermissions (boolean, optional): inject a runtime OpenCode config that allows \`external_directory\` access without interactive prompts; defaults to true for unattended Jasmin.ia runs
...
- Jasmin.ia requires an explicit \`model\` value for \`opencode_local\` agents.
...
- When \`dangerouslySkipPermissions\` is enabled, Jasmin.ia injects a temporary \
