...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.JASMINIA_OPENCODE_COMMAND === "string" &&
    process.env.JASMINIA_OPENCODE_COMMAND.trim().length > 0
      ? process.env.JASMINIA_OPENCODE_COMMAND.trim()
      : "opencode";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["JASMINIA_", "npm_", "NPM_"] as const;
