...
const STATEMENT_BREAKPOINT = "-- jasminia statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900";
...
function formatSqlLiteral(value: string): string {
  ...
  while (sanitized.includes(tag)) {
    tag = `$jasminia_${Math.random().toString(36).slice(2, 8)}$`;
  }
...
async function runPgDumpBackup(opts: {
  ...
  const pgDumpBin = process.env.JASMINIA_PG_DUMP_PATH || "pg_dump";
  const child = spawn(
    pgDumpBin,
    [
...
async function restoreWithPsql(opts: RunDatabaseRestoreOptions, connectTimeout: number): Promise<void> {
  const psqlBin = process.env.JASMINIA_PSQL_PATH || "psql";
  const child = spawn(
    psqlBin,
    [
      `--dbname=${opts.connectionString}`,
...
export async function runDatabaseBackup(opts: RunDatabaseBackupOptions): Promise<RunDatabaseBackupResult> {
  const filenamePrefix = opts.filenamePrefix ?? "jasminia";
  ...
  try {
    ...
    emit("-- Jasmin.ia database backup");
