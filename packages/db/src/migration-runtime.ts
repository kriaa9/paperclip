...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://jasminia:jasminia@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "jasminia");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://jasminia:jasminia@127.0.0.1:${preferredPort}/jasminia`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://jasminia:jasminia@127.0.0.1:${port}/jasminia`,
  ...
  const adminConnectionString = `postgres://jasminia:jasminia@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "jasminia");

  return {
    connectionString: `postgres://jasminia:jasminia@127.0.0.1:${selectedPort}/jasminia`,
