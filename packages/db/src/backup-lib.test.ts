...
async function createTempDatabase(): Promise<string> {
  const db = await startEmbeddedPostgresTestDatabase("jasminia-db-backup-");
...
describe("createBufferedTextFileWriter", () => {
  it("preserves line boundaries across buffered flushes", async () => {
    const tempDir = createTempDir("jasminia-buffered-writer-");
...
describeEmbeddedPostgres("runDatabaseBackup", () => {
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        ...
        "jasminia_restore_target",
      );
      const backupDir = createTempDir("jasminia-db-backup-output-");
      ...
      try {
        ...
        const result = await runDatabaseBackup({
          ...
          filenamePrefix: "jasminia-test",
          backupEngine: "javascript",
        });

        expect(result.backupFile).toMatch(/jasminia-test-.*\.sql\.gz$/);
  ...
  it(
    ...
    async () => {
      ...
      const restoreConnectionString = await createSiblingDatabase(
        sourceConnectionString,
        "jasminia_full_logical_restore_target",
      );
      const backupDir = createTempDir("jasminia-db-full-logical-backup-");
      const sourceSql = postgres(sourceConnectionString, { max: 1, onnotice: () => {} });
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      ...
      try {
        await sourceSql.unsafe(`
          ...
          VALUES ('jasminia-migration-history', 1770000000000);
        `);
        await sourceSql.unsafe(`
        ...
        const result = await runDatabaseBackup({
          ...
          backupDir,
          retention: { dailyDays: 7, weeklyWeeks: 4, monthlyMonths: 1 },
          filenamePrefix: "jasminia-full-logical-test",
          backupEngine: "javascript",
        ...
        const migrationRows = await restoreSql.unsafe<{ hash: string }[]>(`
          ...
          WHERE "hash" = 'jasminia-migration-history'
        `);
        expect(migrationRows).toEqual([{ hash: "jasminia-migration-history" }]);
  ...
  it(
    ...
    async () => {
      ...
      const restoreSql = postgres(restoreConnectionString, { max: 1, onnotice: () => {} });
      const backupDir = createTempDir("jasminia-db-restore-manual-");
      ...
      try {
        await fs.promises.writeFile(
          backupFile,
          [
            "-- Jasmin.ia database backup",
            "-- Created: 2026-04-06T00:00:00.000Z",
            "",
            "BEGIN;",
            "-- jasminia statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900",
