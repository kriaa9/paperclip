...
type GlobalBridge = typeof globalThis & {
  __jasminiaPluginBridge__?: PluginBridgeRegistry;
};

function getBridgeRegistry(): PluginBridgeRegistry | undefined {
  return (globalThis as GlobalBridge).__jasminiaPluginBridge__;
}

function missingBridgeValueError(name: string): Error {
  return new Error(
    `Jasmin.ia plugin UI runtime is not initialized for "${name}". ` +
...
export function renderSdkUiComponent<TProps>(
  ...
  throw new Error(`Jasmin.ia plugin UI component "${name}" is not callable`);
}
