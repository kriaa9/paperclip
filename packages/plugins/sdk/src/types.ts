...
export interface ScopeKey {
  /** What kind of Jasmin.ia object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Jasmin.ia UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Jasmin.ia domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Jasmin.ia domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Jasmin.ia secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Jasmin.ia secret provider (e.g. `"MY_API_KEY"`).
   *
...
/**
 * `ctx.routines` — resolve and reconcile plugin-managed Jasmin.ia routines.
 *
