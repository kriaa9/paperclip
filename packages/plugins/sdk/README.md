...
- Plugin UI bundles run as same-origin JavaScript inside the main Jasmin.ia app. They can call ordinary Jasmin.ia HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Jasmin.ia information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
const child = await ctx.issues.create({
  ...
  originKind: "plugin:jasminia.missions:feature",
...
Avoid raw same-origin `href`s or `window.location.assign()` for Jasmin.ia-internal navigation from plugin UI. Those bypass the host router and can reload the whole app. External links should keep normal anchors with `target="_blank"` and `rel="noopener noreferrer"` as appropriate.
...
or configure work against Jasmin.ia entities. Both are controlled components and
...
```bash
jasminia-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```
...
- `GET /__jasminia__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__jasminia__/events` streams `reload` SSE events on UI build changes
