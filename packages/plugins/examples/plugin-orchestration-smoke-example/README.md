...
curl -X POST http://127.0.0.1:3100/api/plugins/install \
  ...
  -d '{"packageName":"/absolute/path/to/jasminia/packages/plugins/examples/plugin-orchestration-smoke-example","isLocalPath":true}'
```
...
```bash
curl -X POST http://127.0.0.1:3100/api/plugins/jasminiaai.plugin-orchestration-smoke-example/api/issues/<issue-id>/smoke \
  -H "Content-Type: application/json" \
