...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".jasminia-sdk");
...
/**
 * Generate a complete Jasmin.ia plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Jasmin.ia plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "jasminia-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["jasminia", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@jasminiaai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@jasminiaai/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@jasminiaai/plugin-sdk/ui";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@jasminiaai/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@jasminiaai/plugin-sdk\` and \`@jasminiaai/shared\` from a local Jasmin.ia checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.jasminia-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-jasminia-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <jasminia-sdk-path>]");
    process.exit(1);
  }
