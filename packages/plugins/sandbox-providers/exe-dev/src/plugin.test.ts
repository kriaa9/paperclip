...
describe("exe.dev sandbox provider plugin", () => {
  ...
  it("normalizes config and emits SSH guidance warnings", async () => {
    ...
    expect(result).toEqual({
      ok: true,
      warnings: [
        "The Jasmin.ia host must have SSH access to the created exe.dev VM, and its SSH key must be registered with exe.dev. The API token only covers provisioning.",
        "reuseLease keeps the VM alive between runs; this provider does not suspend retained VMs.",
      ],
      normalizedConfig: {
        apiKey: null,
        apiUrl: "https://exe.dev/exec",
        namePrefix: "jasminia-sandbox",
        image: "ubuntu:22.04",
        command: null,
        cpu: 4,
        memory: "8GB",
  ...
  it("acquires a lease by creating a VM and preparing the SSH workspace", async () => {
    fetchMock.mockResolvedValueOnce(
      new Response(JSON.stringify({
        vm_name: "jasminia-env-run",
        ssh_dest: "jasminia-env-run.exe.xyz",
        https_url: "https://jasminia-env-run.exe.xyz",
        status: "running",
      }), { status: 200 }),
    );
    ...
    expect(lease).toMatchObject({
      providerLeaseId: "jasminia-env-run",
      metadata: {
        provider: "exe-dev",
        vmName: "jasminia-env-run",
        sshDest: "jasminia-env-run.exe.xyz",
  ...
  it("surfaces exe.dev SSH onboarding guidance during lease acquisition", async () => {
    ...
    await expect(plugin.definition.onEnvironmentAcquireLease?.({
      ...
      "the Jasmin.ia host SSH key is not registered with exe.dev",
    );

    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'jasminia-env-run'");
  });
  ...
  it("returns exe.dev SSH onboarding guidance for command execution failures", async () => {
    ...
    expect(String(result?.stderr ?? "")).toContain("the Jasmin.ia host SSH key is not registered with exe.dev");
  ...
  it("probes by creating and then deleting a VM after SSH verification", async () => {
    fetchMock
      .mockResolvedValueOnce(
        new Response(JSON.stringify({
          vm_name: "jasminia-probe",
          ssh_dest: "jasminia-probe.exe.xyz",
          status: "running",
        }), { status: 200 }),
      )
    ...
    expect(result).toMatchObject({
      ok: true,
      summary: "Connected to exe.dev VM jasminia-probe.",
      metadata: {
        provider: "exe-dev",
        vmName: "jasminia-probe",
        sshDest: "jasminia-probe.exe.xyz",
        shellCommand: "bash",
      },
    });
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'jasminia-probe'");
  });
  ...
  it("returns onboarding guidance when probe hits exe.dev SSH registration", async () => {
    ...
    expect(String(result?.metadata?.error ?? "")).toContain("the Jasmin.ia host SSH key is not registered with exe.dev");
    expect(String(fetchMock.mock.calls[1]?.[1]?.body ?? "")).toBe("rm --json 'jasminia-probe'");
  });
  ...
  it("realizes a workspace by mkdir-ing the remote cwd over SSH when VM metadata is present", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        ...
        metadata: {
          sshDest: "vm-1.exe.xyz",
          remoteCwd: "/srv/jasminia/run-1",
        },
      },
      workspace: {
        localPath: "/local/jasminia",
    ...
    expect(sshCommand).toContain("/srv/jasminia/run-1");
    expect(result).toMatchObject({
      cwd: "/srv/jasminia/run-1",
      metadata: {
        provider: "exe-dev",
        remoteCwd: "/srv/jasminia/run-1",
      },
  ...
  it("falls back through workspace.remotePath then workspace.localPath when lease.metadata.remoteCwd is missing", async () => {
    ...
    const result = await plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      workspace: {
        localPath: "/local/jasminia",
        remotePath: "/srv/jasminia/remote-fallback",
      },
    });

    expect(result?.cwd).toBe("/srv/jasminia/remote-fallback");
  });
  ...
  it("skips ensureRemoteWorkspace and returns the resolved cwd when no VM metadata is available", async () => {
    ...
    expect(result?.cwd).toBe("/srv/jasminia/no-vm");
  });
});
