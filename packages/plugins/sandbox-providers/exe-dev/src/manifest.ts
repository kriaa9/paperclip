...
const PLUGIN_ID = "jasminia.exe-dev-sandbox-provider";
...
const manifest: JasminiaPluginManifestV1 = {
  ...
  description:
    "Sandbox provider plugin that provisions exe.dev VMs as Jasmin.ia execution environments.",
  author: "Jasmin.ia",
  ...
  environmentDrivers: [
    {
      ...
      description:
        "Provisions exe.dev VMs through the HTTPS API, then runs commands over direct SSH for long-lived Jasmin.ia workloads.",
      configSchema: {
        type: "object",
        properties: {
          apiKey: {
            ...
            format: "secret-ref",
            description:
              "Environment-specific exe.dev API token. Needs `/exec` permission for at least `new`, `ls`, and `rm`. Paste a token or an existing Jasmin.ia secret reference; saved environments store pasted values as company secrets. Falls back to EXE_API_KEY if omitted.",
          },
          apiUrl: {
            type: "string",
            description:
          ...
          sshPrivateKey: {
            ...
            maxLength: 4096,
            description:
              "Optional exe.dev-registered SSH private key. Paste the private key or an existing Jasmin.ia secret reference; saved environments store pasted values as company secrets. If omitted, Jasmin.ia falls back to sshIdentityFile, then the host's default SSH agent/keychain.",
          },
          sshIdentityFile: {
            type: "string",
            description:
              "Optional absolute path to the SSH private key the Jasmin.ia host should use for VM access when sshPrivateKey is omitted. Leave both blank to rely on the host's default SSH agent/keychain.",
          },
          sshPort: {
            type: "number",
