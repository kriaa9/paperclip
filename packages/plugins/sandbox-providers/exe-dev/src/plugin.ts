...
function normalizeNamePrefix(value: string | null): string {
  const normalized = (value ?? "jasminia")
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .replace(/-{2,}/g, "-");
  return normalized.length > 0 ? normalized.slice(0, 24) : "jasminia";
}
...
async function prepareSshIdentity(config: ExeDevDriverConfig): Promise<{
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "jasminia-exe-dev-ssh-"));
...
function formatSshFailure(
  ...
  if (
    ...
    return [
      `Failed to ${action} exe.dev VM ${vmName}: the Jasmin.ia host SSH key is not registered with exe.dev.`,
...
async function buildLease(
  ...
  const remoteCwd = requestedCwd?.trim() || path.posix.join(remote.homeDir, "jasminia-workspace");
