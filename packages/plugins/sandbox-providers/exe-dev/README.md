...
This package lives in the Jasmin.ia monorepo, but it is intentionally excluded from the root `pnpm` workspace and shaped to publish and install like a standalone npm package. That lets operators install it from the Plugins page by package name without introducing root lockfile churn.
...
- When you save an environment, Jasmin.ia stores pasted API keys and pasted SSH private keys as company secrets.
...
- SSH access from the Jasmin.ia host to the resulting `*.exe.xyz` VMs.
