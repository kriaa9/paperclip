...
describe("Cloudflare sandbox provider plugin", () => {
  ...
  it("maps acquire lease responses from the bridge", async () => {
    ...
    const lease = await plugin.definition.onEnvironmentAcquireLease?.({
      ...
      issueId: "issue-1",
      runId: "run-1",
      requestedCwd: "/workspace/jasminia",
      config: {
        bridgeBaseUrl: "https://bridge.example.workers.dev",
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "https://bridge.example.workers.dev/api/jasminia-sandbox/v1/leases/acquire",
      expect.objectContaining({
        method: "POST",
        headers: expect.any(Headers),
      }),
    );
    expect(requestHeadersAt().get("X-Jasmin.ia-Run-Id")).toBe("run-1");
    expect(requestHeadersAt().get("X-Jasmin.ia-Environment-Id")).toBe("env-1");
    expect(requestHeadersAt().get("X-Jasmin.ia-Issue-Id")).toBe("issue-1");
  ...
  it("returns expired lease semantics when resume reports lost state", async () => {
    ...
    const lease = await plugin.definition.onEnvironmentResumeLease?.({
      ...
      leaseMetadata: { remoteCwd: "/workspace/jasminia" },
      config: {
        bridgeBaseUrl: "https://bridge.example.workers.dev",
        bridgeAuthToken: "resolved-token",
      },
  ...
  it("routes bridge-channel execute calls through a dedicated session", async () => {
    ...
    expect(requestBodyAt()).toMatchObject({
      ...
      sessionId: "jasminia-bridge",
      env: {
        KEEP_ME: "visible",
      },
    });
    expect(requestBodyAt().env).not.toHaveProperty("JASMINIA_SANDBOX_EXEC_CHANNEL");
  });
  ...
  it("wraps realizeWorkspace bridge failures and forwards the issue header", async () => {
    ...
    await expect(plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        providerLeaseId: "pc-run-1-abcd1234",
        metadata: { remoteCwd: "/workspace/jasminia" },
      },
      workspace: {
    ...
    })).rejects.toThrow("Failed to prepare Cloudflare sandbox workspace at /workspace/jasminia: mkdir: permission denied");
