...
function buildHeaders(config: CloudflareDriverConfig, extra: BridgeClientHeaders = {}): Headers {
  ...
  if (extra.environmentId) headers.set("X-Jasmin.ia-Environment-Id", extra.environmentId);
  if (extra.runId) headers.set("X-Jasmin.ia-Run-Id", extra.runId);
  if (extra.issueId) headers.set("X-Jasmin.ia-Issue-Id", extra.issueId);
  return headers;
}
...
export function createCloudflareBridgeClient(options: BridgeClientOptions) {
  ...
  const apiPrefix = "/api/jasminia-sandbox/v1";
