...
describe("bridge routes", () => {
  ...
  it("writes lease sentinels through the named-session exec target", async () => {
    ...
    const response = await handleBridgeRequest(
      bridgeRequest("/api/jasminia-sandbox/v1/leases/acquire", {
        environmentId: "env-1",
        runId: "run-1",
    ...
    expect(sessionExec.mock.calls[0]?.[0]).toContain("/workspace/jasminia");
    expect(sessionExec.mock.calls[1]?.[0]).toContain("/workspace/jasminia/.jasminia-lease.json");
  });
  ...
  it("checks lease sentinels through the named-session exec target on resume", async () => {
    ...
    const response = await handleBridgeRequest(
      bridgeRequest("/api/jasminia-sandbox/v1/leases/resume", {
    ...
    expect(commandArg).toContain("/workspace/jasminia/.jasminia-lease.json");
    expect(optionsArg).toEqual({ cwd: "/", timeout: expect.any(Number) });
    expect(optionsArg).not.toHaveProperty("args");
  });
  ...
  it("streams exec stdout and completion metadata when requested", async () => {
    ...
    const response = await handleBridgeRequest(
      bridgeRequest("/api/jasminia-sandbox/v1/exec", {
        providerLeaseId: "pc-run-1-abcd1234",
        command: "echo",
        args: ["hello"],
        sessionStrategy: "named",
