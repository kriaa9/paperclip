...
describe("E2B sandbox provider plugin", () => {
  ...
  it("uses resolved config keys before falling back to E2B_API_KEY", async () => {
    ...
    expect(lease).toMatchObject({
      ...
      metadata: {
        provider: "e2b",
        remoteCwd: "/home/user/jasminia-workspace",
      },
    });
    expect(sandbox.commands.run).toHaveBeenNthCalledWith(1, "pwd");
    expect(sandbox.commands.run).toHaveBeenNthCalledWith(2, "mkdir -p '/home/user/jasminia-workspace'");
  });
  ...
  it("executes commands through a connected sandbox when stdin is provided", async () => {
    ...
    expect(sandbox.files.write).toHaveBeenCalledWith(expect.stringMatching(/^\/tmp\/jasminia-stdin-/), "input");
    ...
    expect(stdinCall[0]).toMatch(/exec env FOO='bar' 'printf' 'hello' < '\/tmp\/jasminia-stdin-/);
    ...
    expect(sandbox.files.remove).toHaveBeenCalledWith(expect.stringMatching(/^\/tmp\/jasminia-stdin-/));
    expect(result).toEqual({
      exitCode: 0,
      timedOut: false,
      stdout: "stdin\n",
  ...
  it("creates the remote workspace before returning it", async () => {
    ...
    await expect(plugin.definition.onEnvironmentRealizeWorkspace?.({
      ...
      lease: {
        ...
        metadata: { remoteCwd: "/home/user/jasminia-workspace" },
      },
      workspace: {
        localPath: "/tmp/jasminia-workspace",
      },
    })).resolves.toEqual({
      cwd: "/home/user/jasminia-workspace",
      metadata: {
        provider: "e2b",
        remoteCwd: "/home/user/jasminia-workspace",
      },
    ...
    expect(sandbox.commands.run).toHaveBeenCalledWith("mkdir -p '/home/user/jasminia-workspace'");
  });
