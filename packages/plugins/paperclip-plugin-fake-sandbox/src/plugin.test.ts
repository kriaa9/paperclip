...
describe("fake sandbox provider plugin", () => {
  it("runs a deterministic provider lifecycle through environment hooks", async () => {
    ...
    expect(realized.cwd).toContain("jasminia-fake-sandbox-");
  ...
  it("does not expose host-only environment variables to executed commands", async () => {
    const previousSecret = process.env.JASMINIA_FAKE_PLUGIN_HOST_SECRET;
    process.env.JASMINIA_FAKE_PLUGIN_HOST_SECRET = "should-not-leak";
    try {
      const definition = plugin.definition;
      const harness = createEnvironmentTestHarness({
        manifest,
      ...
      const executed = await harness.execute({
        ...
        lease,
        command: "sh",
        args: ["-lc", "test -z \"${JASMINIA_FAKE_PLUGIN_HOST_SECRET+x}\" && printf \"$EXPLICIT_ONLY\""],
        cwd: realized.cwd,
      ...
      if (previousSecret === undefined) {
        delete process.env.JASMINIA_FAKE_PLUGIN_HOST_SECRET;
      } else {
        process.env.JASMINIA_FAKE_PLUGIN_HOST_SECRET = previousSecret;
      }
    }
  });
