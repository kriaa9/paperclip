...
describe("jasminia MCP tools", () => {
  ...
  it("adds auth headers and run id to mutating requests", async () => {
    ...
    const tool = getTool("jasminiaUpdateIssue");
    await tool.execute({
      issueId: "PAP-1135",
      status: "done",
    });
    ...
    expect((init.headers as Record<string, string>)["X-Jasmin.ia-Run-Id"]).toBe(
  ...
  it("uses default company id for company-scoped list tools", async () => {
    ...
    const tool = getTool("jasminiaListIssues");
  ...
  it("uses default agent id for checkout requests", async () => {
    ...
    const tool = getTool("jasminiaCheckoutIssue");
    await tool.execute({
      issueId: "PAP-1135",
    });
  ...
  it("allows create issue requests to omit status so the API applies assignee defaults", async () => {
    ...
    const tool = getTool("jasminiaCreateIssue");
    await tool.execute({
  ...
  it("defaults issue document format to markdown", async () => {
    ...
    const tool = getTool("jasminiaUpsertIssueDocument");
    await tool.execute({
      issueId: "PAP-1135",
      key: "plan",
      body: "# Updated",
  ...
  it("controls issue workspace services through the current execution workspace", async () => {
    ...
    const tool = getTool("jasminiaControlIssueWorkspaceServices");
    await tool.execute({
      issueId: "PAP-1135",
      action: "restart",
  ...
  it("waits for an issue workspace runtime service URL", async () => {
    ...
    const tool = getTool("jasminiaWaitForIssueWorkspaceService");
    const response = await tool.execute({
      issueId: "PAP-1135",
      serviceName: "web",
      timeoutSeconds: 1,
  ...
  it("creates suggest_tasks interactions with the expected issue-scoped payload", async () => {
    ...
    const tool = getTool("jasminiaSuggestTasks");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates request_confirmation interactions with plan target payloads", async () => {
    ...
    const tool = getTool("jasminiaRequestConfirmation");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates approvals with the expected company-scoped payload", async () => {
    ...
    const tool = getTool("jasminiaCreateApproval");
    await tool.execute({
      type: "hire_agent",
  ...
  it("rejects invalid generic request paths", async () => {
    ...
    const tool = getTool("jasminiaApiRequest");
    const response = await tool.execute({
      method: "GET",
      path: "issues",
    });
