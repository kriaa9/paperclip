...
export class Jasmin.iaApiClient {
  ...
  resolveCompanyId(companyId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("companyId is required because JASMINIA_COMPANY_ID is not set");
    }
    return resolved;
  }
  ...
  resolveAgentId(agentId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("agentId is required because JASMINIA_AGENT_ID is not set");
    }
    return resolved;
  }
  ...
  async requestJson<T>(method: string, path: string, options: JsonRequestOptions = {}): Promise<T> {
    ...
    if ((options.includeRunId ?? isWriteMethod(method)) && this.config.runId) {
      headers["X-Jasmin.ia-Run-Id"] = this.config.runId;
    }
