...
import { trackRoutineRun } from "@jasminiaai/shared/telemetry";
...
function resolveRoutineVariableValues(
  ...
  for (const variable of variables) {
    ...
    // Jasmin.ia manages from execution context, so callers cannot override them.
  ...
  return {
    ...
    createTrigger: async (
      ...
      if (input.kind === "webhook") {
        ...
        secretMaterial = {
          webhookUrl: `${process.env.JASMINIA_API_URL}/api/routine-triggers/public/${publicId}/fire`,
    ...
    rotateTriggerSecret: async (
      ...
      return {
        trigger: trigger as RoutineTrigger,
        secretMaterial: {
          webhookUrl: `${process.env.JASMINIA_API_URL}/api/routine-triggers/public/${existing.publicId}/fire`,
    ...
    firePublicTrigger: async (publicId: string, input: {
      ...
      if (trigger.signingMode === "none") {
        ...
        // generic X-Jasmin.ia-Signature header so operators can use github_hmac
