 ...
 *    (`~/.jasminia/plugins/`) and `node_modules` for packages matching
 *    the `jasminia-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Jasmin.ia plugins.
 * Packages matching this pattern are considered Jasmin.ia plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "jasminia-plugin-";
...
export type PluginSource =
  | "local-filesystem"  // ~/.jasminia/plugins/ local directory
  | "npm"               // npm packages matching jasminia-plugin-* convention
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the jasminia-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "jasminia-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginLoader {
  ...
  /**
   * Discover Jasmin.ia plugins installed as npm packages in the current
   * Node.js environment matching the "jasminia-plugin-*" naming convention.
   *
   ...
   * the "jasminiaPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Jasmin.ia plugin.
   * Throws if the package is a Jasmin.ia plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
...
/**
 * Check whether a package name matches the Jasmin.ia plugin naming convention.
 * Accepts both the "jasminia-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
 ...
 *
 * The spec defines a "jasminiaPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const jasminiaPlugin = pkgJson["jasminiaPlugin"];
  if (
    jasminiaPlugin !== null &&
    typeof jasminiaPlugin === "object" &&
    !Array.isArray(jasminiaPlugin)
  ) {
    const manifestRelPath = (jasminiaPlugin as Record<string, unknown>)[
      "manifest"
    ];
 ...
 *   packageName: "jasminia-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Jasmin.ia plugin (no manifest found).`,
      );
    }
  ...
  async function refreshPluginManifestFromPackage(
    ...
    if (!manifest) {
      throw new Error(`Plugin package ${plugin.packageName} no longer exposes a Jasmin.ia manifest`);
    }
   ...
   * if the package is not a Jasmin.ia plugin.
   */
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      const workerOptions: WorkerStartOptions = {
        ...
        autoRestart: true,
        env: {
          JASMINIA_DEPLOYMENT_MODE: instanceInfo.deploymentMode ?? "",
          JASMINIA_DEPLOYMENT_EXPOSURE: instanceInfo.deploymentExposure ?? "",
        },
      };
