    ...
    if (!stat.isDirectory()) {
      ...
      if (access === "readWrite") {
        try {
          ...
          const probePath = path.join(realPath, `.jasminia-local-folder-probe-${process.pid}-${Date.now()}`);
...
export async function writePluginLocalFolderTextAtomic(
  ...
  const tempPath = path.join(
    ...
    `.jasminia-${path.basename(resolved.absolutePath)}-${process.pid}-${randomUUID()}.tmp`,
  );
...
export function defaultLocalFolderBasePath(pluginKey: string, companyId: string) {
  return path.join(os.homedir(), ".jasminia", "plugin-data", companyId, pluginKey);
}
