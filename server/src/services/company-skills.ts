...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "jasminia_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Jasmin.ia skills are read-only.",
      sourceLabel: "Jasmin.ia bundled",
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "Jasmin.ia workspace",
        sourceBadge: "jasminia",
...
export function companySkillService(db: Db) {
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "jasminia_bundled";
      out.push({
        key: skill.key,
