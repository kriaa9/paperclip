...
import type { AdapterRuntimeServiceReport } from "@jasminiaai/adapter-utils";
import type { Db } from "@jasminiaai/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@jasminiaai/db";
import {
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".jasminia", "dist", "node_modules"]);
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.JASMINIA_WORKSPACE_CWD = input.worktreePath;
  env.JASMINIA_WORKSPACE_PATH = input.worktreePath;
  env.JASMINIA_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.JASMINIA_WORKSPACE_BRANCH = input.branchName;
  env.JASMINIA_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.JASMINIA_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.JASMINIA_WORKSPACE_SOURCE = input.base.source;
  env.JASMINIA_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.JASMINIA_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.JASMINIA_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.JASMINIA_PROJECT_ID = input.base.projectId ?? "";
  env.JASMINIA_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.JASMINIA_AGENT_ID = input.agent.id ?? "";
  env.JASMINIA_AGENT_NAME = input.agent.name;
  env.JASMINIA_COMPANY_ID = input.agent.companyId;
  env.JASMINIA_ISSUE_ID = input.issue?.id ?? "";
  env.JASMINIA_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.JASMINIA_ISSUE_TITLE = input.issue?.title ?? "";
  env.JASMINIA_ISSUE_WORK_MODE = input.issue?.workMode ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.JASMINIA_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.JASMINIA_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.JASMINIA_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.JASMINIA_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.JASMINIA_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.JASMINIA_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.JASMINIA_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.JASMINIA_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.JASMINIA_PROJECT_ID = input.workspace.projectId ?? "";
  env.JASMINIA_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.JASMINIA_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".jasminia", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Jasmin.ia", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
