 ...
 * ~/.jasminia/adapter-plugins.json. This is the source of truth for which
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-jasminia-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  ...
  return {
    ...
    adapterSettingsPath: path.join(jasminiaDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "jasminia-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for Jasmin.ia external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
