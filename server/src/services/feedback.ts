...
const FEEDBACK_SCHEMA_VERSION = "jasminia-feedback-envelope-v2";
const FEEDBACK_BUNDLE_VERSION = "jasminia-feedback-bundle-v2";
const FEEDBACK_PAYLOAD_VERSION = "jasminia-feedback-v1";
const FEEDBACK_DESTINATION = "jasminia_labs_feedback_v1";
...
function captureStatusFromFiles(files: FeedbackTraceBundleFile[]): FeedbackTraceBundleCaptureStatus {
  ...
  const hasAdapterFiles = files.some((file) =>
    file.source !== "jasminia_run" &&
    file.source !== "jasminia_run_events" &&
    file.source !== "jasminia_run_log",
  );
...
async function buildOpenCodeTraceFiles(input: {
  ...
  const opencodeRoot = resolveHomeAwarePath(
    process.env.JASMINIA_OPENCODE_STORAGE_DIR ?? "~/.local/share/opencode",
  );
...
async function buildFeedbackTraceBundleFromRow(
  ...
  let jasminiaRun: Record<string, unknown> | null = null;
  ...
  if (!sourceRunId) {
    ...
    if (!run || run.companyId !== row.companyId) {
      ...
      jasminiaRun = sanitizeFeedbackValue(
        {
          id: run.id,
      ...
      files.push(makeBundleFile({
        path: "jasminia/run.json",
        contentType: "application/json",
        source: "jasminia_run",
        contents: `${JSON.stringify(jasminiaRun, null, 2)}\n`,
      }));

      const sanitizedEvents = sanitizeFeedbackValue(
        events,
        state,
        "bundle.jasminiaRun.events",
        MAX_TRACE_FILE_CHARS,
      );
      files.push(makeBundleFile({
        path: "jasminia/run-events.json",
        contentType: "application/json",
        source: "jasminia_run_events",
        contents: `${JSON.stringify(sanitizedEvents, null, 2)}\n`,
      }));

      if (logText) {
        files.push(makeBundleFile({
          path: "jasminia/run-log.ndjson",
          contentType: "application/x-ndjson",
          source: "jasminia_run_log",
          contents: `${sanitizeFeedbackText(logText, state, "bundle.jasminiaRun.log", MAX_TRACE_FILE_CHARS)}\n`,
        }));
      } else {
