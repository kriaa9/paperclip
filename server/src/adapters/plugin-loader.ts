...
function extractUiParserSource(
  ...
  const contractVersion = pkg.jasminia?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no jasminia.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
