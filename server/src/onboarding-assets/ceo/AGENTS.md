...
2. **Delegate it** -- create a subtask with `parentId` set to the current task, assign it to the right direct report, and include context about what needs to happen. Use these routing rules:
   ...
   - If the right report doesn't exist yet, use the `jasminia-create-agent` skill to hire one before delegating.
...
- Use child issues for delegated work and wait for Jasmin.ia wake events or comments instead of polling agents, sessions, or processes in a loop.
