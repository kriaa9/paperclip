...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@jasminiaai/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  jasminiaUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).jasminiaUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.jasminiaUpgradeContext = context;
