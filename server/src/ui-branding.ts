const FAVICON_BLOCK_START = "<!-- JASMINIA_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- JASMINIA_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- JASMINIA_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- JASMINIA_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.JASMINIA_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.JASMINIA_WORKTREE_NAME) ?? nonEmpty(env.JASMINIA_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.JASMINIA_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="jasminia-worktree-enabled" content="true" />',
    `<meta name="jasminia-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="jasminia-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="jasminia-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
