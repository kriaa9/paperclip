...
describeEmbeddedPostgres("plugin-managed agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-plugin-managed-agents-");
  ...
  it("materializes declared managed agent instructions with local folder paths", async () => {
    const previousHome = process.env.JASMINIA_HOME;
    const previousInstance = process.env.JASMINIA_INSTANCE_ID;
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-managed-agent-home-"));
    const wikiRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-managed-agent-wiki-")));
    process.env.JASMINIA_HOME = tempHome;
    process.env.JASMINIA_INSTANCE_ID = "test";
    try {
      const pluginManifest = manifest();
      ...
      if (previousHome === undefined) delete process.env.JASMINIA_HOME;
      else process.env.JASMINIA_HOME = previousHome;
      if (previousInstance === undefined) delete process.env.JASMINIA_INSTANCE_ID;
      else process.env.JASMINIA_INSTANCE_ID = previousInstance;
  ...
  it("respects board approval policy for new managed agents", async () => {
    ...
    expect(approval?.payload).toMatchObject({
      agentId: created.agentId,
      sourcePluginKey: "jasminia.managed-agents-test",
