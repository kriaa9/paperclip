...
import { testEnvironment } from "@jasminiaai/adapter-cursor-local/server";
...
const outPath = process.env.JASMINIA_TEST_ARGS_PATH;
if (outPath) {
...
describe("cursor environment diagnostics", () => {
  ...
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `jasminia-cursor-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
  ...
  it("adds --yolo to hello probe args by default", async () => {
    const root = path.join(
      os.tmpdir(),
      `jasminia-cursor-local-probe-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
    ...
    const result = await testEnvironment({
      ...
      config: {
        ...
        env: {
          ...
          JASMINIA_TEST_ARGS_PATH: argsCapturePath,
  ...
  it("does not auto-add --yolo when extraArgs already bypass trust", async () => {
    const root = path.join(
      os.tmpdir(),
      `jasminia-cursor-local-probe-extra-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
    const binDir = path.join(root, "bin");
    const cwd = path.join(root, "workspace");
    const argsCapturePath = path.join(root, "args.json");
  ...
  it("prefers ~/.local/bin/cursor-agent for remote sandbox probes when using the default command", async () => {
    const root = path.join(
      os.tmpdir(),
      `jasminia-cursor-sandbox-probe-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
  ...
  it("emits cursor_native_auth_present when cli-config.json has authInfo and CURSOR_API_KEY is unset", async () => {
    const root = path.join(
      os.tmpdir(),
      `jasminia-cursor-auth-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
  ...
  it("emits cursor_api_key_missing when neither env var nor native auth exists", async () => {
    const root = path.join(
      os.tmpdir(),
      `jasminia-cursor-noauth-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
    const cursorHome = path.join(root, ".cursor");
    const cwd = path.join(root, "workspace");
