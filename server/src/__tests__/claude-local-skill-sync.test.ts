...
describe("claude local skill sync", () => {
  ...
  it("defaults to mounting all built-in Jasmin.ia skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === jasminiaKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat Jasmin.ia skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("jasminia-claude-user-skills-");
    ...
    expect(snapshot.entries).toContainEqual(expect.objectContaining({
      ...
      readOnly: true,
      detail: "Installed outside Jasmin.ia management in the Claude skills home.",
    }));
  });
});
