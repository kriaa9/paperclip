...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Jasmin.ia Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@jasminia.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `jasminia-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "jasminia-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "jasminia-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "jasminia-close-check",
      baseRef: "main",
      metadata: {
