...
describe("heartbeat comment wake batching", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("jasminia-heartbeat-comment-wake-");
  ...
  it("defers approval-approved wakes for a running issue so the assignee resumes after the run", async () => {
    ...
    expect((deferred?.payload as Record<string, unknown>)._jasminiaWakeContext).toMatchObject({
      issueId,
      taskId: issueId,
      approvalId: "approval-1",
      approvalStatus: "approved",
  ...
  it("batches deferred comment wakes and forwards the ordered batch to the next run", async () => {
    ...
    try {
      ...
      const deferredContext = (deferredWake?.payload as Record<string, unknown> | null)?._jasminiaWakeContext as
      ...
      expect(secondPayload.jasminia).toMatchObject({
        wake: {
  ...
  it("promotes deferred comment wakes with their comments after the active run is cancelled", async () => {
    ...
    try {
      ...
      expect(promotedPayload.jasminia).toMatchObject({
        wake: {
  ...
  it("queues exactly one follow-up run when an issue-bound run exits without a comment", async () => {
    ...
    try {
      ...
      expect(firstPayload.jasminia).toMatchObject({
        wake: {
          reason: "issue_assigned",
          issue: {
            id: issueId,
      ...
      expect(String(firstPayload.message ?? "")).toContain("## Jasmin.ia Wake Payload");
