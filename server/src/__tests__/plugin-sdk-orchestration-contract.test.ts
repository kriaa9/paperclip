...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    expect(created.originKind).toBe("plugin:jasminia.test-orchestration");
  ...
  it("enforces plugin origin namespaces in the test harness", async () => {
    ...
    const created = await harness.ctx.issues.create({
      companyId,
      title: "Generated issue",
      originKind: "plugin:jasminia.test-orchestration:feature",
    });

    expect(created.originKind).toBe("plugin:jasminia.test-orchestration:feature");
    await expect(
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:jasminia.test-orchestration");
    await expect(
  ...
  it("supports generic plugin operation issue visibility in the test harness", async () => {
    ...
    expect(created.originKind).toBe("plugin:jasminia.test-orchestration:operation");
