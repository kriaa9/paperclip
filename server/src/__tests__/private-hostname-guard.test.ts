...
describe("privateHostnameGuard", () => {
  ...
  it("blocks unknown hostnames with remediation command", async () => {
    ...
    expect(res.body?.error).toContain(`please run pnpm jasminia allowed-hostname ${unknownHostname}`);
  });
  ...
  it("blocks unknown hostnames on page routes with plain-text remediation command", async () => {
    ...
    expect(res.send).toHaveBeenCalledWith(
      expect.stringContaining(`please run pnpm jasminia allowed-hostname ${unknownHostname}`),
    );
  }, 20_000);
});
