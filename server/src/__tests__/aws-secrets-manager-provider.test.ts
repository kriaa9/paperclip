...
describe("awsSecretsManagerProvider", () => {
  const previousEnv = {
    JASMINIA_SECRETS_AWS_REGION: process.env.JASMINIA_SECRETS_AWS_REGION,
    AWS_REGION: process.env.AWS_REGION,
    AWS_DEFAULT_REGION: process.env.AWS_DEFAULT_REGION,
    JASMINIA_SECRETS_AWS_DEPLOYMENT_ID: process.env.JASMINIA_SECRETS_AWS_DEPLOYMENT_ID,
    JASMINIA_SECRETS_AWS_KMS_KEY_ID: process.env.JASMINIA_SECRETS_AWS_KMS_KEY_ID,
  ...
  it("creates Jasmin.ia-managed AWS secrets without persisting plaintext in provider material", async () => {
    ...
    const provider = createAwsSecretsManagerProvider({
      ...
      gateway: {
        async createSecret(input) {
          ...
          return {
            ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company-1/openai-api-key",
    ...
    expect(calls).toEqual([
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          Name: "jasminia/prod-use1/company-1/openai-api-key",
          KmsKeyId: "arn:aws:kms:us-east-1:123456789012:key/test",
        }),
      }),
    ]);
    expect(JSON.stringify(prepared)).not.toContain("super-secret-value");
    expect(prepared.externalRef).toContain("jasminia/prod-use1/company-1/openai-api-key");
  ...
  it("creates AWS secrets from selected provider vault config without deployment env fallback", async () => {
    delete process.env.JASMINIA_SECRETS_AWS_REGION;
    delete process.env.AWS_REGION;
    delete process.env.AWS_DEFAULT_REGION;
    delete process.env.JASMINIA_SECRETS_AWS_DEPLOYMENT_ID;
    delete process.env.JASMINIA_SECRETS_AWS_KMS_KEY_ID;
      ...
      expect.objectContaining({
        ...
        input: expect.objectContaining({
          ...
          Tags: expect.arrayContaining([
            { Key: "jasminia:provider-owner", Value: "platform" },
            { Key: "jasminia:environment", Value: "production" },
          ]),
        }),
      }),
    ]);
  ...
  it("signs AWS Secrets Manager JSON requests with default runtime credentials", async () => {
    ...
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(
      new Response(
        JSON.stringify({
          ARN: "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod/company-1/openai-api-key",
          VersionId: "aws-version-1",
        }),
        { status: 200 },
      ),
    ...
    expect(calls).toEqual([
      {
        ...
        input: {
          ...
          VersionStages: ["JASMINIA_PENDING"],
        },
      },
    ]);
  ...
  it("rejects linked external references under the Jasmin.ia-managed namespace", async () => {
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
    ...
    await expect(
      provider.linkExternalSecret({
        externalRef:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company-2/openai-api-key",
        providerVersionRef: "linked-version-7",
      }),
    ).rejects.toThrow(/Jasmin.ia-managed namespace/i);
  });
  ...
  it("redacts AWS provider exception text when remote listing fails", async () => {
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Jasmin.ia is not authorized to perform secretsmanager:ListSecrets on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
        endpoint: "https://secretsmanager.us-east-1.amazonaws.com",
  ...
  it("resolves AWS secret values by provider version reference", async () => {
    ...
    const resolved = await provider.resolveVersion({
      material: {
        ...
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company-1/openai-api-key",
        versionId: "aws-version-2",
        source: "managed",
      },
      externalRef:
  ...
  it("warns when AWS provider configuration is incomplete and blocks managed writes", async () => {
    ...
    expect(health.message).toContain("missing JASMINIA_SECRETS_AWS_REGION");
    ...
    ).rejects.toThrow(/JASMINIA_SECRETS_AWS_REGION|AWS_REGION/i);
  });

  it("deletes only Jasmin.ia-managed AWS secrets", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
  ...
  it("archives pending Jasmin.ia-managed AWS versions without deleting the secret", async () => {
    const calls: Array<{ op: string; input: Record<string, unknown> }> = [];
    const provider = createAwsSecretsManagerProvider({
      config: {
        region: "us-east-1",
