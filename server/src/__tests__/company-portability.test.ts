...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "jasminia",
        name: "jasminia",
        description: "Jasmin.ia coordination skill",
        markdown: "---\nname: jasminia\ndescription: Jasmin.ia coordination skill\n---\n\n# Jasmin.ia\n",
        sourceType: "github",
        sourceLocator: "#
  ...
  it("exports referenced skills as stubs by default with sanitized Jasmin.ia extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Jasmin.ia"');
    ...
    expect(extension).toContain('schema: "jasminia/v1"');
    ...
    expect(extension).not.toContain("jasminiaSkillSync");
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    expect(asTextFile(exported.files[".jasminia.yaml"])).toContain("requireBoardApprovalForNewAgents: true");
  });
  ...
  it("exports default sidebar order into the Jasmin.ia extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".jasminia.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("exports the company logo into images/ and references it from .jasminia.yaml", async () => {
    ...
    expect(exported.files[".jasminia.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            cleanupCommand: "rm -rf .jasminia-tmp",
            remoteProvider: null,
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Jasmin.ia",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-portability-git-"));
  ...
  it("reads env inputs back from .jasminia.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .jasminia.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Jasmin.ia routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "jasminia-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Jasmin.ia" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Jasmin.ia"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .jasminia.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Jasmin.ia");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".jasminia.yaml"] = `${exported.files[".jasminia.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
