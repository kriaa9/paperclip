...
describeEmbeddedPostgres("plugin orchestration APIs", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-plugin-orchestration-");
  ...
  async function makeLocalRoot() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-plugin-host-folder-"));
    tempRoots.push(root);
    return root;
  }
  ...
  it("creates plugin-origin issues with full orchestration fields and audit activity", async () => {
    ...
    const services = buildHostServices(db, "plugin-record-id", "jasminia.missions", createEventBusStub());
    ...
    expect(stored?.originKind).toBe("plugin:jasminia.missions");
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          details: expect.objectContaining({
            ...
            sourcePluginKey: "jasminia.missions",
  ...
  it("enforces plugin origin namespaces", async () => {
    ...
    expect(featureIssue.originKind).toBe("plugin:jasminia.missions:feature");
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:jasminia.missions");
  ...
  it("creates plugin operation issues with the generic operation origin", async () => {
    ...
    expect(issue.originKind).toBe("plugin:jasminia.missions:operation");
        ...
        localFolders: [
          {
            ...
            access: "readWrite",
            requiredDirectories: ["raw", "wiki", "wiki/concepts", ".jasminia"],
  ...
  it("rejects worker local-folder access for undeclared manifest keys", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "jasminia.local-folders",
      packageName: "@jasminia/plugin-local-folders",
      version: "0.1.0",
      manifestJson: {
        id: "jasminia.local-folders",
        apiVersion: 1,
        version: "0.1.0",
  ...
  it("resolves plugin-managed projects by stable key without overwriting user edits", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "jasminia.missions",
      packageName: "@jasminia/plugin-missions",
      version: "0.1.0",
      apiVersion: 1,
      categories: ["automation"],
      status: "ready",
      manifestJson: {
        id: "jasminia.missions",
        apiVersion: 1,
        version: "0.1.0",
    ...
    const services = buildHostServices(db, pluginId, "jasminia.missions", createEventBusStub());
