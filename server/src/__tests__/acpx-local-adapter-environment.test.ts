...
import { testEnvironment } from "@jasminiaai/adapter-acpx-local/server";
import type { AdapterEnvironmentCheck } from "@jasminiaai/adapter-utils";
...
describe("acpx_local environment credential diagnostics", () => {
  ...
  it("emits an info-level Claude missing credential hint without changing diagnostic health", async () => {
    const root = path.join(os.tmpdir(), `jasminia-acpx-claude-noauth-${Date.now()}-${Math.random().toString(16).slice(2)}`);
  ...
  it("emits an info-level Codex credential hint when native auth is present", async () => {
    const root = path.join(os.tmpdir(), `jasminia-acpx-codex-auth-${Date.now()}-${Math.random().toString(16).slice(2)}`);
  ...
  it("emits an info-level Codex missing credential hint without changing diagnostic health", async () => {
    const root = path.join(os.tmpdir(), `jasminia-acpx-codex-noauth-${Date.now()}-${Math.random().toString(16).slice(2)}`);
    const codexHome = path.join(root, ".codex");
