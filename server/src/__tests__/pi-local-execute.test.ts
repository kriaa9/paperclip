...
import { execute } from "@jasminiaai/adapter-pi-local/server";
...
describe("pi_local execute", () => {
  it("fails the run when Pi exhausts automatic retries despite exiting 0", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-pi-execute-"));
  ...
  it("prepends installed skill bin/ dirs to the spawned Pi child PATH", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-pi-path-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "pi");
  ...
  it("does not expose bin/ dirs from skills that are not injected", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-pi-path-neg-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "pi");
    ...
    try {
      await execute({
        ...
        config: {
          ...
          model: "google/gemini-3-flash-preview",
          promptTemplate: "Keep working.",
          // required:false with no explicit jasminiaSkillSync preference →
          // resolveJasmin.iaDesiredSkillNames returns [] → skill is not injected.
          jasminiaRuntimeSkills: [
