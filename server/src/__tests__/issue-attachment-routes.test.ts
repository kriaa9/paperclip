...
describe("normalizeIssueAttachmentMaxBytes", () => {
  it("keeps the process-level attachment cap as the final cap", async () => {
    const previous = process.env.JASMINIA_ATTACHMENT_MAX_BYTES;
    process.env.JASMINIA_ATTACHMENT_MAX_BYTES = "5";
    vi.resetModules();
    try {
      ...
      if (previous === undefined) {
        delete process.env.JASMINIA_ATTACHMENT_MAX_BYTES;
      } else {
        process.env.JASMINIA_ATTACHMENT_MAX_BYTES = previous;
      }
      vi.resetModules();
    }
  });
