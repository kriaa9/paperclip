...
describe("resolveEnvironmentExecutionTarget", () => {
  ...
  it("keeps sandbox targets on bridge mode even when lease metadata includes a Jasmin.ia API URL", async () => {
    mockResolveEnvironmentDriverConfigForRuntime.mockResolvedValue({
      driver: "sandbox",
      config: {
        provider: "fake-plugin",
    ...
    const target = await resolveEnvironmentExecutionTarget({
      ...
      leaseId: "lease-1",
      leaseMetadata: {
        jasminiaApiUrl: "https://jasminia.example.test",
      },
      lease: null,
      environmentRuntime: null,
    });
    ...
    expect(target).not.toHaveProperty("jasminiaApiUrl");
    expect(target).not.toHaveProperty("jasminiaTransport");
  });
  ...
  it("resolves SSH execution targets in bridge mode", async () => {
    mockResolveEnvironmentDriverConfigForRuntime.mockResolvedValue({
      ...
      config: {
        ...
        port: 22,
        username: "jasminia",
        remoteWorkspacePath: "/srv/jasminia",
