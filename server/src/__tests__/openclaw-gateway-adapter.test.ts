...
import { execute, testEnvironment } from "@jasminiaai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@jasminiaai/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("jasminia:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("JASMINIA_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("JASMINIA_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## Jasmin.ia Wake Payload");
      ...
      expect(payload?.jasminia).toMatchObject({
        wake: {
          latestCommentId: "comment-2",
          commentIds: ["comment-1", "comment-2"],
        },
