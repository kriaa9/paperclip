...
describeEmbeddedPostgres("environmentRuntimeService", () => {
  ...
  it("acquires and releases an SSH run lease through the runtime seam", async () => {
    ...
    const fixtureRoot = await mkdtemp(path.join(os.tmpdir(), "jasminia-environment-runtime-ssh-"));
  ...
  it("uses plugin-backed sandbox config for execute and release", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "jasminia.fake-plugin-sandbox-provider",
      packageName: "@jasminiaai/plugin-fake-sandbox",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "jasminia.fake-plugin-sandbox-provider",
        apiVersion: 1,
        version: "1.0.0",
  ...
  it("delegates plugin environment leases through the plugin worker manager", async () => {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "acme.environments",
      packageName: "@acme/jasminia-environments",
      version: "1.0.0",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
