...
<head>
    <!-- JASMINIA_RUNTIME_BRANDING_START -->
    <!-- JASMINIA_RUNTIME_BRANDING_END -->
    <!-- JASMINIA_FAVICON_START -->
    <link rel="icon" href="/favicon.ico" sizes="48x48" />
    <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
    <link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png" />
    <link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png" />
    <!-- JASMINIA_FAVICON_END -->
</head>`;

describe("ui branding", () => {
  it("detects worktree mode from JASMINIA_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ JASMINIA_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ JASMINIA_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ JASMINIA_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      JASMINIA_IN_WORKTREE: "true",
      JASMINIA_WORKTREE_NAME: "jasminia-pr-432",
      JASMINIA_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("jasminia-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="jasminia-worktree-name"');
    expect(meta).toContain('content="jasminia-pr-432"');
    expect(meta).toContain('name="jasminia-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="jasminia-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="jasminia-worktree-name"');
  });
});
