...
describe("agent instructions service", () => {
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const jasminiaHome = await makeTempDir("jasminia-agent-instructions-home-");
    const externalRoot = await makeTempDir("jasminia-agent-instructions-external-");
    cleanupDirs.add(jasminiaHome);
    cleanupDirs.add(externalRoot);
    process.env.JASMINIA_HOME = jasminiaHome;
    process.env.JASMINIA_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("jasminia-agent-instructions-new-external-");
    cleanupDirs.add(jasminiaHome);
    cleanupDirs.add(externalRoot);
    process.env.JASMINIA_HOME = jasminiaHome;
    process.env.JASMINIA_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("jasminia-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const jasminiaHome = await makeTempDir("jasminia-agent-instructions-recover-");
    cleanupDirs.add(jasminiaHome);
    process.env.JASMINIA_HOME = jasminiaHome;
    process.env.JASMINIA_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const jasminiaHome = await makeTempDir("jasminia-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("jasminia-agent-instructions-stale-root-");
    cleanupDirs.add(jasminiaHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const jasminiaHome = await makeTempDir("jasminia-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("jasminia-agent-instructions-heal-write-stale-");
    cleanupDirs.add(jasminiaHome);
    cleanupDirs.add(staleRoot);
    process.env.JASMINIA_HOME = jasminiaHome;
    process.env.JASMINIA_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const jasminiaHome = await makeTempDir("jasminia-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("jasminia-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(jasminiaHome);
    cleanupDirs.add(staleRoot);
    process.env.JASMINIA_HOME = jasminiaHome;
    process.env.JASMINIA_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const jasminiaHome = await makeTempDir("jasminia-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("jasminia-agent-instructions-partial-root-");
    cleanupDirs.add(jasminiaHome);
    cleanupDirs.add(staleRoot);
    process.env.JASMINIA_HOME = jasminiaHome;
    process.env.JASMINIA_INSTANCE_ID = "test-instance";
