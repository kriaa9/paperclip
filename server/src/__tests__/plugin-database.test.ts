...
const multiMigrationPluginKey = "jasminia.dbfixture";
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["jasminia.dbtest", "jasminia.escape", "jasminia.refresh", multiMigrationPluginKey]) {
  ...
  async function createInstallablePluginPackage(
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        type: "module",
        jasminiaPlugin: { manifest: "./manifest.js" },
      }),
      "utf8",
    );
    await writeFile(
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("jasminia.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
    ...
    expect(workerManager.startWorker).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: {
          ...
          JASMINIA_DEPLOYMENT_EXPOSURE: "public",
        },
