...
import { companies, companySkills, createDb } from "@jasminiaai/db";
import {
...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-company-skills-service-");
  ...
  it("lists skills without exposing markdown content", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-heavy-skill-"));
