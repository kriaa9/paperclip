...
const capturePath = process.env.JASMINIA_TEST_CAPTURE_PATH;
const payload = {
  argv: process.argv.slice(2),
  prompt: fs.readFileSync(0, "utf8"),
  codexHome: process.env.CODEX_HOME || null,
  jasminiaWakePayloadJson: process.env.JASMINIA_WAKE_PAYLOAD_JSON || null,
  jasminiaApiUrl: process.env.JASMINIA_API_URL || null,
  jasminiaApiKey: process.env.JASMINIA_API_KEY || null,
  jasminiaApiBridgeMode: process.env.JASMINIA_API_BRIDGE_MODE || null,
  jasminiaEnvKeys: Object.keys(process.env)
...
type CapturePayload = {
  ...
  jasminiaWakePayloadJson: string | null;
  jasminiaApiUrl?: string | null;
  jasminiaApiKey?: string | null;
  jasminiaApiBridgeMode?: string | null;
  jasminiaEnvKeys: string[];
};
...
describe("codex execute", () => {
  it("uses a Jasmin.ia-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-default-"));
    ...
    delete process.env.JASMINIA_IN_WORKTREE;
    ...
    try {
      ...
      const result = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            JASMINIA_TEST_CAPTURE_PATH: capturePath,
          },
          promptTemplate: "Follow the jasminia heartbeat.",
        },
        context: {},
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using Jasmin.ia-managed Codex home"),
        }),
      );
    } finally {
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-notes-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(commandNotes).toContain(
        "Codex exec automatically applies repo-scoped AGENTS.md instructions from the current workspace; Jasmin.ia does not currently suppress that discovery.",
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-meta-"));
    const workspace = path.join(root, "workspace");
    ...
    try {
      ...
      expect(loggedEnv.JASMINIA_RESOLVED_COMMAND).toBe(commandPath);
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("injects bridge env into sandbox-managed remote runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-sandbox-"));
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.codexHome).toBe(path.join(remoteWorkspace, ".jasminia-runtime", "codex", "home"));
      expect(capture.jasminiaApiUrl).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(capture.jasminiaApiKey).not.toBe("run-jwt-token");
      expect(capture.jasminiaApiBridgeMode).toBe("queue_v1");
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousPath === undefined) delete process.env.PATH;
  ...
  it("injects structured Jasmin.ia wake payloads into env and prompt", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.jasminiaEnvKeys).toContain("JASMINIA_WAKE_PAYLOAD_JSON");
      expect(capture.jasminiaWakePayloadJson).not.toBeNull();
      expect(JSON.parse(capture.jasminiaWakePayloadJson ?? "{}")).toMatchObject({
        reason: "issue_commented",
        latestCommentId: "comment-2",
        commentIds: ["comment-1", "comment-2"],
      });
      expect(capture.prompt).toContain("## Jasmin.ia Wake Payload");
  ...
  it("classifies remote-compaction high-demand failures as retryable transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-transient-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("persists retry-not-before metadata for codex usage-limit failures", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-usage-limit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingCodexCommand(
  ...
  it("uses safer invocation settings and a fresh-session handoff for codex transient fallback retries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-fallback-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(capture.prompt).toContain("Jasmin.ia session handoff:");
  ...
  it("renders execution-stage wake instructions for reviewer and executor roles", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-stage-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const executorResult = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            JASMINIA_TEST_CAPTURE_PATH: executorCapturePath,
          },
          promptTemplate: "Follow the jasminia heartbeat.",
        },
        context: {
  ...
  it("renders an issue-scoped wake prompt even when the wake has no comments yet", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-issue-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    ...
    try {
      ...
      expect(capture.prompt).toContain("## Jasmin.ia Resume Delta");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      expect(capture.prompt).not.toContain("Follow the jasminia heartbeat.");
      expect(capture.prompt).not.toContain("You are managed instructions.");
      expect(invocationPrompt).toContain("## Jasmin.ia Resume Delta");
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const homeSkill = path.join(isolatedCodexHome, "skills", "jasminia");
    await fs.mkdir(workspace, { recursive: true });
    await fs.mkdir(sharedCodexHome, { recursive: true });
    await fs.writeFile(path.join(sharedCodexHome, "auth.json"), '{"token":"shared"}\n', "utf8");
    await fs.writeFile(path.join(sharedCodexHome, "config.toml"), 'model = "codex-mini-latest"\n', "utf8");
    ...
    process.env.HOME = root;
    process.env.JASMINIA_HOME = jasminiaHome;
    process.env.JASMINIA_INSTANCE_ID = "worktree-1";
    process.env.JASMINIA_IN_WORKTREE = "true";
    process.env.CODEX_HOME = sharedCodexHome;
    ...
    try {
      ...
      expect(capture.argv).toEqual(expect.arrayContaining(["exec", "--json", "-"]));
      expect(capture.prompt).toContain("Follow the jasminia heartbeat.");
      expect(capture.jasminiaEnvKeys).toEqual(
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining('Injected Codex skill "jasminia"'),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
  ...
  it("respects an explicit CODEX_HOME config override even in worktree mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-codex-execute-explicit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    try {
      ...
      expect((await fs.lstat(path.join(explicitCodexHome, "skills", "jasminia"))).isSymbolicLink()).toBe(true);
      await expect(fs.lstat(path.join(jasminiaHome, "instances", "worktree-1", "codex-home"))).rejects.toThrow();
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousJasminiaHome === undefined) delete process.env.JASMINIA_HOME;
