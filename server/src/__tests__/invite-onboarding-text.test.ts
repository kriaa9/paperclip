...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Jasmin.ia OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("/api/invites/token-123/skills/jasminia");
    expect(text).toContain("Suggested Jasmin.ia base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("jasminiaApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.jasminiaApiUrl");
    expect(text).toContain("~/.openclaw/workspace/jasminia-claimed-api-key.json");
    expect(text).toContain("JASMINIA_API_KEY");
  ...
  it("includes LAN candidates when the advertised host is tailnet-only", () => {
    ...
    try {
      const req = buildReq("jasminia.example.test:3103");
      const invite = {
        id: "invite-4",
        companyId: "company-1",
        inviteType: "company_join",
      ...
      const text = buildInviteOnboardingTextDocument(req, "token-999", invite as any, {
        ...
        deploymentExposure: "private",
        bindHost: "0.0.0.0",
        allowedHostnames: ["jasminia.example.test", "203.0.113.42"],
      });
