...
import { resolveJasmin.iaConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@jasminiaai/shared";
...
async function createTempRepo(defaultBranch = "main") {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "jasminia@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Jasmin.ia Test"]);
...
afterEach(async () => {
  ...
  delete process.env.JASMINIA_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Jasmin.ia and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/jasminia",
      JASMINIA_HOME: "/tmp/jasminia-home",
      JASMINIA_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.JASMINIA_HOME).toBeUndefined();
    expect(sanitized.JASMINIA_INSTANCE_ID).toBeUndefined();
...
describe("ensureServerWorkspaceLinksCurrent", () => {
  it("relinks stale server workspace dependencies inside the current repo root", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-links-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-links-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@jasminiaai");
    ...
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/jasminia-main/.git/worktrees/runtime-links\n", "utf8");
    await fs.writeFile(path.join(repoRoot, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n  - server\n", "utf8");
    await fs.writeFile(
      path.join(repoRoot, "server", "package.json"),
      JSON.stringify({
        name: "@jasminiaai/server",
        dependencies: {
          "@jasminiaai/db": "workspace:*",
        },
      }),
      "utf8",
    );
    await fs.writeFile(
      ...
      JSON.stringify({ name: "@jasminiaai/db" }),
      "utf8",
    );
    await fs.writeFile(
  ...
  it("skips relinking when server workspace dependencies already point at the repo", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-links-current-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@jasminiaai");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    ...
    await fs.mkdir(expectedPackageDir, { recursive: true });
    await fs.mkdir(serverNodeModulesScopeDir, { recursive: true });
    await fs.writeFile(path.join(repoRoot, ".git"), "gitdir: /tmp/jasminia-main/.git/worktrees/runtime-links-current\n", "utf8");
    await fs.writeFile(path.join(repoRoot, "pnpm-workspace.yaml"), "packages:\n  - packages/*\n  - server\n", "utf8");
    await fs.writeFile(
      path.join(repoRoot, "server", "package.json"),
      JSON.stringify({
  ...
  it("skips relinking outside linked git worktrees", async () => {
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-links-non-worktree-"));
    const staleRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-links-non-worktree-stale-"));
    const serverNodeModulesScopeDir = path.join(repoRoot, "server", "node_modules", "@jasminiaai");
    const expectedPackageDir = path.join(repoRoot, "packages", "db");
    const stalePackageDir = path.join(staleRoot, "db");
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".jasminia", "worktrees"));
  ...
  it("rejects reusing an empty directory that only looks like a worktree because it sits inside the repo", async () => {
    ...
    const poisonedPath = path.join(repoRoot, ".jasminia", "worktrees", branchName);
  ...
  it("reuses the current linked worktree instead of nesting another worktree inside it", async () => {
    ...
    const currentWorktree = path.join(repoRoot, ".jasminia", "worktrees", branchName);
  ...
  it("reuses an already checked out branch from git worktree metadata even when the target path differs", async () => {
    const repoRoot = await createTempRepo();
    const branchName = "PAP-1355-worktree-reuse";
    const existingWorktree = path.join(repoRoot, ".jasminia", "worktrees", branchName);
    ...
    const realized = await realizeExecutionWorkspace({
      ...
      config: {
        workspaceStrategy: {
          type: "git_worktree",
          branchTemplate: "{{issue.identifier}}-{{slug}}",
          worktreeParentDir: ".jasminia/other-worktrees",
        },
      },
      issue: {
        id: "issue-1",
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$JASMINIA_WORKSPACE_BRANCH\" > .jasminia-provision-branch",
        "printf '%s\\n' \"$JASMINIA_WORKSPACE_BASE_CWD\" > .jasminia-provision-base",
        "printf '%s\\n' \"$JASMINIA_WORKSPACE_CREATED\" > .jasminia-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".jasminia-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".jasminia-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".jasminia-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".jasminia-provision-created"), "utf8")).resolves.toBe("false\n");
  });
  ...
  it("uses the latest repo-managed provision script when reusing an existing worktree", async () => {
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".jasminia-provision-version"), "utf8")).resolves.toBe("v1\n");
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".jasminia-provision-version"), "utf8")).resolves.toBe("v2\n");
  }, 30_000);

  it("writes an isolated repo-local Jasmin.ia config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const previousPath = process.env.PATH;
    const jasminiaHome = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktrees-"));
    const isolatedBin = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-bin-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(jasminiaHome, "instances", instanceId);
    ...
    process.env.JASMINIA_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/jasminia"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".jasminia", "config.json");
      const envPath = path.join(workspace.cwd, ".jasminia", ".env");
      ...
      expect(envVars.JASMINIA_HOME).toBe(isolatedWorktreeHome);
      expect(envVars.JASMINIA_INSTANCE_ID).toBe(expectedInstanceId);
      expect(await fs.realpath(envVars.JASMINIA_CONFIG!)).toBe(await fs.realpath(configPath));
      expect(envVars.JASMINIA_IN_WORKTREE).toBe("true");
      expect(envVars.JASMINIA_WORKTREE_NAME).toBe("PAP-885-show-worktree-banner");

      process.chdir(workspace.cwd);
      expect(resolveJasmin.iaConfigPath()).toBe(configPath);
      ...
      await fs.writeFile(envPath, `${envContents}JASMINIA_WORKTREE_COLOR="#112233"\n`, "utf8");
      ...
      expect(reusedEnvContents).toContain('JASMINIA_WORKTREE_COLOR="#112233"');
    } finally {
  ...
  it("provisions successfully when install is needed but there are no symlinked node_modules to move", async () => {
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".jasminia", "config.json"), "utf8")).resolves.toContain(
      "\"database\"",
    );
  }, 30_000);

  it("fails instead of writing an unseeded fallback config when worktree init errors after CLI detection succeeds", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-provision-fail-"));
    ...
    try {
      ...
      await fs.writeFile(
        ...
        [
          "#!/bin/sh",
          "if [ \"$1\" = \"jasminiaai\" ] && [ \"$2\" = \"--help\" ]; then",
          "  exit 0",
          "fi",
          "if [ \"$1\" = \"jasminiaai\" ] && [ \"$2\" = \"worktree\" ] && [ \"$3\" = \"init\" ]; then",
      ...
      try {
        await execFileAsync(scriptPath, [], {
          ...
          env: {
            ...
            JASMINIA_WORKSPACE_BASE_CWD: baseRoot,
            JASMINIA_WORKSPACE_CWD: worktreeRoot,
          },
        });
      } catch (error) {
      ...
      await expect(fs.stat(path.join(worktreeRoot, ".jasminia", "config.json"))).rejects.toThrow();
      await expect(fs.stat(path.join(worktreeRoot, ".jasminia", ".env"))).rejects.toThrow();
    } finally {
  ...
  it("retries worktree-local pnpm install without a frozen lockfile when the lockfile is outdated", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-outdated-lockfile-"));
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await expect(fs.readFile(path.join(worktreeRoot, ".jasminia", "config.json"), "utf8")).resolves.toContain(
        "\"database\"",
      );
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
  ...
  it("reattaches a missing persisted git worktree before manual control starts it", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$JASMINIA_WORKSPACE_BRANCH\" > .jasminia-restored-branch",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".jasminia-restored-branch"), "utf8")).resolves.toBe(`${branchName}\n`);
  ...
  it("reprovisions an existing persisted git worktree before manual control starts it", async () => {
    ...
    await fs.rm(path.join(initial.cwd, ".jasminia-restored-state"), { force: true });
    ...
    await expect(fs.readFile(path.join(initial.cwd, ".jasminia-restored-state"), "utf8")).resolves.toBe("reprovisioned\n");
  }, 15_000);
  ...
  it("auto-detects the default branch when baseRef is not configured", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-bare-"));
  ...
  it("auto-detects the default branch via symbolic-ref when origin/HEAD is set", async () => {
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-bare-symref-"));
    await runGit(bareRemote, ["init", "--bare"]);
    await runGit(repoRoot, ["remote", "add", "origin", bareRemote]);
    await runGit(repoRoot, ["push", "-u", "origin", "master"]);
    await runGit(repoRoot, ["fetch", "origin"]);
...
describe("ensureRuntimeServicesForRun", () => {
  it("leaves manual runtime services untouched during agent runs", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-manual-"));
  ...
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-workspace-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".jasminia", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.JASMINIA_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              JASMINIA_HOME: "{{workspace.cwd}}/.jasminia/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".jasminia", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".jasminia", "runtime-services"));
  });

  it("does not leak parent Jasmin.ia instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "jasminiaConfig: process.env.JASMINIA_CONFIG ?? null,",
          "jasminiaHome: process.env.JASMINIA_HOME ?? null,",
          "jasminiaInstanceId: process.env.JASMINIA_INSTANCE_ID ?? null,",
    ...
    process.env.JASMINIA_CONFIG = "/tmp/base-jasminia-config.json";
    process.env.JASMINIA_HOME = "/tmp/base-jasminia-home";
    process.env.JASMINIA_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/jasminia";
    ...
    expect(captured.jasminiaConfig).toBeNull();
    expect(captured.jasminiaHome).toBeNull();
    expect(captured.jasminiaInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-sibling-"));
  ...
  it("starts only the selected workspace-controlled runtime service", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-control-start-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("stops only the selected execution workspace runtime service", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-control-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-workspace-runtime-");
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-reconcile-"));
    const jasminiaHome = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-home-"));
    process.env.JASMINIA_HOME = jasminiaHome;
    process.env.JASMINIA_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("marks persisted local services stopped when the registry pid is stale", async () => {
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      serviceName: "jasminia-dev",
      status: "running",
      lifecycle: "shared",
      reuseKey: `project_workspace:${projectWorkspaceId}:jasminia-dev`,
      command: "pnpm dev",
      cwd: "/tmp/jasminia-primary",
      port: 49195,
    ...
    await writeLocalServiceRegistryRecord({
      version: 1,
      serviceKey: "workspace-runtime-jasminia-dev-stale",
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-stop-persisted-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const runId = randomUUID();
  ...
  it("restarts a stopped auto-port service on the same port when it is available", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-runtime-port-reuse-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
    const projectId = randomUUID();
    const executionWorkspaceId = randomUUID();
