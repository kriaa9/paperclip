...
describe("feedbackService.saveIssueVote", () => {
  ...
  beforeAll(async () => {
    const started = await startEmbeddedPostgresTestDatabase("jasminia-feedback-service-");
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    const instructionsDir = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-feedback-instructions-"));
  ...
  it("builds a detailed sanitized shared bundle with issue and agent context", async () => {
    ...
    expect(trace?.schemaVersion).toBe("jasminia-feedback-envelope-v2");
    expect(trace?.bundleVersion).toBe("jasminia-feedback-bundle-v2");
  ...
  it("keeps earlier local votes local when a later vote enables sharing", async () => {
    ...
    expect(localTrace?.payloadVersion).toBe("jasminia-feedback-v1");
    ...
    expect(sharedTrace?.payloadVersion).toBe("jasminia-feedback-v1");
  });

  it("captures Claude project session artifacts as full traces", async () => {
    const claudeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-feedback-claude-"));
  ...
  it("captures OpenCode message and part files as full traces", async () => {
    const opencodeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-feedback-opencode-"));
    ...
    vi.stubEnv("JASMINIA_OPENCODE_STORAGE_DIR", opencodeRoot);
    const uploadTraceBundle = vi.fn().mockResolvedValue({ objectKey: "feedback-traces/test.json" });
    const flushingSvc = feedbackService(db, {
      shareClient: {
        uploadTraceBundle,
  ...
  it("flushes pending shared traces into configured object storage and marks them sent", async () => {
    ...
    expect(uploadTraceBundle.mock.calls[0]?.[0]).toMatchObject({
      ...
      envelope: {
        destination: "jasminia_labs_feedback_v1",
        exportId: traces[0]?.exportId,
      },
    });
  });
