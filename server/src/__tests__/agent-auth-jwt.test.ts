...
describe("agent local JWT", () => {
  const secretEnv = "JASMINIA_AGENT_JWT_SECRET";
  const betterAuthSecretEnv = "BETTER_AUTH_SECRET";
  const ttlEnv = "JASMINIA_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "JASMINIA_AGENT_JWT_ISSUER";
  const audienceEnv = "JASMINIA_AGENT_JWT_AUDIENCE";
  ...
  it("falls back to BETTER_AUTH_SECRET when JASMINIA_AGENT_JWT_SECRET is absent", () => {
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "jasminia";
    process.env[audienceEnv] = "jasminia-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
