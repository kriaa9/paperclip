...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const jasminiaDir = path.join(worktreeRoot, ".jasminia");
    const configPath = path.join(jasminiaDir, "config.json");
    const envPath = path.join(jasminiaDir, ".env");
    const sharedRoot = path.join(tempRoot, ".jasminia", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".jasminia-worktrees");

    await fs.mkdir(jasminiaDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Jasmin.ia environment variables",
        "JASMINIA_IN_WORKTREE=true",
        "JASMINIA_WORKTREE_NAME=PAP-884-ai-commits-component",
        "JASMINIA_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.JASMINIA_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.JASMINIA_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`JASMINIA_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('JASMINIA_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`JASMINIA_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`JASMINIA_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('JASMINIA_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.JASMINIA_HOME).toBe(isolatedHome);
    expect(process.env.JASMINIA_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Jasmin.ia environment variables",
        "JASMINIA_IN_WORKTREE=true",
        "JASMINIA_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.JASMINIA_IN_WORKTREE = "true";
    process.env.JASMINIA_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.JASMINIA_WORKTREES_DIR = isolatedHome;
    delete process.env.JASMINIA_HOME;
    delete process.env.JASMINIA_INSTANCE_ID;
    delete process.env.JASMINIA_CONFIG;
  ...
  it("does not persist transient runtime home overrides over repo-local worktree env", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-runtime-override-"));
    const isolatedHome = path.join(tempRoot, ".jasminia-worktrees");
    ...
    await fs.writeFile(
      ...
      [
        "# Jasmin.ia environment variables",
        `JASMINIA_HOME=${JSON.stringify(isolatedHome)}`,
        `JASMINIA_INSTANCE_ID=${JSON.stringify(instanceId)}`,
        `JASMINIA_CONFIG=${JSON.stringify(configPath)}`,
        `JASMINIA_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`,
        'JASMINIA_IN_WORKTREE="true"',
        'JASMINIA_WORKTREE_NAME="PAP-989-multi-user-implementation-using-plan-from-pap-958"',
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.JASMINIA_IN_WORKTREE = "true";
    process.env.JASMINIA_WORKTREE_NAME = "PAP-989-multi-user-implementation-using-plan-from-pap-958";
    process.env.JASMINIA_HOME = transientHome;
    ...
    expect(repairedEnv).toContain(`JASMINIA_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).not.toContain(`JASMINIA_HOME=${JSON.stringify(transientHome)}`);
    expect(process.env.JASMINIA_HOME).toBe(isolatedHome);
  });

  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".jasminia-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".jasminia", "worktrees");
    ...
    const jasminiaDir = path.join(currentWorktreeRoot, ".jasminia");
    const configPath = path.join(jasminiaDir, "config.json");
    const envPath = path.join(jasminiaDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".jasminia", "config.json");
  ...
  it("persists runtime-selected worktree ports back into explicit-port auth URLs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.JASMINIA_IN_WORKTREE = "true";
    process.env.JASMINIA_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.JASMINIA_HOME = isolatedHome;
    process.env.JASMINIA_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.JASMINIA_CONFIG = configPath;
  ...
  it("does not rewrite no-port public auth URLs when persisting runtime-selected ports", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-worktree-public-ports-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify(
        {
          ...buildLegacyConfig(instanceRoot, "https://jasminia.example"),
          database: {
            mode: "embedded-postgres",
            embeddedPostgresDataDir: path.join(instanceRoot, "db"),
            embeddedPostgresPort: 54331,
    ...
    process.chdir(worktreeRoot);
    process.env.JASMINIA_IN_WORKTREE = "true";
    process.env.JASMINIA_WORKTREE_NAME = "PAP-125-public-base-url";
    process.env.JASMINIA_HOME = isolatedHome;
    process.env.JASMINIA_INSTANCE_ID = "pap-125-public-base-url";
    process.env.JASMINIA_CONFIG = configPath;
    ...
    expect(writtenConfig.server.port).toBe(3103);
    expect(writtenConfig.database.embeddedPostgresPort).toBe(54335);
    expect(writtenConfig.auth.publicBaseUrl).toBe("https://jasminia.example");
  });
  ...
  it("does not rewrite the in-memory config when auth URL has no explicit port", () => {
    const { config, changed } = applyRuntimePortSelectionToConfig(
      buildLegacyConfig("/tmp/shared", "https://jasminia.example"),
      {
        serverPort: 3104,
        databasePort: 54340,
        allowServerPortWrite: false,
    ...
    expect(config.server.port).toBe(3100);
    expect(config.database.embeddedPostgresPort).toBe(54340);
    expect(config.auth.publicBaseUrl).toBe("https://jasminia.example");
  });
});
