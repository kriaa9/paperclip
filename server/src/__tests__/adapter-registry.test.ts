...
import { buildSandboxNpmInstallCommand } from "@jasminiaai/adapter-utils";
...
vi.mock("hermes-jasminia-adapter/server", () => ({
...
describe("server adapter registry", () => {
  ...
  it("injects the local agent JWT and Jasmin.ia API auth guidance into Hermes", async () => {
    ...
    expect(patchedCtx.agent.adapterConfig).toMatchObject({
      env: {
        OPENAI_API_KEY: "llm-token",
        JASMINIA_API_KEY: "agent-run-jwt",
        JASMINIA_RUN_ID: "run-123",
      },
    });
    expect(patchedCtx.agent.adapterConfig.promptTemplate).toContain(
      "Authorization: Bearer $JASMINIA_API_KEY",
    );
    expect(patchedCtx.agent.adapterConfig.promptTemplate).toContain(
      "X-Jasmin.ia-Run-Id: $JASMINIA_RUN_ID",
    );
  ...
  it("preserves Hermes command normalization while injecting auth", async () => {
    ...
    expect(patchedCtx.agent.adapterConfig.env.JASMINIA_API_KEY).toBe("agent-run-jwt");
  });
  ...
  it("passes the original Hermes context through when authToken is absent", async () => {
    ...
    const ctx = {
      ...
      agent: {
        ...
        adapterConfig: {
          env: {
            JASMINIA_API_KEY: "server-level-key",
          },
          promptTemplate: "Existing prompt",
        },
      },
  ...
  it("preserves an explicit Hermes Jasmin.ia API key and does not set promptTemplate when none was configured", async () => {
    const adapter = requireServerAdapter("hermes_local");
    ...
    await adapter.execute({
      ...
      agent: {
        ...
        adapterConfig: {
          env: {
            JASMINIA_API_KEY: "explicit-agent-key",
            JASMINIA_RUN_ID: "stale-run-id",
          },
        },
      },
      runtime: {},
    ...
    const [patchedCtx] = hermesExecuteMock.mock.calls[0];
    expect(patchedCtx.agent.adapterConfig.env.JASMINIA_API_KEY).toBe("explicit-agent-key");
    expect(patchedCtx.agent.adapterConfig.env.JASMINIA_RUN_ID).toBe("run-123");
