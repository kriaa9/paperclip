...
describe("secret provider registry", () => {
  ...
  const previousMasterKey = process.env.JASMINIA_SECRETS_MASTER_KEY;
  ...
  afterEach(() => {
    ...
    if (previousMasterKey === undefined) {
      ...
      process.env.JASMINIA_SECRETS_MASTER_KEY = previousMasterKey;
    }
  ...
  it("warns when the local encrypted key file is readable by group or others", async () => {
    const dir = path.join(os.tmpdir(), `jasminia-secret-provider-${randomBytes(6).toString("hex")}`);
    tmpDirs.push(dir);
    ...
    process.env.JASMINIA_SECRETS_MASTER_KEY_FILE = keyFile;
    delete process.env.JASMINIA_SECRETS_MASTER_KEY;
