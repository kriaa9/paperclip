...
function createTempStatusFile(payload: unknown) {
  const dir = mkdtempSync(path.join(os.tmpdir(), "jasminia-dev-status-"));
  tempDirs.push(dir);
...
describe("dev server status helpers", () => {
  it("reads and normalizes persisted supervisor state", () => {
    ...
    expect(readPersistedDevServerStatus({ JASMINIA_DEV_SERVER_STATUS_FILE: filePath })).toEqual({
      dirty: true,
      lastChangedAt: "2026-03-20T12:00:00.000Z",
      changedPathCount: 4,
      changedPathsSample: ["server/src/app.ts", "packages/shared/src/index.ts"],
  ...
  it("ignores oversized persisted status files", () => {
    ...
    expect(readPersistedDevServerStatus({ JASMINIA_DEV_SERVER_STATUS_FILE: filePath })).toBeNull();
  });
});
