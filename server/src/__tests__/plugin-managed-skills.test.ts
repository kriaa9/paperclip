...
describeEmbeddedPostgres("plugin-managed skills", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-plugin-managed-skills-");
  ...
  it("installs and resolves managed company skills by stable resource key", async () => {
    ...
    expect(created.skill).toMatchObject({
      ...
      key: "plugin/jasminia-managed-skills-test/wiki-maintainer",
      sourceType: "catalog",
  ...
  it("preserves operator edits during reconcile and restores manifest defaults on reset", async () => {
    ...
    expect(reset.skill?.markdown).toContain("key: \"plugin/jasminia-managed-skills-test/wiki-maintainer\"");
  });
  ...
  it("adds the canonical managed key to manifest-provided markdown skills", async () => {
    ...
    expect(created.status).toBe("created");
    expect(created.skill).toMatchObject({
      key: "plugin/jasminia-managed-skills-test/markdown-skill",
      name: "markdown-skill",
    });
    expect(created.skill?.markdown).toContain("key: \"plugin/jasminia-managed-skills-test/markdown-skill\"");
  });
});
