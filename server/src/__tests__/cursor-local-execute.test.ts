...
import { execute } from "@jasminiaai/adapter-cursor-local/server";
...
describe("cursor execute", () => {
  it("injects jasminia env vars and prompt note by default", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-cursor-execute-"));
    ...
    try {
      ...
      expect(capture.argv).not.toContain("Follow the jasminia heartbeat.");
      ...
      expect(capture.prompt).toContain("Jasmin.ia runtime note:");
      expect(capture.prompt).toContain("JASMINIA_API_KEY");
  ...
  it("passes --mode when explicitly configured", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-cursor-execute-mode-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("injects company-library runtime skills into the Cursor skills home before execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-cursor-execute-runtime-skill-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "agent");
    ...
    const jasminiaDir = await createSkillDir(runtimeSkillsRoot, "jasminia");
  ...
  it("prefers ~/.local/bin/cursor-agent for remote sandbox execution when using the default command", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-cursor-sandbox-execute-"));
  ...
  it("keeps explicit command overrides for remote sandbox execution", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-cursor-sandbox-explicit-"));
    const homeDir = path.join(root, "home");
    const workspace = path.join(root, "workspace");
    const remoteWorkspace = path.join(root, "remote-workspace");
    const capturePath = path.join(root, "capture.json");
