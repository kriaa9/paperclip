...
function resolveEnvLabStatePath(): string {
  const instanceRoot =
    process.env.JASMINIA_INSTANCE_ROOT?.trim() ||
    path.join(process.env.HOME ?? "/tmp", ".jasminia-worktrees", "instances", "live-ssh-test");
  return path.join(instanceRoot, "env-lab", "ssh-fixture", "state.json");
}

/** Attempt to build config from explicit JASMINIA_ENV_LIVE_SSH_* env vars. */
function tryExplicitConfig(): {
  ...
  const host = process.env.JASMINIA_ENV_LIVE_SSH_HOST?.trim() ?? "";
  const username = process.env.JASMINIA_ENV_LIVE_SSH_USERNAME?.trim() ?? "";
  const remoteWorkspacePath =
    process.env.JASMINIA_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH?.trim() ?? "";
  const port = Number.parseInt(process.env.JASMINIA_ENV_LIVE_SSH_PORT ?? "22", 10);
 ...
 * 1. Explicit JASMINIA_ENV_LIVE_SSH_* env vars
...
async function resolveSshConfig(): Promise<SshConnectionConfig | null> {
  ...
  if (explicit) {
    return {
      ...
      privateKey: await readOptionalSecret(
        process.env.JASMINIA_ENV_LIVE_SSH_PRIVATE_KEY,
        process.env.JASMINIA_ENV_LIVE_SSH_PRIVATE_KEY_PATH,
      ),
      knownHosts: await readOptionalSecret(
        process.env.JASMINIA_ENV_LIVE_SSH_KNOWN_HOSTS,
        process.env.JASMINIA_ENV_LIVE_SSH_KNOWN_HOSTS_PATH,
      ),
      strictHostKeyChecking:
        (process.env.JASMINIA_ENV_LIVE_SSH_STRICT_HOST_KEY_CHECKING ?? "true").toLowerCase() !== "false",
    };
  }
  ...
  if (process.env.JASMINIA_ENV_LIVE_SSH_NO_AUTO_FIXTURE !== "true") {
...
const describeLiveSsh = (() => {
  ...
  if (process.env.JASMINIA_ENV_LIVE_SSH_NO_AUTO_FIXTURE === "true") {
    console.warn(
      "Skipping live SSH smoke test. Set JASMINIA_ENV_LIVE_SSH_HOST, JASMINIA_ENV_LIVE_SSH_USERNAME, and JASMINIA_ENV_LIVE_SSH_REMOTE_WORKSPACE_PATH to enable it, or remove JASMINIA_ENV_LIVE_SSH_NO_AUTO_FIXTURE to auto-start env-lab.",
    );
...
describeLiveSsh("live SSH environment smoke", () => {
  ...
  it("connects to the configured SSH environment and verifies basic runtime tools", async () => {
    ...
    if (!resolvedConfig) {
      console.warn(
        "Live SSH smoke test could not resolve SSH config from env vars or env-lab fixture. Set JASMINIA_ENV_LIVE_SSH_NO_AUTO_FIXTURE=true to mark this suite skipped intentionally.",
      );
      return;
    }
