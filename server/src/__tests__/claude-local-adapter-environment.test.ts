...
import { testEnvironment } from "@jasminiaai/adapter-claude-local/server";
...
describe("claude_local environment diagnostics", () => {
  ...
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `jasminia-claude-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
    ...
    expect(
      result.checks.some(
        (check) =>
          ...
          check.message === "Working directory is valid: /srv/jasminia/workspace",
      ),
    ).toBe(true);
