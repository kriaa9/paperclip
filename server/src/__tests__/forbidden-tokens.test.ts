...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "jasminia", LOGNAME: "jasminia", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "jasminia" }),
      },
    );

    expect(tokens).toEqual(["jasminia", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "jasminia" }, {
        userInfo: () => ({ username: "jasminia" }),
      });

      expect(tokens).toEqual(["jasminia", "custom-token"]);
    } finally {
