...
const ORIGINAL_JASMINIA_RUNTIME_API_CANDIDATES_JSON = process.env.JASMINIA_RUNTIME_API_CANDIDATES_JSON;
const ORIGINAL_JASMINIA_LISTEN_HOST = process.env.JASMINIA_LISTEN_HOST;
const ORIGINAL_JASMINIA_LISTEN_PORT = process.env.JASMINIA_LISTEN_PORT;
...
function buildTestConfig(overrides: Record<string, unknown> = {}) {
  return {
    ...
    databaseUrl: "postgres://jasminia:jasminia@127.0.0.1:5432/jasminia",
    embeddedPostgresDataDir: "/tmp/jasminia-test-db",
    embeddedPostgresPort: 54329,
    databaseBackupEnabled: false,
    databaseBackupIntervalMinutes: 60,
    databaseBackupRetentionDays: 30,
    databaseBackupDir: "/tmp/jasminia-test-backups",
    serveUi: false,
    uiDevMiddleware: false,
    secretsProvider: "local_encrypted",
    secretsStrictMode: false,
    secretsMasterKeyFilePath: "/tmp/jasminia-master.key",
    storageProvider: "local_disk",
    storageLocalDiskBaseDir: "/tmp/jasminia-storage",
    storageS3Bucket: "jasminia-test",
...
describe("startServer JASMINIA_API_URL handling", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    loadConfigMock.mockReturnValue(buildTestConfig());
    process.env.BETTER_AUTH_SECRET = "test-secret";
    delete process.env.JASMINIA_API_URL;
  });

  afterEach(() => {
    if (ORIGINAL_JASMINIA_API_URL === undefined) delete process.env.JASMINIA_API_URL;
    else process.env.JASMINIA_API_URL = ORIGINAL_JASMINIA_API_URL;

    if (ORIGINAL_JASMINIA_RUNTIME_API_URL === undefined) delete process.env.JASMINIA_RUNTIME_API_URL;
    else process.env.JASMINIA_RUNTIME_API_URL = ORIGINAL_JASMINIA_RUNTIME_API_URL;

    if (ORIGINAL_JASMINIA_RUNTIME_API_CANDIDATES_JSON === undefined) {
      delete process.env.JASMINIA_RUNTIME_API_CANDIDATES_JSON;
    } else {
      process.env.JASMINIA_RUNTIME_API_CANDIDATES_JSON = ORIGINAL_JASMINIA_RUNTIME_API_CANDIDATES_JSON;
    }

    if (ORIGINAL_JASMINIA_LISTEN_HOST === undefined) delete process.env.JASMINIA_LISTEN_HOST;
    else process.env.JASMINIA_LISTEN_HOST = ORIGINAL_JASMINIA_LISTEN_HOST;

    if (ORIGINAL_JASMINIA_LISTEN_PORT === undefined) delete process.env.JASMINIA_LISTEN_PORT;
    else process.env.JASMINIA_LISTEN_PORT = ORIGINAL_JASMINIA_LISTEN_PORT;
  });

  it("uses the externally set JASMINIA_API_URL when provided", async () => {
    process.env.JASMINIA_API_URL = "http://custom-api:3100";

    const started = await startServer();

    expect(started.apiUrl).toBe("http://custom-api:3100");
    expect(process.env.JASMINIA_API_URL).toBe("http://custom-api:3100");
    expect(JSON.parse(process.env.JASMINIA_RUNTIME_API_CANDIDATES_JSON ?? "[]")).toEqual(
      expect.arrayContaining(["http://custom-api:3100"]),
    );
    expect(JSON.parse(process.env.JASMINIA_RUNTIME_API_CANDIDATES_JSON ?? "[]")[0]).toBe("http://custom-api:3100");
  });

  it("falls back to host-based URL when JASMINIA_API_URL is not set", async () => {
    const started = await startServer();

    expect(started.apiUrl).toBe("http://127.0.0.1:3210");
    expect(process.env.JASMINIA_API_URL).toBe("http://127.0.0.1:3210");
  });
  ...
  it("rewrites explicit-port auth public URLs when detect-port selects a new port", async () => {
    ...
    expect(process.env.JASMINIA_RUNTIME_API_URL).toBe("http://my-host.ts.net:3110");
  });
  ...
  it("keeps no-port auth public URLs stable when detect-port selects a new port", async () => {
    loadConfigMock.mockReturnValueOnce(buildTestConfig({
      port: 3100,
      authBaseUrlMode: "explicit",
      authPublicBaseUrl: "https://jasminia.example",
    }));
    detectPortMock.mockResolvedValueOnce(3110);
    ...
    expect(started.listenPort).toBe(3110);
    expect(started.apiUrl).toBe("https://jasminia.example");
    expect(process.env.JASMINIA_RUNTIME_API_URL).toBe("https://jasminia.example");
  });
});
