...
import { buildProjectMentionHref, MAX_ISSUE_REQUEST_DEPTH } from "@jasminiaai/shared";
...
describeEmbeddedPostgres("issueService.list participantAgentId", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-issues-service-");
  ...
  it("hides plugin operation issues from default lists and inbox-style filters while preserving explicit retrieval", async () => {
    ...
    await db.insert(issues).values([
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:jasminia.missions:operation",
    ...
    await expect(svc.list(companyId, { originKind: "plugin:jasminia.missions:operation" }))
...
describeEmbeddedPostgres("issueService.create workspace inheritance", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-issues-create-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService blockers and dependency wake readiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-issues-blockers-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.findMentionedProjectIds", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-issues-mentioned-projects-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.clearExecutionRunIfTerminal", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-issues-execution-lock-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
