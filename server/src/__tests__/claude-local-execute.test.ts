...
const capturePath = process.env.JASMINIA_TEST_CAPTURE_PATH;
const statePath = process.env.JASMINIA_TEST_STATE_PATH;
...
describe("claude execute", () => {
  ...
  it("passes --append-system-prompt-file on a fresh session when instructionsFile is set", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-exec-fresh-"));
    ...
    try {
      await execute({
        ...
        config: {
          command: commandPath,
          cwd: workspace,
          env: { JASMINIA_TEST_CAPTURE_PATH: capturePath },
  ...
  it("omits --append-system-prompt-file on a resumed session even when instructionsFile is set", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-exec-resume-"));
    const { workspace, commandPath, capturePath, restore } = await setupExecuteEnv(root);
    const instructionsFile = path.join(root, "instructions.md");
    await fs.writeFile(instructionsFile, "# Agent instructions", "utf-8");
    try {
  ...
  it("commandNotes reports injection on a fresh session with instructionsFile", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-exec-notes-fresh-"));
  ...
  it("commandNotes is empty on a resumed session even when instructionsFile is set", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-exec-notes-resume-"));
    const { workspace, commandPath, restore } = await setupExecuteEnv(root);
    const instructionsFile = path.join(root, "instructions.md");
    await fs.writeFile(instructionsFile, "# Agent instructions", "utf-8");
  ...
  it("rebuilds the combined instructions file when an unknown resumed session falls back to fresh", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-exec-resume-fallback-"));
      ...
      const result = await execute({
        ...
        config: {
          ...
          env: {
            ...
            JASMINIA_TEST_STATE_PATH: statePath,
          },
          promptTemplate: "Do work.",
          instructionsFilePath: instructionsFile,
        },
  ...
  it("normalizes max-turn exhaustion into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-exec-max-turns-"));
    const resultEvent = {
      type: "result",
  ...
  it("does not normalize unstructured max-turn text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-exec-max-turn-text-"));
    const resultEvent = {
      type: "result",
      subtype: "error",
      session_id: "claude-session-1",
  ...
  it("does not normalize fallback stdout/stderr max-turn text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-exec-max-turn-fallback-"));
    const { workspace, commandPath, restore } = await setupExecuteEnv(root, {
  ...
  it("logs HOME, CLAUDE_CONFIG_DIR, and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-execute-meta-"));
    const workspace = path.join(root, "workspace");
    const binDir = path.join(root, "bin");
    const commandPath = path.join(binDir, "claude");
    const capturePath = path.join(root, "capture.json");
  ...
  it("injects bridge env into sandbox-managed remote runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-execute-sandbox-"));
    ...
    try {
      ...
      expect(capture.claudeConfigDir).toBe(path.join(remoteWorkspace, ".jasminia-runtime", "claude", "config"));
  ...
  it("reuses a stable Jasmin.ia-managed Claude prompt bundle across equivalent runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-execute-bundle-"));
    const workspace = path.join(root, "workspace");
    ...
    try {
      const first = await execute({
        ...
        config: {
          ...
          env: {
            JASMINIA_TEST_CAPTURE_PATH: capturePath1,
          },
          promptTemplate: "Follow the jasminia heartbeat.",
        },
        context: {},
      ...
      const second = await execute({
        ...
        config: {
          ...
          instructionsFilePath: instructionsPath,
          env: {
            JASMINIA_TEST_CAPTURE_PATH: capturePath2,
          },
          promptTemplate: "Follow the jasminia heartbeat.",
        },
        context: {
      ...
      expect(capture1.skillEntries).toContain("jasminia");
      expect(capture2.argv).toContain("--resume");
      expect(capture2.argv).toContain("claude-session-1");
      expect(capture2.prompt).toContain("## Jasmin.ia Resume Delta");
      expect(capture2.prompt).not.toContain("Follow the jasminia heartbeat.");
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
  ...
  it("starts a fresh Claude session when the stable prompt bundle changes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-execute-reset-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "claude");
    ...
    try {
      ...
      expect(after.prompt).toContain("Follow the jasminia heartbeat.");
  ...
  it("classifies Claude 'out of extra usage' failures as transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-execute-transient-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "claude");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("classifies rate-limit / overloaded failures without reset metadata as transient", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-execute-rate-limit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "claude");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingClaudeCommand(commandPath, {
  ...
  it("does not reclassify deterministic Claude failures (auth, max turns) as transient", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jasminia-claude-execute-max-turns-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "claude");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingClaudeCommand(commandPath, {
