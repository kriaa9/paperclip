...
const originalSecretsProviderEnv = process.env.JASMINIA_SECRETS_PROVIDER;
...
describeEmbeddedPostgres("routine service live-execution coalescing", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-routines-service-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    if (originalSecretsProviderEnv === undefined) {
      delete process.env.JASMINIA_SECRETS_PROVIDER;
    } else {
      process.env.JASMINIA_SECRETS_PROVIDER = originalSecretsProviderEnv;
    }
  ...
  it("interpolates routine variables into the execution issue and stores resolved values", async () => {
    ...
    expect(storedIssue?.title).toBe("repo triage for jasminia");
    expect(storedIssue?.description).toBe("Review jasminia for high bugs");
  ...
  it("uses the configured provider for generated webhook trigger secrets", async () => {
    process.env.JASMINIA_SECRETS_PROVIDER = "aws_secrets_manager";
