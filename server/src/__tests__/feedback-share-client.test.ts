...
describe("feedback trace share client", () => {
  ...
  it("defaults to telemetry.jasminia.com when no backend url is configured", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://telemetry.jasminia.com/feedback-traces",
  ...
  it("wraps the feedback trace payload as gzip+base64 json before upload", async () => {
    const client = createFeedbackTraceShareClientFromConfig({
      feedbackExportBackendUrl: "https://telemetry.jasminia.com",
    ...
    expect(call?.[0]).toBe("https://telemetry.jasminia.com/feedback-traces");
