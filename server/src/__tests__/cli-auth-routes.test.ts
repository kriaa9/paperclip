...
describe.sequential("cli auth routes", () => {
  ...
  it.sequential("rejects anonymous access to generic skill documents", async () => {
    ...
    const skillRes = await request(skillApp).get("/api/skills/jasminia");
  ...
  it.sequential("serves the invite-scoped jasminia skill anonymously for active invites", async () => {
    const invite = {
      id: "invite-1",
      companyId: "company-1",
    ...
    const res = await request(app).get("/api/invites/token-123/skills/jasminia");

    expect(res.status).toBe(200);
    expect(res.headers["content-type"]).toContain("text/markdown");
    expect(res.text).toContain("# Jasmin.ia Skill");
  });
