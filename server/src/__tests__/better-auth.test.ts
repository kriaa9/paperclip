...
const ORIGINAL_INSTANCE_ID = process.env.JASMINIA_INSTANCE_ID;

afterEach(() => {
  if (ORIGINAL_INSTANCE_ID === undefined) delete process.env.JASMINIA_INSTANCE_ID;
  else process.env.JASMINIA_INSTANCE_ID = ORIGINAL_INSTANCE_ID;
});

describe("Better Auth cookie scoping", () => {
  it("derives an instance-scoped cookie prefix", () => {
    expect(deriveAuthCookiePrefix("default")).toBe("jasminia-default");
    expect(deriveAuthCookiePrefix("PAP-1601-worktree")).toBe("jasminia-PAP-1601-worktree");
  });

  it("uses JASMINIA_INSTANCE_ID for the Better Auth cookie prefix", () => {
    process.env.JASMINIA_INSTANCE_ID = "sat-worktree";

    const advanced = buildBetterAuthAdvancedOptions({ disableSecureCookies: false });

    expect(advanced).toEqual({
      cookiePrefix: "jasminia-sat-worktree",
    });
    expect(getCookies({ advanced } as BetterAuthOptions).sessionToken.name).toBe(
      "jasminia-sat-worktree.session_token",
    );
  });

  it("keeps local http auth cookies non-secure while preserving the scoped prefix", () => {
    process.env.JASMINIA_INSTANCE_ID = "pap-worktree";

    expect(buildBetterAuthAdvancedOptions({ disableSecureCookies: true })).toEqual({
      cookiePrefix: "jasminia-pap-worktree",
