...
describeEmbeddedPostgres("secretService", () => {
  ...
  const previousKeyFile = process.env.JASMINIA_SECRETS_MASTER_KEY_FILE;
  const secretsTmpDir = path.join(os.tmpdir(), `jasminia-secrets-service-${randomUUID()}`);

  beforeAll(async () => {
    mkdirSync(secretsTmpDir, { recursive: true });
    process.env.JASMINIA_SECRETS_MASTER_KEY_FILE = path.join(secretsTmpDir, "master.key");
  ...
  afterAll(async () => {
    ...
    if (previousKeyFile === undefined) {
      ...
      process.env.JASMINIA_SECRETS_MASTER_KEY_FILE = previousKeyFile;
    }
  ...
  it("blocks coming-soon provider vaults from secret selection", async () => {
    ...
    const draftVault = await svc.createProviderConfig(companyId, {
      ...
      config: { projectId: "jasminia-prod1" },
    });
  ...
  it("passes selected provider vault config through create, rotate, and resolve", async () => {
    ...
    const createSpy = vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        ...
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company/openai-api-key",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
      fingerprintSha256: "fingerprint-sha-1",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company/openai-api-key",
  ...
  it("cleans up managed provider secrets when create persistence fails", async () => {
    ...
    const prepared = {
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company/create-rollback",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("keeps a local cleanup handle when create rollback cleanup fails", async () => {
    ...
    const prepared = {
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company/create-cleanup-handle",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("archives managed provider versions when rotate persistence fails", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company/rotate-rollback",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("keeps a disabled version cleanup handle when rotate rollback cleanup fails", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company/rotate-cleanup-handle",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("rejects generic provider vault reassignment for managed secrets", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company/vault-reassign",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("sanitizes AWS remote import preview provider errors before crossing the service boundary", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Jasmin.ia is not authorized to perform secretsmanager:ListSecrets";
  ...
  it("sanitizes AWS remote import row provider errors", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/Jasmin.ia is not authorized to perform secretsmanager:DescribeSecret on arn:aws:secretsmanager:us-east-1:123456789012:secret:prod/openai";
  ...
  it("rejects Jasmin.ia-managed AWS namespace refs during preview and import commit", async () => {
    const companyId = await seedCompany();
    const svc = secretService(db);
    const awsVault = await svc.createProviderConfig(companyId, {
      provider: "aws_secrets_manager",
    ...
    vi.spyOn(awsSecretsManagerProvider, "listRemoteSecrets").mockResolvedValue({
      secrets: [
        {
          externalRef:
            "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company-b/openai",
          name: "jasminia/prod-use1/company-b/openai",
          providerVersionRef: null,
          metadata: {
            arn: "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company-b/openai",
            description: "must not leak",
            tags: [{ Key: "jasminia:company-id", Value: "company-b" }],
          },
        },
      ],
    });
    ...
    expect(JSON.stringify(preview)).not.toContain("jasminia:company-id");
    ...
    expect(result.results[0]?.reason).toMatch(/Jasmin.ia-managed namespace/i);
  ...
  it("rejects externalRef overrides on managed secrets", async () => {
    ...
    await expect(
      svc.update(secret.id, {
        externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod/company-b/openai-api-key",
      }),
  ...
  it("passes managed AWS secret context into provider delete during removal", async () => {
    ...
    const svc = secretService(db);
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company-1/openai-api-key";
  ...
  it("renames name and key during removal before provider deletion", async () => {
    ...
    const svc = secretService(db);
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company-1/remove-failure";
    const secret = await db
      .insert(companySecrets)
      .values({
        companyId,
  ...
  it("treats missing provider secrets as already removed during removal retry", async () => {
    ...
    const svc = secretService(db);
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod-use1/company-1/retry-delete";
  ...
  it("removes DB rows even when the attached provider vault is disabled", async () => {
    ...
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod/company-1/openai-api-key";
    const secret = await db
      .insert(companySecrets)
      .values({
        companyId,
