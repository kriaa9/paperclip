...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.JASMINIA_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.JASMINIA_AGENT_JWT_SECRET === "string" ? parsed.JASMINIA_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
