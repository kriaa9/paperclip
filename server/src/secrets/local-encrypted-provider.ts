...
function resolveMasterKeyFilePath() {
  const fromEnv = process.env.JASMINIA_SECRETS_MASTER_KEY_FILE;
...
function loadOrCreateMasterKey(): Buffer {
  const envKeyRaw = process.env.JASMINIA_SECRETS_MASTER_KEY;
  if (envKeyRaw && envKeyRaw.trim().length > 0) {
    const fromEnv = decodeMasterKey(envKeyRaw);
    if (!fromEnv) {
      throw badRequest(
        "Invalid JASMINIA_SECRETS_MASTER_KEY (expected 32-byte base64, 64-char hex, or raw 32-char string)",
      );
    }
    return fromEnv;
  }
...
async function inspectLocalEncryptedHealth(): Promise<SecretProviderHealthCheck> {
  ...
  if (envKeyRaw && envKeyRaw.trim().length > 0) {
    ...
    return {
      provider: "local_encrypted",
      status: "ok",
      message: "Local encrypted provider is using JASMINIA_SECRETS_MASTER_KEY",
      backupGuidance: [
...
export const localEncryptedProvider: SecretProviderModule = {
  ...
  async deleteOrArchive() {
    // Secret metadata deletion is handled in Jasmin.ia DB; the local key is shared and must remain.
  },
  async healthCheck() {
