...
const DEFAULT_PREFIX = "jasminia";
const DEFAULT_OWNER_TAG = "jasminia";
const DEFAULT_VERSION_STAGE = "AWSCURRENT";
const JASMINIA_PENDING_VERSION_STAGE = "JASMINIA_PENDING";
...
const AWS_RUNTIME_CREDENTIAL_WARNING =
  "AWS bootstrap credentials must be available to the Jasmin.ia server runtime through the AWS SDK default credential provider chain: IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, web identity, container/instance metadata, or short-lived shell credentials.";
const AWS_CREDENTIAL_CUSTODY_WARNING =
  "Do not store AWS root credentials or long-lived IAM user access keys in Jasmin.ia company_secrets; the AWS provider bootstrap belongs in deployment infrastructure, the process environment, an AWS profile, or the orchestrator secret store.";
...
function readProviderVaultConfig(input: SecretProviderVaultRuntimeConfig): AwsSecretsManagerConfig {
  ...
  const recoveryWindowRaw = process.env.JASMINIA_SECRETS_AWS_DELETE_RECOVERY_DAYS?.trim();
  ...
  if (!Number.isFinite(recoveryWindow) || recoveryWindow < 7 || recoveryWindow > 30) {
    throw unprocessable(
      "JASMINIA_SECRETS_AWS_DELETE_RECOVERY_DAYS must be an integer between 7 and 30",
    );
  }
  ...
  return {
    region,
    endpoint:
      process.env.JASMINIA_SECRETS_AWS_ENDPOINT?.trim() ||
...
function getAwsConfigReadiness() {
  const region = (
    process.env.JASMINIA_SECRETS_AWS_REGION ??
    process.env.AWS_REGION ??
    process.env.AWS_DEFAULT_REGION
  )?.trim();
  const deploymentId = process.env.JASMINIA_SECRETS_AWS_DEPLOYMENT_ID?.trim();
  const kmsKeyId = process.env.JASMINIA_SECRETS_AWS_KMS_KEY_ID?.trim();
  const missingConfig: string[] = [];

  if (!region) {
    missingConfig.push("JASMINIA_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION");
  }
  if (!deploymentId) {
    missingConfig.push("JASMINIA_SECRETS_AWS_DEPLOYMENT_ID");
  }
  if (!kmsKeyId) {
    missingConfig.push("JASMINIA_SECRETS_AWS_KMS_KEY_ID");
  }
...
function loadAwsSecretsManagerConfig(): AwsSecretsManagerConfig {
  ...
  const region =
    process.env.JASMINIA_SECRETS_AWS_REGION?.trim() ||
  ...
  if (!region) {
    throw unprocessable(
      "AWS Secrets Manager provider requires JASMINIA_SECRETS_AWS_REGION or AWS_REGION",
    );
  }
  if (!deploymentId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires JASMINIA_SECRETS_AWS_DEPLOYMENT_ID",
    );
  }
  if (!kmsKeyId) {
    throw unprocessable(
      "AWS Secrets Manager provider requires JASMINIA_SECRETS_AWS_KMS_KEY_ID",
    );
  }
  ...
  return {
    ...
    endpoint:
      ...
      `https://secretsmanager.${region}.amazonaws.com`,
    deploymentId,
    prefix: sanitizePathSegment(process.env.JASMINIA_SECRETS_AWS_PREFIX?.trim() || DEFAULT_PREFIX),
    kmsKeyId,
    environmentTag:
      process.env.JASMINIA_SECRETS_AWS_ENVIRONMENT?.trim() ||
      process.env.NODE_ENV?.trim() ||
      "unknown",
    providerOwnerTag:
      process.env.JASMINIA_SECRETS_AWS_PROVIDER_OWNER?.trim() || DEFAULT_OWNER_TAG,
    deleteRecoveryWindowDays: recoveryWindow,
  };
}
...
function assertNotManagedNamespaceExternalRef(
  ...
  throw unprocessable(
    "AWS Jasmin.ia-managed namespace secrets cannot be imported as external references",
  );
}
...
function buildManagedSecretTags(
  ...
  if (!context) return [];
  return [
    { Key: "jasminia:managed-by", Value: "jasminia" },
    { Key: "jasminia:provider-owner", Value: config.providerOwnerTag },
    { Key: "jasminia:deployment-id", Value: config.deploymentId },
    { Key: "jasminia:company-id", Value: context.companyId },
    { Key: "jasminia:secret-key", Value: context.secretKey },
    { Key: "jasminia:environment", Value: config.environmentTag },
  ];
}
...
export function createAwsSecretsManagerProvider(
  ...
  async function validateConfig(
    ...
    if (!config.prefix) {
      warnings.push("JASMINIA_SECRETS_AWS_PREFIX should be set to a deployment-scoped prefix");
    }
  ...
  async function healthCheck(
    ...
    try {
      ...
      return {
        ...
        backupGuidance: [
          "Back up Jasmin.ia metadata separately from AWS-managed secrets.",
          "Restoring access requires the Jasmin.ia database plus the same AWS secret namespace and KMS permissions.",
        ],
      };
    } catch (error) {
      const readiness = getAwsConfigReadiness();
      ...
      return {
        ...
        details: {
          ...
          requiredProviderConfig: input?.providerConfig
            ? ["region"]
            : [
                "JASMINIA_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION",
                "JASMINIA_SECRETS_AWS_DEPLOYMENT_ID",
                "JASMINIA_SECRETS_AWS_KMS_KEY_ID",
              ],
          optionalProviderConfig: [
            "JASMINIA_SECRETS_AWS_PREFIX",
            "JASMINIA_SECRETS_AWS_ENVIRONMENT",
            "JASMINIA_SECRETS_AWS_PROVIDER_OWNER",
            "JASMINIA_SECRETS_AWS_ENDPOINT",
            "JASMINIA_SECRETS_AWS_DELETE_RECOVERY_DAYS",
          ],
          credentialSource: "AWS SDK default credential provider chain",
          detectedCredentialSources: readiness.credentialSources,
        },
    ...
    async createSecret(input) {
      ...
      try {
        const createInput = {
          ...
          Description: input.context ? `Jasmin.ia secret ${input.context.secretName}` : undefined,
    ...
    async createVersion(input) {
      ...
      try {
        const created = await gateway.putSecretValue({
          SecretId: secretId,
          SecretString: input.value,
          VersionStages: [JASMINIA_PENDING_VERSION_STAGE],
        });
        const normalizedSecretId = created.ARN ?? created.Name ?? secretId;
        return {
          material: createManagedMaterial(normalizedSecretId, created.VersionId ?? null),
        ...
        if (input.mode === "archive") {
          if (material.versionId && gateway.updateSecretVersionStage) {
            await gateway.updateSecretVersionStage({
              SecretId: secretId,
              VersionStage: JASMINIA_PENDING_VERSION_STAGE,
