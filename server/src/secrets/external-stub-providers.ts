...
function unavailableProvider(
  ...
  return {
    ...
    async createSecret() {
      throw unprocessable(`${id} provider is not configured for Jasmin.ia-managed values`);
    },
    async createVersion() {
      throw unprocessable(`${id} provider is not configured for Jasmin.ia-managed values`);
    },
    ...
    async deleteOrArchive() {
      // External references are metadata-only in Jasmin.ia for unconfigured providers.
    },
    async healthCheck() {
      return {
        provider: id,
