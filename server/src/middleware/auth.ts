...
import { agentApiKeys, agents, authUsers, companies, companyMemberships, instanceUserRoles } from "@jasminiaai/db";
...
export function actorMiddleware(db: Db, opts: ActorMiddlewareOptions): RequestHandler {
  ...
  return async (req, _res, next) => {
    ...
    const runIdHeader = req.header("x-jasminia-run-id");
...
async function resolveCloudTenantActor(db: Db, req: Request): Promise<Express.Request["actor"] | null> {
  const expectedToken = process.env.JASMINIA_CLOUD_TENANT_SERVER_TOKEN?.trim();
  if (!expectedToken) return null;

  const token = req.header("x-jasminia-cloud-tenant-token")?.trim();
  if (!token || !constantTimeStringEqual(token, expectedToken)) return null;

  const userId = requiredCloudHeader(req, "x-jasminia-cloud-user-id");
  const userEmail = requiredCloudHeader(req, "x-jasminia-cloud-user-email").toLowerCase();
  const stackId = requiredCloudHeader(req, "x-jasminia-cloud-stack-id");
  const stackRole = stackMembershipRole(req.header("x-jasminia-cloud-stack-role"));
  const userName = req.header("x-jasminia-cloud-user-name")?.trim() || userEmail;
  const jasminiaCompanyId = req.header("x-jasminia-cloud-jasminia-company-id")?.trim();
  const companyId = cloudTenantCompanyId(stackId);
  const companyName = jasminiaCompanyId || `${stackId} Jasmin.ia`;
  const now = new Date();
  ...
  await db
    ...
    .values({
      id: companyId,
      name: companyName,
      description: `Provisioned by Jasmin.ia Cloud for stack ${stackId}.`,
      status: "active",
...
function cloudTenantCompanyId(stackId: string): string {
  const bytes = createHash("sha256").update(`jasminia-cloud-tenant-company:${stackId}`).digest();
