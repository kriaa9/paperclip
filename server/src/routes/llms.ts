...
import { AGENT_ICON_NAMES } from "@jasminiaai/shared";
...
export function llmRoutes(db: Db) {
  ...
  router.get("/llms/agent-configuration.txt", async (req, res) => {
    ...
    const lines = [
      "# Jasmin.ia Agent Configuration Index",
      "",
      "Installed adapters:",
      ...
      "- Use the jasminia-create-agent skill for end-to-end hiring: adapter reflection, config comparison, instruction source selection, icon choice, desiredSkills, sourceIssueId/sourceIssueIds, and approval follow-up.",
