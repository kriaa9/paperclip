...
import { trackProjectCreated } from "@jasminiaai/shared/telemetry";
...
export function projectRoutes(db: Db) {
  ...
  async function handleProjectWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Project workspace needs a local path before Jasmin.ia can run workspace commands" });
      return;
    }
