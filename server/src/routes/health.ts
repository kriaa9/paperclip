...
function hasDevServerStatusToken(providedToken: string | undefined) {
  const expectedToken = process.env.JASMINIA_DEV_SERVER_STATUS_TOKEN?.trim();
...
export function healthRoutes(
  ...
  router.get("/", async (req, res) => {
    ...
    const exposeDevServerDetails =
      exposeFullDetails || hasDevServerStatusToken(req.get("x-jasminia-dev-server-status-token"));
