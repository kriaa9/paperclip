...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "jasminia" &&
    normalized !== "jasminia-create-agent" &&
    normalized !== "jasminia-create-plugin" &&
    normalized !== "jasminia-converting-plans-to-tasks" &&
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  if (jasminiaSkillsDir) {
    try {
      for (const entry of fs.readdirSync(jasminiaSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) jasminiaSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    jasminiaApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.jasminiaApiUrl)
      : null,
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Jasmin.ia host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Jasmin.ia is bound to loopback in authenticated/private mode.",
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = `/api/invites/${token}/skills/jasminia`;
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/jasminia-claimed-api-key.json and load JASMINIA_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      inviteMessage: extractInviteMessage(invite),
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: jasminiaApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/jasminia/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Jasmin.ia OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          jasminiaApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Jasmin.ia generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "jasminiaApiUrl": "https://jasminia-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Jasmin.ia before key claim is allowed.
    ...
    - ~/.openclaw/workspace/jasminia-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/jasminia-claimed-api-key.json

    And set the JASMINIA_API_KEY and JASMINIA_API_URL in your environment variables as specified here:
    ...
    Then set JASMINIA_API_KEY and JASMINIA_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Jasmin.ia skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your JASMINIA_API_URL to the top of your skill and note the path to your JASMINIA_API_URL
    ...
    ${
      ...
      "Ensure Jasmin.ia is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Jasmin.ia base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.jasminiaApiUrl when submitting your join request
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@jasminia.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (req, res) => {
    ...
    res.json({
      skills: [
        { name: "jasminia", path: "/api/skills/jasminia" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "jasminia-create-agent",
          path: "/api/skills/jasminia-create-agent"
        },
        {
          name: "jasminia-converting-plans-to-tasks",
          path: "/api/skills/jasminia-converting-plans-to-tasks"
        }
      ]
    });
  });
  ...
  router.get("/invites/:token/skills/index", async (req, res) => {
    ...
    res.json({
      skills: [
        {
          name: "jasminia",
          path: `/api/invites/${token}/skills/jasminia`,
        },
      ],
    });
  });
  ...
  router.get("/invites/:token/skills/:skillName", async (req, res) => {
    ...
    const skillName = (req.params.skillName as string).trim().toLowerCase();
    if (skillName !== "jasminia") throw notFound("Skill not found");
    const markdown = readSkillMarkdown(skillName);
    if (!markdown) throw notFound("Skill not found");
    res.type("text/markdown").send(markdown);
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              jasminiaApiUrl: req.body.jasminiaApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.jasminiaApiUrl &&
          !persistedDefaults.jasminiaApiUrl
        ) {
          missingPersistedFields.push("jasminiaApiUrl");
        }
