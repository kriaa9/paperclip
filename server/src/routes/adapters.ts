 ...
 * toggle server-side adapter code for the whole Jasmin.ia instance.
 *
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-jasminia-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/hermes-jasminia-adapter@0.3.0" → packageName + version
  ...
  // without modifying Jasmin.ia's source code.
  //
