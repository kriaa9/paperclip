...
import { issues, projects, projectWorkspaces } from "@jasminiaai/db";
import {
...
import type { WorkspaceRuntimeDesiredState, WorkspaceRuntimeServiceStateMap } from "@jasminiaai/shared";
...
export function executionWorkspaceRoutes(db: Db) {
  ...
  async function handleExecutionWorkspaceRuntimeCommand(req: Request, res: Response) {
    ...
    if (!workspaceCwd) {
      res.status(422).json({ error: "Execution workspace needs a local path before Jasmin.ia can run workspace commands" });
      return;
    }
      ...
      run: async () => {
        ...
        if (action === "run") {
          ...
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before Jasmin.ia can run workspace commands");
          }
        ...
        if (action === "start" || action === "restart") {
          const availableWorkspace = await ensureWorkspaceAvailable();
          if (!availableWorkspace) {
            throw new Error("Execution workspace needs a local path before Jasmin.ia can manage local runtime services");
          }
