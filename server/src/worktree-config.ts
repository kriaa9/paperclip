...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Jasmin.ia worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  const worktreeName =
    nonEmpty(persistedEnv.JASMINIA_WORKTREE_NAME) ??
    nonEmpty(env.JASMINIA_WORKTREE_NAME) ??
    path.basename(worktreeRoot);
  const instanceId =
    nonEmpty(persistedEnv.JASMINIA_INSTANCE_ID) ??
    nonEmpty(env.JASMINIA_INSTANCE_ID) ??
    sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(persistedEnv.JASMINIA_HOME) ??
      nonEmpty(env.JASMINIA_HOME) ??
      nonEmpty(env.JASMINIA_WORKTREES_DIR) ??
      "~/.jasminia-worktrees",
  );
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".jasminia", "config.json");
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.JASMINIA_HOME = context.homeDir;
  process.env.JASMINIA_INSTANCE_ID = context.instanceId;
  process.env.JASMINIA_CONFIG = context.configPath;
  process.env.JASMINIA_CONTEXT = context.contextPath;
  process.env.JASMINIA_WORKTREE_NAME = context.worktreeName;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    JASMINIA_HOME: context.homeDir,
    JASMINIA_INSTANCE_ID: context.instanceId,
    JASMINIA_CONFIG: context.configPath,
    JASMINIA_CONTEXT: context.contextPath,
    JASMINIA_IN_WORKTREE: "true",
    JASMINIA_WORKTREE_NAME: context.worktreeName,
  };
