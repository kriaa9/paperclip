...
const isSameFile = existsSync(CWD_ENV_PATH) && existsSync(JASMINIA_ENV_FILE_PATH)
  ? realpathSync(CWD_ENV_PATH) === realpathSync(JASMINIA_ENV_FILE_PATH)
  : CWD_ENV_PATH === JASMINIA_ENV_FILE_PATH;
...
function detectTailnetBindHost(): string | undefined {
  const explicit = process.env.JASMINIA_TAILNET_BIND_HOST?.trim();
...
export function loadConfig(): Config {
  ...
  const providerFromEnvRaw = process.env.JASMINIA_SECRETS_PROVIDER;
  ...
  const storageProviderFromEnvRaw = process.env.JASMINIA_STORAGE_PROVIDER;
  ...
  const storageS3Bucket = process.env.JASMINIA_STORAGE_S3_BUCKET ?? fileStorage?.s3?.bucket ?? "jasminia";
  const storageS3Region = process.env.JASMINIA_STORAGE_S3_REGION ?? fileStorage?.s3?.region ?? "us-east-1";
  const storageS3Endpoint = process.env.JASMINIA_STORAGE_S3_ENDPOINT ?? fileStorage?.s3?.endpoint ?? undefined;
  const storageS3Prefix = process.env.JASMINIA_STORAGE_S3_PREFIX ?? fileStorage?.s3?.prefix ?? "";
  const storageS3ForcePathStyle =
    process.env.JASMINIA_STORAGE_S3_FORCE_PATH_STYLE !== undefined
      ? process.env.JASMINIA_STORAGE_S3_FORCE_PATH_STYLE === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);
  const feedbackExportBackendUrl =
    process.env.JASMINIA_FEEDBACK_EXPORT_BACKEND_URL?.trim() ||
    process.env.JASMINIA_TELEMETRY_BACKEND_URL?.trim() ||
    undefined;
  const feedbackExportBackendToken =
    process.env.JASMINIA_FEEDBACK_EXPORT_BACKEND_TOKEN?.trim() ||
    process.env.JASMINIA_TELEMETRY_BACKEND_TOKEN?.trim() ||
    undefined;

  const deploymentModeFromEnvRaw = process.env.JASMINIA_DEPLOYMENT_MODE;
  const deploymentModeFromEnv =
    deploymentModeFromEnvRaw && DEPLOYMENT_MODES.includes(deploymentModeFromEnvRaw as DeploymentMode)
      ? (deploymentModeFromEnvRaw as DeploymentMode)
      : null;
  const deploymentMode: DeploymentMode = deploymentModeFromEnv ?? fileConfig?.server.deploymentMode ?? "local_trusted";
  const strictModeFromEnv = process.env.JASMINIA_SECRETS_STRICT_MODE;
  const secretsStrictMode =
    strictModeFromEnv !== undefined
      ? strictModeFromEnv === "true"
      : (fileSecrets?.strictMode ?? deploymentMode === "authenticated");
  const deploymentExposureFromEnvRaw = process.env.JASMINIA_DEPLOYMENT_EXPOSURE;
  ...
  const bindFromEnvRaw = process.env.JASMINIA_BIND;
  const bindFromEnv =
  ...
  const customBindHost = process.env.JASMINIA_BIND_HOST ?? fileConfig?.server.customBindHost;
  const authBaseUrlModeFromEnvRaw = process.env.JASMINIA_AUTH_BASE_URL_MODE;
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = process.env.JASMINIA_PUBLIC_URL;
  ...
  const disableSignUpFromEnv = process.env.JASMINIA_AUTH_DISABLE_SIGN_UP;
  const authDisableSignUp: boolean =
    disableSignUpFromEnv !== undefined
      ? disableSignUpFromEnv === "true"
      : (fileConfig?.auth?.disableSignUp ?? false);
  const allowedHostnamesFromEnvRaw = process.env.JASMINIA_ALLOWED_HOSTNAMES;
  ...
  const companyDeletionEnvRaw = process.env.JASMINIA_ENABLE_COMPANY_DELETION;
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ? companyDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    process.env.JASMINIA_DB_BACKUP_ENABLED !== undefined
      ? process.env.JASMINIA_DB_BACKUP_ENABLED === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(process.env.JASMINIA_DB_BACKUP_INTERVAL_MINUTES) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(process.env.JASMINIA_DB_BACKUP_RETENTION_DAYS) ||
      fileDatabaseBackup?.retentionDays ||
      7,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    process.env.JASMINIA_DB_BACKUP_DIR ??
  ...
  return {
    ...
    uiDevMiddleware: process.env.JASMINIA_UI_DEV_MIDDLEWARE === "true",
    secretsProvider,
    secretsStrictMode,
