...
import { resolveJasmin.iaInstanceId } from "../home-paths.js";
...
export function createBetterAuthInstance(db: Db, config: Config, trustedOrigins: string[]): BetterAuthInstance {
  const baseUrl = config.authBaseUrlMode === "explicit" ? config.authPublicBaseUrl : undefined;
  const secret = process.env.BETTER_AUTH_SECRET ?? process.env.JASMINIA_AGENT_JWT_SECRET;
  if (!secret) {
    throw new Error(
      "BETTER_AUTH_SECRET (or JASMINIA_AGENT_JWT_SECRET) must be set. " +
      "For local development, set BETTER_AUTH_SECRET=jasminia-dev-secret in your .env file.",
    );
  }
  const publicUrl = process.env.JASMINIA_PUBLIC_URL ?? baseUrl;
