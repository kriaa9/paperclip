...
Jasmin.ia supports two runtime modes:
...
Jasmin.ia now treats **bind** as a separate concern from auth:
...
When running `authenticated` mode, if the only instance admin is `local-board`, Jasmin.ia emits a startup warning with a one-time high-entropy claim URL.
