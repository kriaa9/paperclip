...
ClipHub is the public registry where people share, discover, and download Jasmin.ia company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Jasmin.ia what a package registry is to a programming language. Jasmin.ia already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Jasmin.ia — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Jasmin.ia instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Jasmin.ia spec. It contains:
...
**Install (fresh start):**
```
jasminia install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Jasmin.ia instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Jasmin.ia, export your company as a template, then publish:

```
jasminia export --template my-company
jasminia publish cliphub my-company
```
...
ClipHub is a **separate service** from Jasmin.ia itself. Jasmin.ia is self-hosted; ClipHub is a hosted registry that Jasmin.ia instances talk to.
...
| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Jasmin.ia) |
| Backend | TypeScript + Hono (consistent with Jasmin.ia) |
| Database | PostgreSQL |
...
## Relationship to Jasmin.ia

ClipHub is **not required** to use Jasmin.ia. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Jasmin.ia what a package registry is to a language runtime: optional, but transformative.
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Jasmin.ia's own philosophy)
