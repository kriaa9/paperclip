...
This document defines the product and UI plan for skill management in Jasmin.ia.
...
- Jasmin.ia company import/export can include skills as package content
...
3. The UI must always tell the truth about what Jasmin.ia knows:
   - desired state in Jasmin.ia
...
Jasmin.ia should treat skills at two scopes:
...
- created locally inside Jasmin.ia later
...
7. “Understand what Jasmin.ia knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Jasmin.ia-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Jasmin.ia did not install.

These should be clearly marked:

- external
- not managed by Jasmin.ia
...
- `desired_only`: Jasmin.ia wants it, adapter does not show it yet
- `external`: adapter has it but Jasmin.ia does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Jasmin.ia only tracks desired state
...
- Jasmin.ia can still track desired skills
...
- Jasmin.ia can see adapter skills
...
- `.jasminia.yaml` may add Jasmin.ia-specific fidelity, but should not replace the base shortname association model
...
- `.jasminia.yaml` may add Jasmin.ia-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Jasmin.ia should have a company-scoped skill model or managed package model representing:
...
Jasmin.ia should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Jasmin.ia-specific.
...
That gives Jasmin.ia one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
