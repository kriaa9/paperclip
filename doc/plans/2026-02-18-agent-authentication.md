...
Agents need API keys to authenticate with Jasmin.ia. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Jasmin.ia
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Jasmin.ia generates a token and passes it directly to the agent
...
`JASMINIA_API_URL`, `JASMINIA_AGENT_ID`, `JASMINIA_COMPANY_ID`. We need to
add a `JASMINIA_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Jasmin.ia CLI
...
```
jasminia auth login
# Opens browser -> user confirms -> token stored at ~/.jasminia/credentials
```
...
managed by the Jasmin.ia server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Jasmin.ia.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Jasmin.ia SKILL.md (or a link to it)
   - What information Jasmin.ia needs from the agent (e.g. webhook URL, adapter
...
5. Jasmin.ia stores the pending registration.
...
7. On approval, Jasmin.ia provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Jasmin.ia issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.jasminia.com/skills/jasminia/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Jasmin.ia should send you heartbeats",
...
1. Generate a Jasmin.ia invite link for the company.
...
4. A Jasmin.ia company member approves the new agent.
5. Jasmin.ia begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `JASMINIA_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `jasminiaai auth login` for developer-managed remote agents.                                      |
