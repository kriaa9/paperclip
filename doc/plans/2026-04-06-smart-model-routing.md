...
This document defines a V1 plan for "smart model routing" in Jasmin.ia.
...
- preserve Jasmin.ia's existing task, session, and audit invariants
...
That separation is a better fit for Jasmin.ia than copying Hermes' exact keyword heuristic.

## 3. Current Jasmin.ia State

Jasmin.ia already has the right execution shape for adapter-specific routing, but it currently assumes one model per heartbeat run.

Current implementation facts:

- `server/src/services/heartbeat.ts` builds rich run context, including `jasminiaWake`, workspace metadata, and session handoff context
...
Jasmin.ia should implement smart model routing as an adapter-local, opt-in execution pattern.

V1 decision:

1. Do not add a global server-side router that tries to understand every adapter.
2. Do not copy Hermes' prompt-keyword classifier as Jasmin.ia's default routing policy.
...
- Jasmin.ia heartbeats are structured, issue-scoped, and already include wake metadata
...
- the adapter already injects Jasmin.ia skills and workspace metadata cleanly
...
This matters because Jasmin.ia's board UI is supposed to make cost and behavior legible.

## 9. Why Not Copy Hermes Exactly

Hermes' cheap-route heuristic is useful precedent, but Jasmin.ia should not start there.

Reasons:

- Hermes is optimizing free-form conversational turns
- Jasmin.ia agents run structured, issue-scoped heartbeats with explicit task and workspace context
- Jasmin.ia already knows whether a run is fresh vs resumed, issue-scoped vs approval follow-up, and what workspace/session exists
- those execution facts are stronger routing signals than prompt keyword matching

If Jasmin.ia later wants a cheap-only completion path for trivial runs, that can be a second-stage feature built on observed run data, not the first implementation.
...
Jasmin.ia should ship smart model routing as:
