...
- local mode that still feels instant (`npx jasminia run` and go)
...
2. CLI command `pnpm jasminia auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm jasminia onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm jasminia onboard`).
...
- expose key instance settings in API + CLI (`jasminia configure` / `jasminia onboard`)
...
7. `pnpm jasminia onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
