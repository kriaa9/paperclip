...
- local mode that still feels instant (`npx jasminiaai run` and go)
...
2. CLI command `pnpm jasminiaai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm jasminiaai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm jasminiaai onboard`).
...
- expose key instance settings in API + CLI (`jasminiaai configure` / `jasminiaai onboard`)
...
7. `pnpm jasminiaai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
