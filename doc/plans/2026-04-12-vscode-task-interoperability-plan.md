...
Jasmin.ia should not replace its workspace runtime service model with VS Code tasks.
...
- Jasmin.ia owns long-running workspace services and their desired state
- Jasmin.ia shows operators exactly which named thing they are starting or stopping
- Jasmin.ia distinguishes long-running services from one-shot jobs
...
This is directionally correct for Jasmin.ia because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Jasmin.ia does not yet leverage existing workspace metadata.

## Recommendation

Keep Jasmin.ia runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Jasmin.ia.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Jasmin.ia service or job semantics should become runnable in Jasmin.ia.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Jasmin.ia runtime services.
...
That creates a bad fit if Jasmin.ia tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Jasmin.ia's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Jasmin.ia should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Jasmin.ia can resolve
...
- selected variable substitution when Jasmin.ia can resolve it safely from workspace context
- mapping task metadata into Jasmin.ia readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Jasmin.ia cannot execute directly
...
Jasmin.ia needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Jasmin.ia metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Jasmin.ia
...
For a workspace with `cwd`, Jasmin.ia should look for `.vscode/tasks.json`.
...
Jasmin.ia should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Jasmin.ia metadata marks it as a service, or
...
Jasmin.ia-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Jasmin.ia would import and why.
...
Goal:
run supported imported service commands through the current Jasmin.ia supervisor.
...
Success condition:
imported service commands behave like native Jasmin.ia services once adopted.
...
- removal of Jasmin.ia's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Jasmin.ia to VS Code and making the model worse for non-VS-Code repos
...
Jasmin.ia should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Jasmin.ia workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
