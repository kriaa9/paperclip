...
Yes, there is a likely root cause in Jasmin.ia's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Jasmin.ia can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Jasmin.ia already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Jasmin.ia may think it cancelled the run because the parent process is gone
...
This work is successful when Jasmin.ia can:
...
- require every skill author to add bespoke browser teardown logic before Jasmin.ia can clean up correctly
...
- make the leak measurable from Jasmin.ia's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Jasmin.ia
...
Treat this as a Jasmin.ia executor ownership bug, not an `agent-browser` bug.
