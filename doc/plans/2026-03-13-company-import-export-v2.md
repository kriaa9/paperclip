...
This document defines the next-stage plan for Jasmin.ia company import/export.

The core shift is:

- move from a Jasmin.ia-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Jasmin.ia.
...
Jasmin.ia already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Jasmin.ia
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `jasminia.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.jasminia.yaml` sidecar for high-fidelity Jasmin.ia-specific details
...
11. adapter-aware skill sync surfaces so Jasmin.ia can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Jasmin.ia at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Jasmin.ia sidecar.
...
Jasmin.ia must not redefine `SKILL.md`.
...
- Jasmin.ia-specific fidelity lives in `.jasminia.yaml`
- Jasmin.ia may resolve and install `SKILL.md` packages, but it must not require a Jasmin.ia-only skill format
...
### 5.4 Base Package Vs Jasmin.ia Extension
...
- base package:
  ...
  - no Jasmin.ia-only runtime fields by default
- Jasmin.ia extension:
  - `.jasminia.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Jasmin.ia tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`jasminia.manifest.json` is not part of the future package direction.
...
Jasmin.ia import/export should support these entity kinds:
...
- Jasmin.ia can treat “this set of agents came from team package X” as the imported-team model
...
Jasmin.ia should support source references in package metadata with:
...
Jasmin.ia should define a new adapter surface area around skills:
...
Planned Jasmin.ia behavior:

- if an adapter supports read, Jasmin.ia should show current skills in the UI
- if an adapter supports write, Jasmin.ia should let the user enable/disable imported skills
- if an adapter supports sync, Jasmin.ia should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.jasminia.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.jasminia.yaml` alongside the base package
...
## 10. Storage Model Inside Jasmin.ia
...
Jasmin.ia should add managed package/provenance records so imports are not anonymous one-off copies.
...
1. Should imported skill packages be stored as managed package files in Jasmin.ia storage, or only referenced at import time?
...
3. Should Jasmin.ia support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Jasmin.ia aligned with:
