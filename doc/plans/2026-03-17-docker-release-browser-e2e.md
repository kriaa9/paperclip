...
Today release smoke testing for published Jasmin.ia packages is manual and shell-driven:
...
- Docker
- `npx jasminiaai@canary`
- `npx jasminiaai@latest`
...
- runs `jasminiaai@${JASMINIAAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@jasminia.local`
- password: `jasminia-smoke-password`
...
- the suite passes locally against both:
  - `JASMINIAAI_VERSION=canary`
  - `JASMINIAAI_VERSION=latest`
...
1. A published `jasminiaai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `jasminiaai@latest` Docker install can be smoke-tested by Playwright in CI.
