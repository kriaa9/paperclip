...
- runs `jasminia@${JASMINIAAI_VERSION}` inside Docker
- waits for health
...
1. A published `jasminia@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `jasminia@latest` Docker install can be smoke-tested by Playwright in CI.
