...
Jasmin.ia needs to support two very different but equally valid ways of working:
...
Today, Jasmin.ia already has the beginnings of this model:
...
- making Jasmin.ia feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Jasmin.ia users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Jasmin.ia deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Jasmin.ia into a full code review product
...
This object must be recorded explicitly so that Jasmin.ia can:
...
Jasmin.ia should treat PRs as a type of work product linked back to:
...
Jasmin.ia already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Jasmin.ia must be a first-class deployment mode

Jasmin.ia cannot assume that it is running on the same machine as the code.

In cloud deployments, Jasmin.ia may:
...
- `Runtime service`: a process or service Jasmin.ia owns or tracks for a workspace
...
It also lets cloud-hosted Jasmin.ia deployments point at codebases and remotes without pretending the Jasmin.ia host has direct filesystem access.
...
Without an explicit `execution workspace` record, Jasmin.ia has nowhere stable to attach:
...
Jasmin.ia needs a single place to show:
...
- Jasmin.ia-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Jasmin.ia owning the execution host
...
## 1. Jasmin.ia host and execution host must be decoupled
...
without the Jasmin.ia host itself running local git or local preview processes.
...
Jasmin.ia should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Jasmin.ia.
...
Jasmin.ia should keep workspace automation centered on a small set of hooks:
...
Jasmin.ia should provide the equivalent visibility in the project `Code` surface:
...
Jasmin.ia should treat execution workspaces as the natural home for:
...
Jasmin.ia should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Jasmin.ia should continue treating cleanup policy as part of the core workspace model:
...
Jasmin.ia should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Jasmin.ia host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Jasmin.ia should borrow Worktrunk's good execution patterns while keeping the broader Jasmin.ia model:
...
This is how Jasmin.ia supports:
...
That keeps Jasmin.ia focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Jasmin.ia deployments remain first-class

That is a better fit for Jasmin.ia than either extreme:
