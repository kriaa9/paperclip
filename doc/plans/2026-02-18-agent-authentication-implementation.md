...
- New config/env settings:
  - `JASMINIA_AGENT_JWT_SECRET`
  - `JASMINIA_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `JASMINIA_AGENT_JWT_ISSUER` (default: `jasminia`)
  - `JASMINIA_AGENT_JWT_AUDIENCE` (default: `jasminia-api`)
...
1. In:
   ...
   inject `JASMINIA_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `JASMINIA_API_KEY`, do not overwrite it.
- Continue injecting:
  - `JASMINIA_AGENT_ID`
  - `JASMINIA_COMPANY_ID`
  - `JASMINIA_API_URL`
...
- Local adapters authenticate without manual `JASMINIA_API_KEY` config.
