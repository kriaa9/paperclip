...
This document defines the V1 design for a Jasmin.ia adapter that integrates with
...
4. synthesized stdout events for Jasmin.ia UI/CLI
...
3. **Agent callback auth to Jasmin.ia** uses a bootstrap exchange flow (no long-lived Jasmin.ia key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Jasmin.ia endpoints, not full SKILL.md prompt injection.
...
- `jasminiaPublicUrl` (optional override; else `JASMINIA_PUBLIC_URL` env)
...
## Jasmin.ia Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `JASMINIA_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.jasminiaPublicUrl`
2. `process.env.JASMINIA_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Jasmin.ia credentials in prompt text.

Flow:

1. Before launch/follow-up, Jasmin.ia mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Jasmin.ia validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Jasmin.ia API calls.
...
1. Prompt includes a compact instruction to fetch skills from Jasmin.ia.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/jasminia`
   - `GET /api/skills/jasminia-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <jasminiaPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Jasmin.ia run context.
...
5. `jasminiaPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Jasmin.ia skill endpoints |
| Jasmin.ia auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Jasmin.ia context.
3. Add richer PR metadata surfacing in Jasmin.ia UI.
