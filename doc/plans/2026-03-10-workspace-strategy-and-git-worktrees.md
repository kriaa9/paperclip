...
`PAP-447` asks how Jasmin.ia should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Jasmin.ia instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Jasmin.ia:

- some operators use Jasmin.ia to manage Jasmin.ia and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Jasmin.ia
- Claude and Codex expose different built-in affordances, so Jasmin.ia should not overfit to one tool

## Core Product Decision

Jasmin.ia should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Jasmin.ia
...
- Jasmin.ia should not assume that every adapter can see or use a host filesystem path directly
...
If Jasmin.ia makes worktrees agent-first, it will blur:
...
- Jasmin.ia core resolves and records execution workspace state
...
That gives Jasmin.ia room to support:
...
- Jasmin.ia handles workspace runtime services under the hood using project/workspace policy
...
Once Jasmin.ia is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Jasmin.ia should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Jasmin.ia cannot infer
...
- when issue moves to done, should Jasmin.ia auto-commit?
- should it auto-push?
...
Jasmin.ia already has the right foundation for a project-first model.
...
For server-spawned local adapters, Jasmin.ia already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Jasmin.ia should mirror directly

Implication:

- for `codex_local`, Jasmin.ia should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Jasmin.ia should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Jasmin.ia should separate:
...
Jasmin.ia should be able to express intentions such as:
...
These are not specific to Jasmin.ia. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".jasminia/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Jasmin.ia-specific `devServer` flag.
...
Jasmin.ia should distinguish between:
...
Jasmin.ia should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Jasmin.ia should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Jasmin.ia
...
For runtime services specifically, Jasmin.ia should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Jasmin.ia-only dev-server flags
...
3. Inject resolved execution workspace details into `context.jasminiaWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- Jasmin.ia can decide whether to reuse or start a fresh service deterministically
...
3. Test against Jasmin.ia-on-Jasmin.ia workflows first.
...
7. Workspace runtime services are modeled generically, not as Jasmin.ia-specific dev-server toggles.
