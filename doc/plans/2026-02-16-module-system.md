...
Jasmin.ia's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Jasmin.ia's API, UI, and data model | Yes |
...
modules/
  observability/
    jasminia.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@jasminiaai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@jasminiaai/core";
...
```
1. Scan modules/ directory for jasminia.module.json manifests
...
```jsonc
// jasminia.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@jasminiaai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@jasminiaai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:jasminia/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.jasminia.com/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm jasminiaai store list                    # browse available modules and templates
pnpm jasminiaai store install <module-id>     # install a module
pnpm jasminiaai store import <template-id>    # import a company template
pnpm jasminiaai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Jasmin.ia instance | `server:started` |
...
4. **Module config** — `jasminia.config.json` with per-module config, env var interpolation
...
10. **`@jasminiaai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
