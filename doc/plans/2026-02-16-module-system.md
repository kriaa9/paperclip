...
- **`id`**: Unique identifier, used as the npm package name suffix (`@jasminia/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@jasminia/core";
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@jasminia/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@jasminia/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
...
```bash
pnpm jasminia store list                    # browse available modules and templates
pnpm jasminia store install <module-id>     # install a module
pnpm jasminia store import <template-id>    # import a company template
pnpm jasminia store export                  # export current company as template
```
...
10. **`@jasminia/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
