...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/jasminia-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `JASMINIA_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `JASMINIA_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Jasmin.ia may create host-local worktrees, processes, and paths
- `cloud`
  - Jasmin.ia should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `jasminia | github | vercel | s3 | custom`
...
- use `JASMINIA_EXECUTION_TOPOLOGY` as a defaulting guardrail
