...
- Team of 3 can use one shared Jasmin.ia instance.
...
Jasmin.ia should manage the **issue → workspace → preview/PR → review handoff** lifecycle, but leave diffs/review/merge to external tools.
...
execution:
  ...
  preview:
    command: "pnpm dev --port $JASMINIA_PREVIEW_PORT"
    healthPath: "/"
    ttlMinutes: 120
  vcs:
    provider: github
...
- A chat plugin can converse without requiring chat to become the core Jasmin.ia product.
...
- **Keep** Jasmin.ia as the board-level control plane.
