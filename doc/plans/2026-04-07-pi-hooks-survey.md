...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Jasmin.ia.
...
- current Jasmin.ia plugin and adapter surfaces in this repo
...
## What Jasmin.ia already has

Jasmin.ia already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Jasmin.ia

These ideas from `pi-mono` fit Jasmin.ia with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Jasmin.ia should continue exposing run and transcript events to plugins, for example:
...
This matches Jasmin.ia's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Jasmin.ia already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Jasmin.ia already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Jasmin.ia, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Jasmin.ia core:

### 1. Arbitrary mutation hooks on control-plane decisions

Jasmin.ia should not let general plugins rewrite:
...
Jasmin.ia should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Jasmin.ia is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Jasmin.ia's board-and-issues model.

## Recommended Jasmin.ia direction
...
If the question is "what works for Jasmin.ia too?":
