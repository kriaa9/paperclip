...
Turn the `agent-os` research into a low-risk Jasmin.ia execution plan that preserves Jasmin.ia's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Jasmin.ia should not absorb `agent-os` as a product model or orchestration layer.

Jasmin.ia should evaluate `agent-os` in three narrow areas:
...
This work is successful when Jasmin.ia has:
...
- a proposed runtime capability model that fits current Jasmin.ia adapters
...
- replace Jasmin.ia heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Jasmin.ia core

## Existing Jasmin.ia integration points
...
- prove that Jasmin.ia can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Jasmin.ia's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Jasmin.ia-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Jasmin.ia workloads
...
- identify where Jasmin.ia should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Jasmin.ia
...
- runtime experimentation could blur the boundary between Jasmin.ia as control plane and Jasmin.ia as execution platform
