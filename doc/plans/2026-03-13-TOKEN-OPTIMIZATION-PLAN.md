...
- local runtime data from the default Jasmin.ia instance on 2026-03-13
...
3. **Repeated context reacquisition.** The `jasminia` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `jasminia` skill currently tells agents to do this on essentially every heartbeat:
...
- Jasmin.ia discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing Jasmin.ia skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after Jasmin.ia-side skill changes land.
...
- `skills/jasminia/SKILL.md`: 17,441 bytes
- `.agents/skills/create-agent-adapter/SKILL.md`: 31,832 bytes
- `skills/jasminia-create-agent/SKILL.md`: 4,718 bytes
...
- Store both:
  ...
  - Jasmin.ia-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching Jasmin.ia to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
Update the `jasminia` skill so the default pattern becomes:
...
- For `codex_local`, either:
  ...
  - treat Jasmin.ia-owned Codex skill symlinks as repairable when they point at a different checkout
...
4. heartbeat delta APIs + `jasminia` skill rewrite
...
6. Rewrite `skills/jasminia/SKILL.md` around delta-fetch behavior.
