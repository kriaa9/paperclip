...
This spec defines how Jasmin.ia actually runs agents while staying runtime-agnostic.
...
1. Jasmin.ia is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Jasmin.ia manages lifecycle, and on exit Jasmin.ia parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `jasminia.skill` (shared Jasmin.ia skill text block)
...
2. Preferred transport is env vars (`JASMINIA_*`) injected at runtime.
