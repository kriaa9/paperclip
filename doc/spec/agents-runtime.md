...
For `claude_local` and `codex_local`, Jasmin.ia assumes the CLI is already installed and authenticated on the host machine.
...
Jasmin.ia stores resumable session state per `(agent, taskKey, adapterType)`.
