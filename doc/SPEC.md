# Jasmin.ia Specification

Target specification for the Jasmin.ia control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One Jasmin.ia instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the Jasmin.ia protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

Jasmin.ia doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### Jasmin.ia Protocol (What Jasmin.ia Knows)

At the protocol level, Jasmin.ia tracks:
...
- **Fat payload** — Jasmin.ia bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to Jasmin.ia.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls Jasmin.ia's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a Jasmin.ia agent: **be callable.** That's it. Jasmin.ia can invoke you via command or webhook. No requirement to report back — Jasmin.ia infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, Jasmin.ia provides progressively richer integration:

1. **Callable** (minimum) — Jasmin.ia can start you. That's the only contract.
...
Jasmin.ia ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **Jasmin.ia skill** (a Claude Code skill for interacting with the Jasmin.ia API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the Jasmin.ia SKILL.md which is recommended for all agents. Jasmin.ia the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. Jasmin.ia defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how Jasmin.ia invokes the agent. Built-in adapters include:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets Jasmin.ia check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the Jasmin.ia REST API.

### What Jasmin.ia Controls
...
### What Jasmin.ia Does NOT Control
...
- Health monitoring — how does Jasmin.ia distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of Jasmin.ia. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to Jasmin.ia. Costs are tracked at every level:
...
Jasmin.ia ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **Jasmin.ia Skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
### Jasmin.ia Skill (SKILL.md)

A skill definition that teaches agents how to interact with Jasmin.ia. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my Jasmin.ia instance."

#### Agent Authentication

When a user creates an Agent, Jasmin.ia generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. Jasmin.ia generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), Jasmin.ia sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the Jasmin.ia SKILL, but sometimes it's unavoidable.
...
Jasmin.ia manages task-linked work artifacts: issue documents (rich-text plans, specs, notes attached to issues) and file attachments. Agents read and write these through the API as part of normal task execution. Full delivery infrastructure (code repos, deployments, production runtime) remains the agent's domain — Jasmin.ia orchestrates the work, not the build pipeline.
...
When an agent crashes or disappears mid-task, Jasmin.ia does **not** auto-reassign or auto-release the task. Instead:

- Jasmin.ia surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- Jasmin.ia does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: Jasmin.ia reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core Jasmin.ia system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
The plugin framework has shipped. Plugins can register new adapter types, hook into lifecycle events, and contribute UI components (e.g. global toolbar buttons). A plugin SDK and CLI commands (`jasminiaai plugin`) are available for authoring and installing plugins.
...
**Full loop with one adapter.** V1 must demonstrate the complete Jasmin.ia cycle end-to-end, even if narrow.
...
- [ ] **Default agent** — basic Claude Code/Codex loop with Jasmin.ia skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **Jasmin.ia skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the Jasmin.ia core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things Jasmin.ia explicitly does **not** do:

- **Not an Agent runtime** — Jasmin.ia orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. Jasmin.ia is the control plane, not the execution plane.
