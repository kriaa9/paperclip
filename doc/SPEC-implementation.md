# Jasmin.ia V1 Implementation Spec
...
Jasmin.ia V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.jasminia/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.jasminia/instances/default/data/storage` (`local_disk`)
...
- `runtime_config` jsonb not null default `{}`; may include Jasmin.ia runtime policy such as `modelProfiles.cheap.adapterConfig` for an optional low-cost model lane that does not change the primary adapter config
...
- project env is merged into run environment for issues in that project and overrides conflicting agent env keys before Jasmin.ia runtime-owned keys are injected
...
- when Jasmin.ia cannot safely infer the next action, it surfaces the problem through visible blocked/recovery work instead of silently completing or reassigning work
...
- if unset, auto-use embedded PostgreSQL under `~/.jasminia/instances/default/db`
...
- `.jasminia.yaml` sidecar for Jasmin.ia-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.jasminia.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and Jasmin.ia routine fidelity in `.jasminia.yaml`
- Jasmin.ia imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.jasminia.yaml`
