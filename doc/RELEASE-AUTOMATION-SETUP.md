...
This document covers the GitHub and npm setup required for the current Jasmin.ia release model:
...
Do this for every public package that Jasmin.ia publishes.
...
3. add a trusted publisher for the GitHub repository `jasminiaai/jasminia`
...
2. open that package on npm and add the `jasminiaai/jasminia` trusted publisher for `.github/workflows/release.yml`
...
2. revoke any personal automation token that used to publish Jasmin.ia
