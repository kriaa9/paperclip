...
Jasmin.ia uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
1. Creates a `~/.jasminia/instances/default/db/` directory for storage
2. Ensures the `jasminia` database exists
3. Runs migrations automatically for empty databases
4. Starts serving requests

Data persists across restarts in `~/.jasminia/instances/default/db/`. To reset local dev data, delete that directory.
...
When `DATABASE_URL` is unset, this command targets the current embedded PostgreSQL instance for your active Jasmin.ia config/instance.
...
Docker note: the Docker quickstart image also uses embedded PostgreSQL by default. Persist `/jasminia` to keep DB state across container restarts (see `doc/DOCKER.md`).
...
# DATABASE_URL=postgres://jasminia:jasminia@localhost:5432/jasminia
```
...
If you later run the app with a pooled runtime URL, set `DATABASE_MIGRATION_URL` to the direct connection URL. Jasmin.ia uses it for startup schema checks/migrations and plugin namespace migrations, while the app continues to use `DATABASE_URL` for runtime queries:
...
If your hosted database requires transaction-pooling-only connections, use a direct or session-pooled connection for Jasmin.ia until runtime pooling support is documented in this guide. Do not edit database client source files as part of deployment setup.
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.jasminia/instances/default/db/`) |
...
Jasmin.ia supports automatic and manual logical database backups. These dumps include
...
`jasminiaai db:backup` / `pnpm db:backup` commands and backup retention
configuration.
...
Jasmin.ia stores secret metadata and versions in:
...
- Default key file: `~/.jasminia/instances/default/secrets/master.key` (auto-created if missing).
- CLI config location: `~/.jasminia/instances/default/config.json` under `secrets.localEncrypted.keyFilePath`.
...
- `JASMINIA_SECRETS_MASTER_KEY` (32-byte key as base64, hex, or raw 32-char string)
