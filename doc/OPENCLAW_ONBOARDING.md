...
1. Start Jasmin.ia in auth mode.
```bash
cd <jasminia-repo-root>
pnpm dev --bind lan
```
Then verify:
```bash
...
3. In Jasmin.ia UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Jasmin.ia UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Jasmin.ia invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Jasmin.ia with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Jasmin.ia UI that new issue exists.

10. Watch logs during test (optional but helpful):
```bash
docker compose -f /tmp/openclaw-docker/docker-compose.yml -f /tmp/openclaw-docker/.jasminia-openclaw.override.yml logs -f openclaw-gateway
```
