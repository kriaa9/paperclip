...
Jasmin.ia V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Jasmin.ia-shaped design.

Assumption for this document: Jasmin.ia is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Jasmin.ia objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Jasmin.ia.

The main conclusion is:

- Jasmin.ia should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Jasmin.ia should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Jasmin.ia should use multiple extension classes instead of one generic plugin bag:
...
If Jasmin.ia does this well, the examples you listed become straightforward:
...
Relevant Jasmin.ia files reviewed for current extension seams:
...
It is too dangerous for Jasmin.ia core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Jasmin.ia — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Jasmin.ia should absolutely do this.
...
Jasmin.ia should copy this discipline.
...
Jasmin.ia should adopt this style for plugin actions, automations, and UI schemas.
...
Jasmin.ia can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Jasmin.ia too.

## What Jasmin.ia Should Not Copy Directly
...
Jasmin.ia is a control plane for an operator-managed instance with company objects.
...
Jasmin.ia is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Jasmin.ia should not auto-load arbitrary code from a workspace repo like `.jasminia/plugins` or project directories.
...
For Jasmin.ia, equivalent hooks into:
...
Jasmin.ia should not allow plugins to silently replace:
...
For Jasmin.ia it would be risky because it combines:
...
Jasmin.ia should require an explicit operator install step.

## Why Jasmin.ia Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Jasmin.ia |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Jasmin.ia instance |
...
That means Jasmin.ia should borrow the good ideas from `opencode` but use a stricter architecture.

## Jasmin.ia Already Has Useful Pre-Plugin Seams

Jasmin.ia has several extension-like seams already:
...
This is good news.
Jasmin.ia does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Jasmin.ia Plugin Model
...
Jasmin.ia already has this pattern implicitly:
...
- `plugin` means integration code that talks to Jasmin.ia through a typed plugin protocol and capability model
...
- invoke explicit Jasmin.ia actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@jasminiaai/plugin-sdk/ui`) with design tokens and common components.
...
Jasmin.ia should treat plugin installation as a global instance-level action.

Examples:

- install `@jasminia/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Jasmin.ia objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Jasmin.ia already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Jasmin.ia too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@jasminia/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@jasminiaai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Jasmin.ia instance.

A `create-jasminia-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Jasmin.ia achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Jasmin.ia server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Jasmin.ia's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@jasminiaai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@jasminiaai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Jasmin.ia

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@jasminiaai/plugin-sdk";

export default definePlugin({
  id: "@jasminia/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Jasmin.ia entities
    });
    ...
    ctx.events.on("plugin.@jasminia/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@jasminiaai/plugin-sdk/ui";
...
Any Jasmin.ia plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Jasmin.ia objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@jasminia/plugin-linear`)
...
- reuse core tables when the data is clearly part of Jasmin.ia itself
...
If a concept is becoming part of Jasmin.ia's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Jasmin.ia itself
...
### 3. Scope plugin data to Jasmin.ia objects before adding custom schemas

A lot of plugin data naturally hangs off existing Jasmin.ia objects:
...
If Jasmin.ia eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@jasminia/plugin-workspace-files`
...
Package idea: `@jasminia/plugin-terminal`
...
Package idea: `@jasminia/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Jasmin.ia.
- Board reviews the diff after a run without leaving Jasmin.ia.
...
- optional `events.emit` (e.g. `plugin.@jasminia/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@jasminia/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@jasminia/plugin-linear`

This plugin syncs Jasmin.ia work with Linear. It is useful for:

- importing backlog from Linear
- linking Jasmin.ia issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Jasmin.ia and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Jasmin.ia.
- Jasmin.ia issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@jasminia/plugin-github-issues`

This plugin syncs Jasmin.ia issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Jasmin.ia issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Jasmin.ia task governance
...
- Plugin settings:
  ...
  - whether new Jasmin.ia issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Jasmin.ia issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Jasmin.ia.
- GitHub webhooks update status/comment state in Jasmin.ia.
- A PR is linked back to the Jasmin.ia issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@jasminia/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@jasminia/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Jasmin.ia. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Jasmin.ia issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Jasmin.ia plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Jasmin.ia dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Jasmin.ia issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Jasmin.ia core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@jasminia/plugin-runtime-processes`
...
Package idea: `@jasminia/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Jasmin.ia. It is useful for:
...
- Failed payment spikes or churn events can generate Jasmin.ia issues for follow-up.
...
- Stripe data should stay additive to Jasmin.ia core
...
- plugin UI bundle loading, host bridge, `@jasminiaai/plugin-sdk/ui`
...
- `@jasminiaai/plugin-test-harness` and `create-jasminia-plugin` starter template
...
Jasmin.ia should not implement "an OpenCode-style generic in-process hook system."
Jasmin.ia should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Jasmin.ia
...
8. Ship `@jasminiaai/plugin-test-harness` and `create-jasminia-plugin` starter template.
