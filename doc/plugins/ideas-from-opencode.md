...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@jasminia/plugin-sdk/ui`) with design tokens and common components.
...
A `@jasminia/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Jasmin.ia instance.
...
- **Single SDK package**: `@jasminia/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@jasminia/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
```ts
import { definePlugin, z } from "@jasminia/plugin-sdk";
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@jasminia/plugin-sdk/ui";
...
- plugin UI bundle loading, host bridge, `@jasminia/plugin-sdk/ui`
...
- `@jasminia/plugin-test-harness` and `create-jasminia-plugin` starter template
...
8. Ship `@jasminia/plugin-test-harness` and `create-jasminia-plugin` starter template.
