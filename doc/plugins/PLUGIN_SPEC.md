# Jasmin.ia Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Jasmin.ia's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Jasmin.ia app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Jasmin.ia HTTP APIs directly.
...
- The current runtime ships a small host-provided plugin UI component kit through `@jasminiaai/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
Jasmin.ia plugin design is based on the following assumptions:

1. Jasmin.ia is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Jasmin.ia business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Jasmin.ia core.
...
2. Let plugins add major capabilities without editing Jasmin.ia core.
...
4. Depend on project-local plugin folders such as `.jasminia/plugins`.
...
The single Jasmin.ia deployment an operator installs and controls.

### 5.2 Company

A first-class Jasmin.ia business object inside the instance.
...
A trusted in-process extension loaded directly by Jasmin.ia core.
...
An installable instance-wide extension package loaded through the Jasmin.ia plugin runtime.
...
Jasmin.ia has two extension classes.
...
Jasmin.ia already has a concrete workspace model:
...
Plugins live under the Jasmin.ia instance directory.

Suggested layout:

- `~/.jasminia/instances/default/plugins/package.json`
- `~/.jasminia/instances/default/plugins/node_modules/`
- `~/.jasminia/instances/default/plugins/.cache/`
- `~/.jasminia/instances/default/data/plugins/<plugin-id>/`
...
Jasmin.ia should add CLI commands:

- `pnpm jasminiaai plugin list`
- `pnpm jasminiaai plugin install <package[@version]>`
- `pnpm jasminiaai plugin uninstall <plugin-id>`
- `pnpm jasminiaai plugin upgrade <plugin-id> [version]`
- `pnpm jasminiaai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@jasminia/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
export interface Jasmin.iaPluginManifestV1 {
  ...
  minimumJasmin.iaVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Jasmin.ia agents can use during runs.
...
- Jasmin.ia server starts one worker process per installed plugin
...
Receives one typed Jasmin.ia domain event.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@jasminiaai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@jasminiaai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@jasminiaai/plugin-sdk/ui`)
...
Jasmin.ia-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
Jasmin.ia navigation because those can force a full document reload.
...
- Plugin bundles must not import from host internals. They may only import from `@jasminiaai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@jasminiaai/plugin-sdk/ui`
...
The `@jasminiaai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Jasmin.ia secret provider system rather than a plain text input
...
1. Core Jasmin.ia data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Jasmin.ia objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Jasmin.ia product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Jasmin.ia later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Jasmin.ia secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm jasminiaai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Jasmin.ia server. This is a normative requirement, not optional.
...
### 27.1 `@jasminiaai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@jasminiaai/plugin-test-harness";
...
For developing a plugin against a running Jasmin.ia instance:

- The operator installs the plugin from a local path: `pnpm jasminiaai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-jasminia-plugin`) that scaffolds:

- `package.json` with correct `jasminiaPlugin` keys
...
- `@jasminia/plugin-workspace-files`
- `@jasminia/plugin-terminal`
- `@jasminia/plugin-git`
- `@jasminia/plugin-linear`
- `@jasminia/plugin-github-issues`
- `@jasminia/plugin-grafana`
- `@jasminia/plugin-runtime-processes`
- `@jasminia/plugin-stripe`
...
- `@jasminiaai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@jasminiaai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@jasminiaai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@jasminiaai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@jasminiaai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@jasminiaai/plugin-sdk/ui`
...
- `@jasminiaai/plugin-test-harness`
- `create-jasminia-plugin` starter template
...
Jasmin.ia should not implement a generic in-process hook bag modeled directly after local coding tools.

Jasmin.ia should implement:
...
That is the complete target design for the Jasmin.ia plugin system.
