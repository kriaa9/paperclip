...
This guide describes the current, implemented way to create a Jasmin.ia plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Jasmin.ia app.
...
- The host provides a small shared React component kit through
  `@jasminiaai/plugin-sdk/ui`; use it for common Jasmin.ia controls before
...
node packages/plugins/create-jasminia-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Jasmin.ia repo:

```bash
pnpm --filter @jasminiaai/create-jasminia-plugin build
node packages/plugins/create-jasminia-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@jasminiaai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@jasminiaai/plugin-sdk` from the local Jasmin.ia checkout into a `.jasminia-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Jasmin.ia from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
## Managed Jasmin.ia resources

Plugins that provide durable Jasmin.ia business objects should declare them in
...
- Managed agents: declare top-level `agents[]` and require
  ...
  normal Jasmin.ia agents with plugin ownership metadata, not background plugin
  workers.
...
- Managed routines: declare top-level `routines[]` and require
  ...
  jobs that should create visible Jasmin.ia issues. Prefer managed routines over
  ...
  agents: [
    {
      ...
      instructions: {
        content: "Follow the Jasmin.ia heartbeat and produce concise research briefs.",
      },
    },
  ],
  projects: [
...
- Use managed routines for recurring or externally triggered work that should
  ...
  triggers, and each run has the normal Jasmin.ia issue/audit trail.
...
- Keep defaults conservative. Managed declarations are suggestions owned by the
  plugin, but the resulting resources are normal Jasmin.ia records that the
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@jasminiaai/plugin-sdk/ui`
...
Use shared components from `@jasminiaai/plugin-sdk/ui` when the plugin needs a
Jasmin.ia-native control. The host owns the implementation, so plugins inherit
...
Use `FileTree` from `@jasminiaai/plugin-sdk/ui` whenever the plugin only needs
