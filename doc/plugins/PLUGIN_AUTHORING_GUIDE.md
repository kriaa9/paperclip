...
- The host provides a small shared React component kit through
  `@jasminia/plugin-sdk/ui`; use it for common Jasmin.ia controls before
...
Inside this monorepo, the scaffold uses `workspace:*` for `@jasminia/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@jasminia/plugin-sdk` from the local Jasmin.ia checkout into a `.jasminia-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@jasminia/plugin-sdk/ui`
...
Use shared components from `@jasminia/plugin-sdk/ui` when the plugin needs a
...
Use `FileTree` from `@jasminia/plugin-sdk/ui` whenever the plugin only needs
