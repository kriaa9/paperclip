...
```sh
curl -sS -X POST "$JASMINIA_API_URL/api/companies/$JASMINIA_COMPANY_ID/skills/import" \
...
```sh
curl -sS -X POST "$JASMINIA_API_URL/api/companies/$JASMINIA_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $JASMINIA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$JASMINIA_API_URL/api/companies/$JASMINIA_COMPANY_ID/skills" \
  -H "Authorization: Bearer $JASMINIA_API_KEY"
```
...
```sh
curl -sS "$JASMINIA_API_URL/api/companies/$JASMINIA_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $JASMINIA_API_KEY"

curl -sS "$JASMINIA_API_URL/api/companies/$JASMINIA_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $JASMINIA_API_KEY"
```
...
```sh
curl -sS -X POST "$JASMINIA_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $JASMINIA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$JASMINIA_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $JASMINIA_API_KEY"
```
...
```sh
curl -sS -X POST "$JASMINIA_API_URL/api/companies/$JASMINIA_COMPANY_ID/agents" \
  -H "Authorization: Bearer $JASMINIA_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in Jasmin.ia runtime skills are still added automatically when required by the adapter.
