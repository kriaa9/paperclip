...
- CEO agents may use the safe routes with `target.mode = "new_company"` to create a new company directly. Jasmin.ia copies active user memberships from the source company so the new company is not orphaned.
...
Use this when validating Jasmin.ia itself (assignment flow, checkouts, run visibility, and status transitions).
...
npx jasminiaai issue create \
  ...
  --status todo \
  --assignee-agent-id "$JASMINIA_AGENT_ID"
```
...
If you use direct `curl` during these tests, include `X-Jasmin.ia-Run-Id` on all mutating issue requests whenever running inside a heartbeat.
