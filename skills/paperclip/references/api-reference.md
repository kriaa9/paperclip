# Jasmin.ia API Reference

Detailed reference for the Jasmin.ia control plane API. For the core heartbeat procedure and critical rules, see the main `SKILL.md`.
...
There is **no separate execution-decision endpoint**. Review and approval decisions are submitted through `PATCH /api/issues/:issueId`, and Jasmin.ia records the decision row automatically.
...
Jasmin.ia writes the execution decision automatically. If another stage remains, the issue stays in `in_review` and is reassigned to the next participant. If this was the final stage, the issue reaches actual `done`.
...
Jasmin.ia converts that into a `changes_requested` decision, reassigns the issue to `returnAssignee`, and routes it back to the same stage when the executor resubmits.
...
# ^ Load tests depend on caching layer being done first. Jasmin.ia will auto-wake agent-55 when the blocker resolves.
...
{
  "name": "Jasmin.ia Mobile App",
  "description": "Ship iOS + Android client",
  "status": "planned",
  "goalIds": ["{goalId}"],
  "workspace": {
    "name": "jasminia-mobile",
    "cwd": "/Users/me/jasminia-mobile",
    "repoUrl": "https://github.com/acme/jasminia-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
Use `jasminia-create-agent` for the full hiring workflow (reflection + config comparison + prompt drafting).
...
- Use `blockedByIssueIds` for real work dependencies between issues so Jasmin.ia can wake the blocked assignee when all blockers resolve.
...
| Block on another task without `blockedByIssueIds` | No automatic wake when blocker resolves; manual follow-up needed | Set `blockedByIssueIds` so Jasmin.ia auto-wakes the assignee when all blockers are done |
