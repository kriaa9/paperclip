...
Manual local CLI mode (outside heartbeat runs): use `jasminia agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install Jasmin.ia skills for Claude/Codex and print/export the required `JASMINIA_*` environment variables for that agent identity.
