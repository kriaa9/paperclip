...
description: >
  The Jasmin.ia way of converting a plan into executable tasks. Use whenever
  you are asked to plan, scope, or break down work inside a Jasmin.ia company.
  Industry-agnostic guidance on how to translate a plan into assigned issues
  with the right specialty, dependencies, and parallelization so Jasmin.ia's
  executor can pick up the work — it does not prescribe a plan format. Pair
  with the `jasminia` skill, which covers the mechanics of writing the plan
  document and reassigning the issue.
---

# Jasmin.ia — Converting Plans to Tasks

A companion skill for turning a plan into executable Jasmin.ia work. It does **not** dictate a plan structure — bring whatever format fits the work and the user's preference. It tells you _how_ to translate that plan into issues so that the rest of Jasmin.ia works for you.

For the **mechanics** of recording a plan (issue document with key `plan`, comment links, approval gating, who to reassign back to), follow the _Planning_ section of the `jasminia` skill. This skill covers planning method, not the API surface.
...
- **Use the dependency tree.** Jasmin.ia's executor automatically starts any assigned task with no open blockers. Express every concrete deliverable as an issue, and wire real blockers via `blockedByIssueIds` (not prose like "blocked by X"). When `done`, dependents auto-wake.
...
- Not a replacement for the `jasminia` skill's planning mechanics. Use both.
