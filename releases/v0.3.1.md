...
- **Configurable attachment content types** — The `JASMINIA_ALLOWED_ATTACHMENT_TYPES` env var lets operators control which file types can be uploaded. ([#495](# @subhendukundu)
...
- **Tools for Worktree workflow for developers** — New `jasminiaai worktree:make` command provisions isolated development instances with their own database, secrets, favicon branding, and git hooks. Worktrees support minimal seed mode, start-point selection, and automatic workspace rebinding. ([#496](# [#530](# [#545](#
...
- **Remove stale `jasminia` property from OpenClaw Gateway** — Cleaned up an invalid agent parameter that caused warnings. ([#626](# @openagen)
...
Both are additive (new table and new columns) — no existing data is modified. Standard `jasminiaai` startup will apply them automatically.
