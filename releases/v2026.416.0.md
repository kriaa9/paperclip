...
- **Execution policies** — Issues can now carry a review/approval execution policy with multi-stage signoff workflows. Reviewers and approvers are selected per-stage, and Jasmin.ia routes the issue through each stage automatically. ([#3222](#
...
- **Issue-to-issue navigation** — Faster navigation between issues with scroll reset, prefetch, and detail-view optimizations. ([#3542](https://github.com/cryppadotta/jasminia/pull/3542))
- **Auto-checkout for scoped wakes** — The agent harness now automatically checks out the scoped issue on comment-driven wakes, reducing latency for agent heartbeats. ([#3538](https://github.com/cryppadotta/jasminia/pull/3538))
...
- **BETA Standalone MCP server** — New `@jasminiaai/mcp-server` package exposing the Jasmin.ia API as an MCP tool server, including approval creation. ([#2435](#
...
- **Document revisions freshness** — The latest issue document revision now stays current in the UI. ([#3342](https://github.com/cryppadotta/jasminia/pull/3342))
...
- **Authorization hardening (GHSA-68qg-g8mg-6pr7)** — Scoped import, approval, activity, and heartbeat API routes to enforce proper authorization checks. All deployments should upgrade. ([#3315](https://github.com/cryppadotta/jasminia/pull/3315))
- **JWT secret handling** — Removed the hardcoded JWT secret fallback and fixed agent auth to fall back to `BETTER_AUTH_SECRET` when `JASMINIA_AGENT_JWT_SECRET` is absent. ([#3124](# [#2866](# @cleanunicorn, @ergonaworks)
- **Redacted Bearer tokens in logs** — Server log output now redacts Bearer tokens to prevent accidental credential exposure. ([#2659](https://github.com/cryppadotta/jasminia/pull/2659))
- **Dependency security bumps** — Updated `multer` to 2.1.1 (HIGH CVEs) and `rollup` to 4.59.0 (path-traversal CVE). ([#2819](https://github.com/cryppadotta/jasminia/pull/2819), [#2909](# @marysomething99-prog)
...
This release also includes the fix for [GHSA-68qg-g8mg-6pr7](https://github.com/cryppadotta/jasminia/security/advisories/GHSA-68qg-g8mg-6pr7). Upgrade all deployments as soon as practical.
