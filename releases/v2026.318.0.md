...
- **App version label** — The sidebar now displays the running Jasmin.ia version. ([#1096](# @saishankar404)
...
- **Documentation link** — Sidebar documentation link now points to external docs.jasminia.com.
...
- **opencode-local HOME resolution** — `os.userInfo()` is used for model discovery instead of relying on the HOME env var. ([#1145](# @wesseljt)
- **dotenv cwd fallback** — The server now loads `.env` from `cwd` when `.jasminia/.env` is missing. ([#834](# @mvanhorn)
...
- **Agent HOME env var** — `AGENT_HOME` is now set correctly for child agent processes. ([#864](#
...
All migrations are additive (new tables and columns) — no existing data is modified. Standard `jasminiaai` startup will apply them automatically.

If you use the `.env` file, note that the server now falls back to loading `.env` from the current working directory when `.jasminia/.env` is not found.
