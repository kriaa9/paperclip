...
- **`jasminia-dev` skill** — A new opt-in `jasminia-dev` skill ships with Jasmin.ia covering server lifecycle, worktrees, builds, database ops, and diagnostics. Includes open-source hygiene and "push feature branches to a user fork" guidance. ([#3854](# [#4541](# [#4572](#
...
- **AWS ECS Fargate deployment runbook** — End-to-end deployment guide for running Jasmin.ia on ECS Fargate. ([#3897](# @neerazz)
- **Structured agent mentions in jasminia skill** — Skill comments now use structured `[@Name](agent://id)` mentions instead of raw `@Name` strings. ([#4103](# @aronprins)
...
If you previously ran sandbox execution against the in-tree provider, install `@jasminiaai/plugin-e2b` (or your own provider plugin) to keep using sandboxed runtimes after upgrading. The generic provider plugin contract is documented alongside the new environments subsystem.
