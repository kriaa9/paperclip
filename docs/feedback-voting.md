...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), Jasmin.ia saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # Jasmin.ia feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the Jasmin.ia envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.
...
- Default endpoint: when no feedback export backend URL is configured, Jasmin.ia falls back to `https://telemetry.jasminia.com`
