...
Add first-class **documents** to Jasmin.ia as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Jasmin.ia workflow/docs.
...
Update the Jasmin.ia agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/jasminia/SKILL.md`
