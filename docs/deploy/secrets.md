...
Jasmin.ia encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

Jasmin.ia protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. Jasmin.ia does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, Jasmin.ia can no longer guarantee
...
At runtime, Jasmin.ia resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before Jasmin.ia injects its
own `JASMINIA_*` runtime variables.
...
```
~/.jasminia/instances/default/secrets/master.key
```
...
|----------|-------------|
| `JASMINIA_SECRETS_MASTER_KEY` | 32-byte key as base64, hex, or raw string |
| `JASMINIA_SECRETS_MASTER_KEY_FILE` | Custom key file path |
| `JASMINIA_SECRETS_STRICT_MODE` | Set to `true` to enforce secret refs |
...
configuration or `JASMINIA_SECRETS_STRICT_MODE=false`.

## External References

Provider-owned secrets can be linked without copying values into Jasmin.ia by
using `managedMode: "external_reference"` plus a provider `externalRef`.
Jasmin.ia stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted Jasmin.ia Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in Jasmin.ia company secrets**. Allowed credential sources are
workload identity attached to the Jasmin.ia server (instance profile, IRSA, ECS
...
can override) the deployment-level `JASMINIA_SECRETS_AWS_*` env. Bootstrap
...
Jasmin.ia `external_reference` secrets. This is a metadata-only link: Jasmin.ia
...
cursors. Do not expect Jasmin.ia to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the Jasmin.ia runtime
...
- Keep managed create/rotate/delete permissions scoped to the Jasmin.ia
...
dedicated Jasmin.ia runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the Jasmin.ia
  ...
  key bytes.
- `aws_secrets_manager`: back up Jasmin.ia's database for vault metadata
  ...
  configured prefix; restore by pointing the same Jasmin.ia company at the
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in Jasmin.ia. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from Jasmin.ia
...
For Jasmin.ia Cloud, provision the server runtime IAM role/workload identity,
KMS key, deployment prefix, and non-secret `JASMINIA_SECRETS_AWS_*` environment
...
Jasmin.ia secrets. Bootstrap material belongs in infrastructure IAM/workload
...
Use the CLI command for normal operations because it goes through the Jasmin.ia
...
the value custodian; Jasmin.ia stores metadata and provider version references,
