...
Jasmin.ia exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`JASMINIA_API_KEY`)
...
- Run audit trail: include `X-Jasmin.ia-Run-Id` header on all mutating requests during heartbeats
