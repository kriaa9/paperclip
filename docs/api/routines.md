...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Jasmin.ia-Signature` + `X-Jasmin.ia-Timestamp` header pair matching the trigger's signing mode.
