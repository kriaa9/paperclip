...
Jasmin.ia `external_reference` secrets without copying plaintext into
Jasmin.ia.
...
`ListSecrets` only and import stores a Jasmin.ia external reference plus
...
remote metadata, or copies secret plaintext into Jasmin.ia.
...
- `conflict`: the suggested Jasmin.ia `name` or `key` is already in use.
...
Jasmin.ia-managed AWS secret through a broad runtime role.
...
Each row may include an optional Jasmin.ia `description` entered during review;
...
the Jasmin.ia runtime role can list the AWS secret but lacks
