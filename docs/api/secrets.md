...
To link a provider-owned secret without copying the value into Jasmin.ia, create
...
```json
{
  ...
  "externalRef": "arn:aws:secretsmanager:us-east-1:123456789012:secret:jasminia/prod/stripe",
  "providerVersionRef": "version-id-or-label"
}
```

Jasmin.ia stores the provider reference and a non-sensitive fingerprint only.
...
credentials must not be stored in Jasmin.ia `company_secrets`.
...
Remote import links existing AWS Secrets Manager entries into Jasmin.ia as
`external_reference` secrets. Import stores provider reference metadata only; it
does not copy the remote secret plaintext into Jasmin.ia.
...
- `duplicate`: a Jasmin.ia secret already links the same canonical provider
...
`provider_guardrail`. AWS refs under Jasmin.ia's own managed namespace are
blocked as external references; use the Jasmin.ia-managed secret flow for those
resources instead.
...
Jasmin.ia `name`, `key`, optional Jasmin.ia `description`,
`providerVersionRef`, and sanitized `providerMetadata`. Blank descriptions are
stored as `null`; AWS provider descriptions are not copied into Jasmin.ia
...
- `imported`: Jasmin.ia created an active `external_reference` secret and one
...
real value into the agent process environment. Jasmin.ia's custody guarantees
