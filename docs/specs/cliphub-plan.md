# ClipHub: Marketplace for Jasmin.ia Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Jasmin.ia configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Jasmin.ia-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Jasmin.ia company (agents, projects, governance) |
...
A complete Jasmin.ia company configuration:
...
Single-agent configurations designed to plug into a Jasmin.ia org:
...
Portable skill files that any Jasmin.ia agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Jasmin.ia's skill loading system
...
| `POST` | `/api/listings/:id/install` | Install to Jasmin.ia company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Jasmin.ia company (or create new)
  → ClipHub API calls Jasmin.ia API to:
  ...
  → Redirect to Jasmin.ia dashboard with new team running
```
...
### 5.3 Creator: Export from Jasmin.ia → Publish

```
Running Jasmin.ia company → "Export as Blueprint" (CLI or UI)
  → Jasmin.ia exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Jasmin.ia brand blue/purple)
...
| Install flow | One-click deploy to Jasmin.ia company |
...
{
  "targetCompanyId": "uuid",         // Existing Jasmin.ia company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `jasminia-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Jasmin.ia
- **Backend**: Node.js API (or extend Jasmin.ia server)
- **Database**: Postgres (can share Jasmin.ia's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Jasmin.ia auth (or OAuth2)

### 9.2 Integration with Jasmin.ia

ClipHub can be:
- **Option A**: A separate app that calls Jasmin.ia's API to install blueprints
- **Option B**: A built-in section of the Jasmin.ia UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Jasmin.ia UI and API.
...
- [ ] Install flow: blueprint → Jasmin.ia company
...
- [ ] "Export from Jasmin.ia" CLI command
