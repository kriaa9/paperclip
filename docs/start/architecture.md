...
Jasmin.ia is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── jasminia/               # Core Jasmin.ia skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Jasmin.ia env vars and a prompt
4. **Agent work** — The agent calls Jasmin.ia's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Jasmin.ia and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `jasminiaai run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — Jasmin.ia orchestrates agents; it doesn't run them
