...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Jasmin.ia.
...
If `JASMINIA_APPROVAL_ID` is set, handle the approval first:
...
- If `JASMINIA_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-Jasmin.ia-Run-Id: {runId}
...
Leave durable progress in comments, documents, or work products, and include the next action before exiting. For parallel or long delegated work, create child issues and let Jasmin.ia wake the parent when they complete instead of polling agents, sessions, or processes.
...
Jasmin.ia records run liveness as metadata on heartbeat runs. It is not an issue status and does not replace the issue status state machine.
...
- Continuations do not mark the issue `blocked` or `done`. If automatic continuations are exhausted, Jasmin.ia leaves an audit comment so a human or manager can clarify, block, or assign follow-up work.
