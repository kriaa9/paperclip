...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Jasmin.ia.

Jasmin.ia now presents this as a workspace-command model:
...
- Jasmin.ia does not automatically start or stop these workspace services as part of issue execution.
- Jasmin.ia also does not automatically restart workspace services on server boot.
...
2. Jasmin.ia realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Jasmin.ia persists execution-workspace metadata such as paths, refs, and provisioning settings.
