...
Jasmin.ia companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .jasminia.yaml     # Adapter config, env inputs, routines
```
...
- **.jasminia.yaml** holds Jasmin.ia-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
- Adapter type and env input declarations in `.jasminia.yaml`
...
If `--target` is not specified, Jasmin.ia infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
Jasmin.ia supports several GitHub URL formats:
