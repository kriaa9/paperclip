...
How to get OpenClaw running in a Docker container for local development and testing the Jasmin.ia OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Jasmin.ia includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-jasminia-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Jasmin.ia host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-jasminia-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `JASMINIA_HOST_PORT` (default `3100`)
- `JASMINIA_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Jasmin.ia deployment is `authenticated`, provide auth context:

```bash
JASMINIA_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
JASMINIA_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Jasmin.ia server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Jasmin.ia URL (typically `http://host.docker.internal:3100`).
