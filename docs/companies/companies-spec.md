...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Jasmin.ia.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.jasminia.yaml`
...
- Jasmin.ia must not require extra top-level fields for skill validity
- Jasmin.ia-specific extensions must live under `metadata.jasminia` or `metadata.sources`
...
For Jasmin.ia, the preferred fidelity extension is:
...
- Jasmin.ia tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Jasmin.ia shape:
...
Additional rules for Jasmin.ia exporters:
...
- omit empty and default-valued Jasmin.ia fields when possible
...
- Jasmin.ia exporters should emit `.jasminia.yaml` as a sidecar by default
...
## 19. Jasmin.ia Mapping

Jasmin.ia can map this spec to its runtime model like this:
...
- Jasmin.ia extension:
  - `.jasminia.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Jasmin.ia-specific fidelity

Inline Jasmin.ia-only metadata that must live inside a shared markdown file should use:

- `metadata.jasminia`

That keeps the base format broader than Jasmin.ia.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Jasmin.ia.

## 20. Cutover

Jasmin.ia should cut over to this markdown-first package model as the primary portability format.

`jasminia.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Jasmin.ia, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
