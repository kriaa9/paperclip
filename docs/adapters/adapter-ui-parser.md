...
│  Jasmin.ia Server  │◄────────────────────────────────│  uiParserCache    │
│  (in-memory)      │                                 └──────────────────┘
└────────┬─────────┘
         │ serves JS to browser
         ▼
┌──────────────────┐   fetch() + eval   ┌──────────────────┐
│  Jasmin.ia UI     │─────────────────────→│  parseStdoutLine │
...
### 1. `jasminia.adapterUiParser` — contract version
...
The Jasmin.ia host checks this field. If the major version is unsupported, the host logs a warning and falls back to the generic parser instead of executing potentially incompatible code.
