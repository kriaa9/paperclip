...
summary: Build, package, and distribute adapters as plugins without modifying Jasmin.ia source
---

Jasmin.ia supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Jasmin.ia's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `jasminia-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Jasmin.ia | Published to npm or linked via `file:` |
| Updates | Requires Jasmin.ia release | Independent versioning |
...
| `jasminia.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```sh
# Via the Jasmin.ia UI
# Settings → Adapters → Install from npm → "my-jasminia-adapter"
...
Local adapters are symlinked into Jasmin.ia's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.jasminia/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-jasminia-adapter",
...
Other Jasmin.ia users can then install your adapter by package name from the UI or API.
