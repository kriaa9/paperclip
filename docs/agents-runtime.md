...
Audience: Operators setting up and running agents in Jasmin.ia

## 1. What this system does

Agents in Jasmin.ia do not run continuously.  
...
- `hermes_local`: runs your local `hermes` CLI (`hermes-jasminia-adapter`)
...
- `droid_local`: runs your local Factory Droid CLI (`@henkey/droid-jasminia-adapter`)

For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), Jasmin.ia assumes the CLI is already installed and authenticated on the host machine.
...
Jasmin.ia stores session IDs for resumable adapters.
...
Jasmin.ia pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. Jasmin.ia surfaces this as a warning in environment tests, not a hard error.
