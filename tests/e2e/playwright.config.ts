...
const PORT = Number(process.env.JASMINIA_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const JASMINIA_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-e2e-home-"));
...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      ...
      PORT: String(PORT),
      JASMINIA_HOME,
      JASMINIA_INSTANCE_ID: "playwright-e2e",
      JASMINIA_BIND: "loopback",
      JASMINIA_DEPLOYMENT_MODE: "local_trusted",
