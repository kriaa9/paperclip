...
const BASE = process.env.JASMINIA_E2E_BASE_URL ?? "http://127.0.0.1:3105";
const DATA_DIR = process.env.JASMINIA_E2E_DATA_DIR ?? process.env.JASMINIA_HOME;
const CONFIG_PATH = process.env.JASMINIA_E2E_CONFIG_PATH ?? path.resolve(process.cwd(), ".jasminia/config.json");
const BOOTSTRAP_SCRIPT_PATH = path.resolve(process.cwd(), "packages/db/scripts/create-auth-bootstrap-invite.ts");
const OWNER_PASSWORD = "jasminia-owner-password";
const INVITED_PASSWORD = "jasminia-invited-password";
...
const ownerUser: HumanUser = {
  name: "Owner User",
  email: `owner-${runId}@jasminia.local`,
  password: OWNER_PASSWORD,
};
const invitedUser: HumanUser = {
  name: "Invited User",
  email: `invitee-${runId}@jasminia.local`,
  password: INVITED_PASSWORD,
};

function createBootstrapInvite() {
  if (!DATA_DIR) {
    throw new Error("JASMINIA_E2E_DATA_DIR or JASMINIA_HOME is required for authenticated bootstrap tests");
  }
...
async function signUp(page: Page, user: HumanUser) {
  ...
  await expect(page.getByRole("heading", { name: "Sign in to Jasmin.ia" })).toBeVisible();
  await page.getByRole("button", { name: "Create one" }).click();
  await expect(page.getByRole("heading", { name: "Create your Jasmin.ia account" })).toBeVisible();
...
async function acceptBootstrapInvite(page: Page, inviteUrl: string) {
  ...
  await expect(page.getByRole("heading", { name: "Bootstrap your Jasmin.ia instance" })).toBeVisible();
