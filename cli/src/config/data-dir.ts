...
export function applyDataDirOverride(
  ...
  process.env.JASMINIA_HOME = resolvedDataDir;
  ...
  if (support.hasContextOption) {
    const hasContextOverride = Boolean(options.context?.trim()) || Boolean(process.env.JASMINIA_CONTEXT?.trim());
    if (!hasContextOverride) {
      process.env.JASMINIA_CONTEXT = resolveDefaultContextPath();
    }
  }
