...
export function secretsCheck(config: JasminiaConfig, configPath?: string): CheckResult {
  ...
  const envMasterKey = process.env.JASMINIA_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    if (!decodeMasterKey(envMasterKey)) {
      return {
        ...
        status: "fail",
        message:
          "JASMINIA_SECRETS_MASTER_KEY is invalid (expected 32-byte base64, 64-char hex, or raw 32-char string)",
        canRepair: false,
        repairHint: "Set JASMINIA_SECRETS_MASTER_KEY to a valid key or unset it to use a key file",
      };
    }
    ...
    return withStrictModeNote(
      {
        name: "Secrets adapter",
        status: "pass",
        message: "Local encrypted provider configured via JASMINIA_SECRETS_MASTER_KEY",
      },
      config,
    );
  }

  const keyFileOverride = process.env.JASMINIA_SECRETS_MASTER_KEY_FILE;
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Check file permissions or set JASMINIA_SECRETS_MASTER_KEY",
    };
  }
...
function awsSecretsManagerCheck(): CheckResult {
  ...
  if (missingConfig.length > 0) {
    return {
      ...
      canRepair: false,
      repairHint:
        `Set ${missingConfig.join(", ")} in the Jasmin.ia server runtime. ${AWS_CREDENTIAL_SOURCE_HINT}. Do not store AWS root credentials or long-lived IAM user keys in Jasmin.ia secrets.`,
    };
  }
  ...
  const message =
    `AWS Secrets Manager provider configured for deployment ${process.env.JASMINIA_SECRETS_AWS_DEPLOYMENT_ID}; ` +
  ...
  if (staticEnvCredentials) {
    return {
      ...
      canRepair: false,
      repairHint:
        "AWS static environment credentials are visible. Use only short-lived shell credentials locally; prefer IAM role/workload identity for hosted deployments and never store AWS access keys in Jasmin.ia company secrets.",
    };
  }
...
function missingAwsSecretsManagerConfig(): string[] {
  ...
  if (
    ...
    missing.push("JASMINIA_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION");
  }
  if (!process.env.JASMINIA_SECRETS_AWS_DEPLOYMENT_ID?.trim()) {
    missing.push("JASMINIA_SECRETS_AWS_DEPLOYMENT_ID");
  }
  if (!process.env.JASMINIA_SECRETS_AWS_KMS_KEY_ID?.trim()) {
    missing.push("JASMINIA_SECRETS_AWS_KMS_KEY_ID");
  }
  return missing;
}
