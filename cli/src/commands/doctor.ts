...
export async function doctor(opts: {
  ...
  p.intro(pc.bgCyan(pc.black(" jasminia doctor ")));
  ...
  try {
    ...
    const readResult: CheckResult = {
      ...
      canRepair: false,
      repairHint: "Run `jasminiaai configure --section database` or `jasminiaai onboard`",
    };
...
async function runRepairableCheck(input: {
  ...
  loadJasmin.iaEnvFile(input.configPath);
