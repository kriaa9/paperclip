...
export const DEFAULT_WORKTREE_HOME = "~/.jasminia-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".jasminia");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: Jasmin.iaConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "jasminia",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    JASMINIA_HOME: paths.homeDir,
    JASMINIA_INSTANCE_ID: paths.instanceId,
    JASMINIA_CONFIG: paths.configPath,
    JASMINIA_CONTEXT: paths.contextPath,
    JASMINIA_IN_WORKTREE: "true",
    ...(branding?.name ? { JASMINIA_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { JASMINIA_WORKTREE_COLOR: branding.color } : {}),
  };
}
