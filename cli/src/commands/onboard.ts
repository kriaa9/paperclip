...
const ONBOARD_ENV_KEYS = [
  "JASMINIA_PUBLIC_URL",
  "DATABASE_URL",
  "JASMINIA_DB_BACKUP_ENABLED",
  "JASMINIA_DB_BACKUP_INTERVAL_MINUTES",
  "JASMINIA_DB_BACKUP_RETENTION_DAYS",
  "JASMINIA_DB_BACKUP_DIR",
  "JASMINIA_DEPLOYMENT_MODE",
  "JASMINIA_DEPLOYMENT_EXPOSURE",
  "JASMINIA_BIND",
  "JASMINIA_BIND_HOST",
  "JASMINIA_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "JASMINIA_ALLOWED_HOSTNAMES",
  "JASMINIA_AUTH_BASE_URL_MODE",
  "JASMINIA_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "JASMINIA_STORAGE_PROVIDER",
  "JASMINIA_STORAGE_LOCAL_DIR",
  "JASMINIA_STORAGE_S3_BUCKET",
  "JASMINIA_STORAGE_S3_REGION",
  "JASMINIA_STORAGE_S3_ENDPOINT",
  "JASMINIA_STORAGE_S3_PREFIX",
  "JASMINIA_STORAGE_S3_FORCE_PATH_STYLE",
  "JASMINIA_SECRETS_PROVIDER",
  "JASMINIA_SECRETS_STRICT_MODE",
  "JASMINIA_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.JASMINIA_PUBLIC_URL?.trim() ||
      process.env.JASMINIA_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.JASMINIA_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.JASMINIA_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.JASMINIA_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.JASMINIA_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.JASMINIA_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.JASMINIA_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.JASMINIA_ALLOWED_HOSTNAMES
    ? process.env.JASMINIA_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.JASMINIA_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.JASMINIA_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.JASMINIA_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.JASMINIA_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.JASMINIA_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.JASMINIA_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.JASMINIA_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.JASMINIA_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.JASMINIA_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.JASMINIA_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.JASMINIA_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.JASMINIA_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.JASMINIA_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.JASMINIA_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.JASMINIA_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "JASMINIA_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.JASMINIA_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "JASMINIA_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.JASMINIA_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "JASMINIA_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  if (existingConfig) {
    ...
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("JASMINIA_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.JASMINIA_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("JASMINIA_AGENT_JWT_SECRET")} from environment`);
    } else {
    ...
    p.note(
      [
        ...
        "Agent auth: JASMINIA_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.JASMINIA_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Jasmin.ia setup is ready.");
    return;
  }
