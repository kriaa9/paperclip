...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://jasminia:jasminia@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "jasminia");
      const connectionString = `postgres://jasminia:jasminia@127.0.0.1:${embeddedHandle.port}/jasminia`;
...
export async function disableAllRoutinesInConfig(
  ...
  const companyId =
    ...
    ?? null;
  if (!companyId) {
    throw new Error("Company ID is required. Pass --company-id or set JASMINIA_COMPANY_ID.");
  }
