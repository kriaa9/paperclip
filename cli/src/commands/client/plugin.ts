    ...
    plugin
      ...
      .description(
        "Install a plugin from a local path or npm package.\n" +
          "  Examples:\n" +
          "    jasminia plugin install ./my-plugin              # local path\n" +
          "    jasminia plugin install @acme/plugin-linear      # npm package\n" +
          "    jasminia plugin install @acme/plugin-linear@1.2  # pinned version",
      )
          ...
          for (const ex of rows) {
            console.log(
              `${pc.bold(ex.displayName)}  ${pc.dim(ex.pluginKey)}\n` +
                `  ${ex.description}\n` +
                `  ${pc.cyan(`jasminia plugin install ${ex.localPath}`)}`,
            );
          }
        } catch (err) {
          handleCommandError(err);
