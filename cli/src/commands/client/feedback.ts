...
import type { Company, FeedbackTrace, FeedbackTraceBundle } from "@jasminiaai/shared";
import {
...
export async function resolveFeedbackCompanyId(
  ...
  if (!companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set JASMINIA_COMPANY_ID, or configure a CLI context default.",
    );
  }
  return companyId;
}
...
export function renderFeedbackReport(input: {
  ...
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("Jasmin.ia Feedback Report")));
...
export function renderFeedbackExportSummary(exported: FeedbackExportResult): string {
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("Jasmin.ia Feedback Export")));
