...
function buildAgentEnvExports(input: {
  ...
  return [
    `export JASMINIA_API_URL='${escaped(input.apiBase)}'`,
    `export JASMINIA_COMPANY_ID='${escaped(input.companyId)}'`,
    `export JASMINIA_AGENT_ID='${escaped(input.agentId)}'`,
    `export JASMINIA_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Jasmin.ia skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Jasmin.ia skills into ~/.codex/skills and ~/.claude/skills",
      )
