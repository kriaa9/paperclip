...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Jasmin.ia config file")
    .option("-d, --data-dir <path>", "Jasmin.ia data directory root (isolates state from ~/.jasminia)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Jasmin.ia API")
...
export function resolveCommandContext(
  ...
  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.JASMINIA_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.JASMINIA_COMPANY_ID?.trim() ||
    profile.companyId;
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.JASMINIA_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.JASMINIA_SERVER_PORT || "");
