...
function findContextFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".jasminia", DEFAULT_CONTEXT_BASENAME);
...
export function resolveContextPath(overridePath?: string): string {
  ...
  if (process.env.JASMINIA_CONTEXT) return path.resolve(process.env.JASMINIA_CONTEXT);
