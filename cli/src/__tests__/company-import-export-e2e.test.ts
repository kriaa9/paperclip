...
function createBaseJasmin.iaEnv(options: TestJasmin.iaEnv) {
  ...
  env.JASMINIA_CONFIG = options.configPath;
  env.JASMINIA_HOME = options.jasminiaHome;
  env.JASMINIA_INSTANCE_ID = options.instanceId;
  env.JASMINIA_CONTEXT = path.join(options.jasminiaHome, "context.json");
  env.JASMINIA_AUTH_STORE = path.join(options.jasminiaHome, "auth.json");
...
function createServerEnv(
  ...
  env.SERVE_UI = "false";
  env.JASMINIA_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.JASMINIA_MIGRATION_AUTO_APPLY = "true";
  env.JASMINIA_UI_DEV_MIDDLEWARE = "false";
...
function createCliEnv(options: TestJasmin.iaEnv) {
  ...
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.JASMINIA_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.JASMINIA_MIGRATION_AUTO_APPLY;
  delete env.JASMINIA_UI_DEV_MIDDLEWARE;
  return env;
}
...
describeEmbeddedPostgres("jasminiaai company import/export e2e", () => {
  ...
  let jasminiaHome = "";
  let cliShellHome = "";
  let jasminiaInstanceId = "";
  let serverProcess: ServerProcess | null = null;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "jasminia-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");
    jasminiaHome = path.join(tempRoot, "jasminia-home");
    cliShellHome = path.join(tempRoot, "shell-home");
    jasminiaInstanceId = "company-cli-e2e";
    mkdirSync(jasminiaHome, { recursive: true });
    mkdirSync(cliShellHome, { recursive: true });

    tempDb = await startEmbeddedPostgresTestDatabase("jasminia-company-cli-db-");
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    const expectedContextPath = path.join(jasminiaHome, "context.json");
    const leakedContextPath = path.join(cliShellHome, ".jasminia", "context.json");
    ...
    expect(readFileSync(path.join(exportDir, ".jasminia.yaml"), "utf8")).toContain('schema: "jasminia/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "jasminia-demo"));
