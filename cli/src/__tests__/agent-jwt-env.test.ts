...
function tempConfigPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-jwt-env-"));
...
describe("agent jwt env helpers", () => {
  ...
  it("writes .env next to explicit config path", () => {
    ...
    expect(contents).toContain("JASMINIA_AGENT_JWT_SECRET=");
  });

  it("loads secret from .env next to explicit config path", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "JASMINIA_AGENT_JWT_SECRET=test-secret\n", { mode: 0o600 });

    const loaded = readAgentJwtSecretFromEnv(configPath);
    expect(loaded).toBe("test-secret");
    expect(process.env.JASMINIA_AGENT_JWT_SECRET).toBe("test-secret");
  });

  it("doctor check passes when secret exists in adjacent .env", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "JASMINIA_AGENT_JWT_SECRET=check-secret\n", { mode: 0o600 });
