...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.JASMINIA_HOME = path.join(root, "home");
    process.env.JASMINIA_INSTANCE_ID = "telemetry-test";
