...
function configWithSecretsProvider(provider: Jasmin.iaConfig["secrets"]["provider"]): Jasmin.iaConfig {
  return {
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/jasminia/db",
      embeddedPostgresPort: 55432,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/jasminia/backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/jasminia/logs",
    },
    server: {
    ...
    secrets: {
      ...
      strictMode: true,
      localEncrypted: {
        keyFilePath: "/tmp/jasminia/secrets/master.key",
      },
    },
  };
}
...
describe("secrets CLI helpers", () => {
  ...
  it("reports the AWS bootstrap config required by doctor", () => {
    ...
    expect(result.message).toContain("JASMINIA_SECRETS_AWS_DEPLOYMENT_ID");
  ...
  it("passes AWS doctor checks when non-secret provider config is present", () => {
    process.env.JASMINIA_SECRETS_AWS_REGION = "us-east-1";
    process.env.JASMINIA_SECRETS_AWS_DEPLOYMENT_ID = "prod-us-1";
    process.env.JASMINIA_SECRETS_AWS_KMS_KEY_ID =
      "arn:aws:kms:us-east-1:123456789012:key/test";
    process.env.AWS_PROFILE = "jasminia-prod";
