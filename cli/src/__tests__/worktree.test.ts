...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("jasminia-pr-432")).toBe(
      path.resolve(os.homedir(), "jasminia-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("jasminia/pr-432")).toThrow(
  ...
  it("rewrites auth URLs only when they already include a port", () => {
    ...
    expect(rewriteLocalUrlPort("https://jasminia.example", 3110)).toBe("https://jasminia.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/jasminia-feature",
      homeDir: "/tmp/jasminia-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/jasminia-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/jasminia-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.JASMINIA_HOME).toBe(path.resolve("/tmp/jasminia-worktrees"));
    expect(env.JASMINIA_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.JASMINIA_IN_WORKTREE).toBe("true");
    expect(env.JASMINIA_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.JASMINIA_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export JASMINIA_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  itEmbeddedPostgres("quarantines copied live execution state in seeded worktree databases", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("jasminia-worktree-quarantine-");
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-secrets-"));
    const originalInlineMasterKey = process.env.JASMINIA_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.JASMINIA_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.JASMINIA_SECRETS_MASTER_KEY;
      delete process.env.JASMINIA_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.JASMINIA_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.JASMINIA_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.JASMINIA_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          JASMINIA_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.JASMINIA_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.JASMINIA_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".jasminia-worktrees"),
      });

      const envPath = path.join(repoRoot, ".jasminia", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("JASMINIA_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("JASMINIA_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/JASMINIA_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.JASMINIA_AGENT_JWT_SECRET;
      } else {
        process.env.JASMINIA_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  itEmbeddedPostgres(
    ...
    async () => {
      const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-auth-seed-"));
      ...
      const worktreeHome = path.join(tempRoot, ".jasminia-worktrees");
      const originalCwd = process.cwd();
      const sourceDb = await startEmbeddedPostgresTestDatabase("jasminia-worktree-auth-source-");

      try {
        const sourceDbClient = createDb(sourceDb.connectionString);
        await sourceDbClient.insert(authUsers).values({
          id: "user-existing",
          email: "existing@jasminia.com",
          name: "Existing User",
          emailVerified: true,
          createdAt: new Date(),
          updatedAt: new Date(),
        ...
        try {
          const targetDb = createDb(
            `postgres://jasminia:jasminia@127.0.0.1:${targetConfig.database.embeddedPostgresPort}/jasminia`,
          );
          const seededUsers = await targetDb.select().from(authUsers);
          expect(seededUsers.some((row) => row.email === "existing@jasminia.com")).toBe(true);
        } finally {
          await targetPg.stop();
        }
      } finally {
  ...
  it("avoids ports already claimed by sibling worktree instance configs", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-claimed-ports-"));
    const repoRoot = path.join(tempRoot, "repo");
    const homeDir = path.join(tempRoot, ".jasminia-worktrees");
    ...
    try {
      ...
      const config = JSON.parse(fs.readFileSync(path.join(repoRoot, ".jasminia", "config.json"), "utf8"));
  ...
  it("defaults the seed source config to the current repo-local Jasmin.ia config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("derives worktree reseed target paths from the adjacent env file", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-reseed-target-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".jasminia", "config.json");
    const envPath = path.join(worktreeRoot, ".jasminia", ".env");
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          ...
          "JASMINIA_INSTANCE_ID=pap-1132-chat",
        ].join("\n"),
        "utf8",
      );
      expect(
  ...
  it("rejects reseed targets without worktree env metadata", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-reseed-target-missing-"));
    const worktreeRoot = path.join(tempRoot, "repo");
    const configPath = path.join(worktreeRoot, ".jasminia", "config.json");
    ...
    try {
      fs.mkdirSync(path.dirname(configPath), { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(buildSourceConfig()), "utf8");
      fs.writeFileSync(path.join(worktreeRoot, ".jasminia", ".env"), "", "utf8");

      expect(() =>
        resolveWorktreeReseedTargetPaths({
          configPath,
          rootPath: worktreeRoot,
        })).toThrow("does not look like a worktree-local Jasmin.ia instance");
    } finally {
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("reseed preserves the current worktree ports, instance id, and branding", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-reseed-"));
    const repoRoot = path.join(tempRoot, "repo");
    ...
    const sourcePaths = resolveWorktreeLocalPaths({
      cwd: sourceRoot,
      homeDir: path.join(tempRoot, ".jasminia-source"),
      instanceId: "default",
    });
    const originalCwd = process.cwd();
    const originalJasmin.iaConfig = process.env.JASMINIA_CONFIG;
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        [
          `JASMINIA_HOME=${homeDir}`,
          `JASMINIA_INSTANCE_ID=${currentInstanceId}`,
          "JASMINIA_WORKTREE_NAME=existing-name",
          "JASMINIA_WORKTREE_COLOR=\"#112233\"",
        ].join("\n"),
        "utf8",
      );
      ...
      expect(rewrittenEnv).toContain(`JASMINIA_INSTANCE_ID=${currentInstanceId}`);
      expect(rewrittenEnv).toContain("JASMINIA_WORKTREE_NAME=existing-name");
  ...
  it("restores the current worktree config and instance data if reseed fails", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-reseed-rollback-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceRoot = path.join(tempRoot, "source");
    const homeDir = path.join(tempRoot, ".jasminia-worktrees");
    ...
    try {
      ...
      expect(restoredEnv).toContain(`JASMINIA_INSTANCE_ID=${currentInstanceId}`);
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/jasminia",
        targetRepoRoot: "/Users/example/jasminia-pr-432",
        workspaceCwd: "/Users/example/jasminia",
      }),
    ).toBe("/Users/example/jasminia-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/jasminia",
        targetRepoRoot: "/Users/example/jasminia-pr-432",
        workspaceCwd: "/Users/example/jasminia/packages/db",
      }),
    ).toBe("/Users/example/jasminia-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "jasminia-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("jasminia-make-test", {
        seed: false,
        home: path.join(tempRoot, ".jasminia-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".jasminia", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".jasminia", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
  ...
  it("no-ops on the primary checkout unless --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-repair-primary-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      expect(fs.existsSync(path.join(repoRoot, ".jasminia", "config.json"))).toBe(false);
      expect(fs.existsSync(path.join(repoRoot, ".jasminia", "worktrees"))).toBe(false);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  ...
  it("repairs the current linked worktree when Jasmin.ia metadata is missing", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-repair-current-"));
    const repoRoot = path.join(tempRoot, "repo");
    const worktreePath = path.join(repoRoot, ".jasminia", "worktrees", "repair-me");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".jasminia-worktrees");
  ...
  it("creates and repairs a missing branch worktree when --branch is provided", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-worktree-repair-branch-"));
    const repoRoot = path.join(tempRoot, "repo");
    const sourceConfigPath = path.join(tempRoot, "source-config.json");
    const worktreeHome = path.join(tempRoot, ".jasminia-worktrees");
    const originalCwd = process.cwd();
    const expectedWorktreePath = path.join(repoRoot, ".jasminia", "worktrees", "feature-repair-me");
    ...
    try {
      ...
      expect(fs.existsSync(path.join(expectedWorktreePath, ".jasminia", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(expectedWorktreePath, ".jasminia", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
...
describeEmbeddedPostgres("pauseSeededScheduledRoutines", () => {
  it("pauses only routines with enabled schedule triggers", async () => {
    const tempDb = await startEmbeddedPostgresTestDatabase("jasminia-worktree-routines-");
    const db = createDb(tempDb.connectionString);
    const companyId = randomUUID();
