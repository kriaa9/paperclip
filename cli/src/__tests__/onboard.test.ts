...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-onboard-"));
  const runtimeRoot = path.join(root, "runtime");
  const configPath = path.join(root, ".jasminia", "config.json");
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-onboard-fresh-"));
  return path.join(root, ".jasminia", "config.json");
}
...
describe("onboard", () => {
  beforeEach(() => {
    ...
    delete process.env.JASMINIA_BIND;
    delete process.env.JASMINIA_BIND_HOST;
    delete process.env.JASMINIA_TAILNET_BIND_HOST;
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.JASMINIA_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as Jasmin.iaConfig;
  ...
  it("creates instance-root config and data paths for a fresh JASMINIA_HOME", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-onboard-home-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "jasminia-onboard-cwd-"));
    process.chdir(cwd);
  ...
  it("supports authenticated/private quickstart bind presets", async () => {
    const configPath = createFreshConfigPath();
    process.env.JASMINIA_TAILNET_BIND_HOST = "100.64.0.8";
  ...
  it("ignores deployment env overrides during --yes quickstart", async () => {
    const configPath = createFreshConfigPath();
    process.env.JASMINIA_DEPLOYMENT_MODE = "authenticated";
