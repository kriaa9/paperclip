/**
 * esbuild configuration for building the jasminia CLI for npm.
 *
 * Bundles all workspace packages (@jasminia/*) into a single file.
...
// to npm and resolved at runtime (e.g. @jasminia/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@jasminia/")) {
      externals.add(name);
    }
  }
