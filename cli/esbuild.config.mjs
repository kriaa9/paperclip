/**
 * esbuild configuration for building the jasminiaai CLI for npm.
 *
 * Bundles all workspace packages (@jasminiaai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @jasminiaai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@jasminiaai/")) {
      externals.add(name);
    }
  }
